...
if (isset($_GET['e']) && !phpbb::$user->data['is_registered'])
{
	login_box('', phpbb::$user->lang['LOGIN_NOTIFY_FORUM']);
}

// Permissions check
if (!phpbb::$auth->acl_gets('f_list', 'f_read', $forum_id) || ($forum_data['forum_type'] == FORUM_LINK && $forum_data['forum_link'] && !phpbb::$auth->acl_get('f_read', $forum_id)))
{
...
page_header(phpbb::$user->lang['VIEW_FORUM'] . ' - ' . $forum_data['forum_name'], true, $forum_id);
...
phpbb::$template->assign_vars(array(
	'U_VIEW_FORUM'			=> append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, "f=$forum_id" . (($start == 0) ? '' : "&amp;start=$start")),
));
...
if ($mark_read == 'topics')
{
	...
	$redirect_url = append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $forum_id);
...
{
	phpbb::$template->assign_var('RUN_CRON_TASK', '<img src="' . append_sid(phpbb::$phpbb_root_path . 'cron.' . $phpEx, 'cron_type=prune_forum&amp;f=' . $forum_id) . '" alt="cron" width="1" height="1" />');
}
...
if ((phpbb::$config['email_enable'] || phpbb::$config['jab_enable']) && phpbb::$config['allow_forum_notify'] && $forum_data['forum_type'] == FORUM_POST && phpbb::$auth->acl_get('f_subscribe', $forum_id))
{
...
if ($sort_days)
{
	...
	$sql = 'SELECT COUNT(topic_id) AS num_topics
		...
		" . ((phpbb::$auth->acl_get('m_approve', $forum_id)) ? '' : 'AND topic_approved = 1');
	$result = phpbb::$db->sql_query($sql);
...
{
	$topics_count = (phpbb::$auth->acl_get('m_approve', $forum_id)) ? $forum_data['forum_topics_real'] : $forum_data['forum_topics'];
	$sql_limit_time = '';
}
...
phpbb::$template->assign_vars(array(
	...
	'FOLDER_IMG'				=> phpbb::$user->img('topic_read', 'NO_UNREAD_POSTS'),
	'FOLDER_UNREAD_IMG'			=> phpbb::$user->img('topic_unread', 'UNREAD_POSTS'),
	'FOLDER_HOT_IMG'			=> phpbb::$user->img('topic_read_hot', 'NO_UNREAD_POSTS_HOT'),
	'FOLDER_HOT_UNREAD_IMG'		=> phpbb::$user->img('topic_unread_hot', 'UNREAD_POSTS_HOT'),
	'FOLDER_LOCKED_IMG'			=> phpbb::$user->img('topic_read_locked', 'NO_UNREAD_POSTS_LOCKED'),
	'FOLDER_LOCKED_UNREAD_IMG'	=> phpbb::$user->img('topic_unread_locked', 'UNREAD_POSTS_LOCKED'),
	'FOLDER_STICKY_IMG'			=> phpbb::$user->img('sticky_read', 'POST_STICKY'),
	'FOLDER_STICKY_UNREAD_IMG'	=> phpbb::$user->img('sticky_unread', 'POST_STICKY'),
	'FOLDER_ANNOUNCE_IMG'		=> phpbb::$user->img('announce_read', 'POST_ANNOUNCEMENT'),
	'FOLDER_ANNOUNCE_UNREAD_IMG'=> phpbb::$user->img('announce_unread', 'POST_ANNOUNCEMENT'),
	...
	'S_DISPLAY_POST_INFO'	=> ($forum_data['forum_type'] == FORUM_POST && (phpbb::$auth->acl_get('f_post', $forum_id) || phpbb::$user->data['user_id'] == ANONYMOUS)) ? true : false,

	'S_IS_POSTABLE'			=> ($forum_data['forum_type'] == FORUM_POST) ? true : false,
	'S_USER_CAN_POST'		=> (phpbb::$auth->acl_get('f_post', $forum_id)) ? true : false,
	...
	'S_FORUM_ACTION'		=> append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, "f=$forum_id" . (($start == 0) ? '' : "&amp;start=$start")),
	...
	'U_MCP'				=> (phpbb::$auth->acl_get('m_', $forum_id)) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "f=$forum_id&amp;i=main&amp;mode=forum_view", true, phpbb::$user->session_id) : '',
	...
	'U_VIEW_FORUM'		=> append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, "f=$forum_id" . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '') . (($start == 0) ? '' : "&amp;start=$start")),
	'U_MARK_TOPICS'		=> (phpbb::$user->data['is_registered'] || phpbb::$config['load_anon_lastread']) ? append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'hash=' . generate_link_hash('global') . "&amp;f=$forum_id&amp;mark=topics") : '',
));
...
$sql_approved = (phpbb::$auth->acl_get('m_approve', $forum_id)) ? '' : 'AND t.topic_approved = 1';
...
phpbb::$template->assign_vars(array(
	'PAGINATION'	=> generate_pagination(append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, "f=$forum_id" . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '')), $topics_count, phpbb::$config['topics_per_page'], $start),
...
if (sizeof($topic_list))
{
	...
	if ($s_display_active && sizeof($active_forum_ary))
	{
		...
		{
			$topic_forum_list[$row['forum_id']]['forum_mark_time'] = (phpbb::$config['load_db_lastread'] && phpbb::$user->data['is_registered'] && isset($row['forum_mark_time'])) ? $row['forum_mark_time'] : 0;
	...
	foreach ($topic_list as $topic_id)
	{
		...
		// Replies
		$replies = (phpbb::$auth->acl_get('m_approve', $topic_forum_id)) ? $row['topic_replies_real'] : $row['topic_replies'];
		...
		$topic_unapproved = (!$row['topic_approved'] && phpbb::$auth->acl_get('m_approve', $topic_forum_id)) ? true : false;
		$posts_unapproved = ($row['topic_approved'] && $row['topic_replies'] < $row['topic_replies_real'] && phpbb::$auth->acl_get('m_approve', $topic_forum_id)) ? true : false;
		$u_mcp_queue = ($topic_unapproved || $posts_unapproved) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=queue&amp;mode=' . (($topic_unapproved) ? 'approve_details' : 'unapproved_posts') . "&amp;t=$topic_id", true, phpbb::$user->session_id) : '';
		...
		phpbb::$template->assign_block_vars('topicrow', array(
			...
			'ATTACH_ICON_IMG'		=> (phpbb::$auth->acl_get('u_download') && phpbb::$auth->acl_get('f_download', $topic_forum_id) && $row['topic_attachment']) ? phpbb::$user->img('icon_topic_attach', phpbb::$user->lang['TOTAL_ATTACHMENTS']) : '',
			...
			'S_TOPIC_REPORTED'		=> (!empty($row['topic_reported']) && phpbb::$auth->acl_get('m_report', $topic_forum_id)) ? true : false,
			...
			'U_MCP_REPORT'			=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=reports&amp;mode=reports&amp;f=' . $topic_forum_id . '&amp;t=' . $topic_id, true, phpbb::$user->session_id),
