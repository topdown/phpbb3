...
include(phpbb::$phpbb_root_path . 'system/core/bbcode.' . phpbb::$phpEx);
...
if ($view && !$post_id)
{
	...
	if ($view == 'unread')
	{
		...
		$sql = 'SELECT post_id, topic_id, forum_id
			FROM ' . POSTS_TABLE . "
			WHERE topic_id = $topic_id
				" . ((phpbb::$auth->acl_get('m_approve', $forum_id)) ? '' : 'AND post_approved = 1') . "
		...
		else
		{
			$sql = 'SELECT topic_id, forum_id
				...
				WHERE forum_id = ' . $row['forum_id'] . "
					...
					" . ((phpbb::$auth->acl_get('m_approve', $row['forum_id'])) ? '' : 'AND topic_approved = 1') . "
...
if (!$topic_data)
{
	...
	if ($post_id && $topic_id)
	{
		redirect(append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "t=$topic_id" . (($forum_id) ? "&amp;f=$forum_id" : '')));
	}
...
if ($post_id)
{
	...
	if (!$topic_data['post_approved'] && !phpbb::$auth->acl_get('m_approve', $topic_data['forum_id']))
	{
		// If post_id was submitted, we try at least to display the topic as a last resort...
		if ($topic_id)
		{
	...
	if ($post_id == $topic_data['topic_first_post_id'] || $post_id == $topic_data['topic_last_post_id'])
	{
		...
		{
			$topic_data['prev_posts'] = (phpbb::$auth->acl_get('m_approve', $forum_id)) ? $topic_data['topic_replies_real'] : $topic_data['topic_replies'];
		}
		else
		{
	...
	else
	{
		$sql = 'SELECT COUNT(p1.post_id) AS prev_posts
			...
			WHERE p1.topic_id = {$topic_data['topic_id']}
				...
				" . ((!phpbb::$auth->acl_get('m_approve', $forum_id)) ? 'AND p1.post_approved = 1' : '') . '
...
//
$topic_replies = (phpbb::$auth->acl_get('m_approve', $forum_id)) ? $topic_data['topic_replies_real'] : $topic_data['topic_replies'];
...
if (!$topic_data['topic_approved'] && !phpbb::$auth->acl_get('m_approve', $forum_id))
{
	trigger_error('NO_TOPIC');
}

// Start auth check
if (!phpbb::$auth->acl_get('f_read', $forum_id))
{
...
if (isset($_GET['e']))
{
	...
	$redirect_url = append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f=$forum_id&amp;t=$topic_id");
...
$viewtopic_url = append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start") . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '') . (($highlight_match) ? "&amp;hilit=$highlight" : ''));
...
if ((phpbb::$config['email_enable'] || phpbb::$config['jab_enable']) && phpbb::$config['allow_topic_notify'] && phpbb::$user->data['is_registered'])
{
	...
	if (phpbb::$config['allow_forum_notify'] && phpbb::$auth->acl_get('f_subscribe', $forum_id))
	{
...
// Bookmarks
if (phpbb::$config['allow_bookmarks'] && phpbb::$user->data['is_registered'] && request_var('bookmark', 0))
{
...
{
	$extensions = phpbb::$cache->obtain_attach_extensions($forum_id);
}
...
// Quick mod tools
$allow_change_type = (phpbb::$auth->acl_get('m_', $forum_id) || (phpbb::$user->data['is_registered'] && phpbb::$user->data['user_id'] == $topic_data['topic_poster'])) ? true : false;

$topic_mod = '';
$topic_mod .= (phpbb::$auth->acl_get('m_lock', $forum_id) || (phpbb::$auth->acl_get('f_user_lock', $forum_id) && phpbb::$user->data['is_registered'] && phpbb::$user->data['user_id'] == $topic_data['topic_poster'] && $topic_data['topic_status'] == ITEM_UNLOCKED)) ? (($topic_data['topic_status'] == ITEM_UNLOCKED) ? '<option value="lock">' . phpbb::$user->lang['LOCK_TOPIC'] . '</option>' : '<option value="unlock">' . phpbb::$user->lang['UNLOCK_TOPIC'] . '</option>') : '';
$topic_mod .= (phpbb::$auth->acl_get('m_delete', $forum_id)) ? '<option value="delete_topic">' . phpbb::$user->lang['DELETE_TOPIC'] . '</option>' : '';
$topic_mod .= (phpbb::$auth->acl_get('m_move', $forum_id) && $topic_data['topic_status'] != ITEM_MOVED) ? '<option value="move">' . phpbb::$user->lang['MOVE_TOPIC'] . '</option>' : '';
$topic_mod .= (phpbb::$auth->acl_get('m_split', $forum_id)) ? '<option value="split">' . phpbb::$user->lang['SPLIT_TOPIC'] . '</option>' : '';
$topic_mod .= (phpbb::$auth->acl_get('m_merge', $forum_id)) ? '<option value="merge">' . phpbb::$user->lang['MERGE_POSTS'] . '</option>' : '';
$topic_mod .= (phpbb::$auth->acl_get('m_merge', $forum_id)) ? '<option value="merge_topic">' . phpbb::$user->lang['MERGE_TOPIC'] . '</option>' : '';
$topic_mod .= (phpbb::$auth->acl_get('m_move', $forum_id)) ? '<option value="fork">' . phpbb::$user->lang['FORK_TOPIC'] . '</option>' : '';
$topic_mod .= ($allow_change_type && phpbb::$auth->acl_gets('f_sticky', 'f_announce', $forum_id) && $topic_data['topic_type'] != POST_NORMAL) ? '<option value="make_normal">' . phpbb::$user->lang['MAKE_NORMAL'] . '</option>' : '';
$topic_mod .= ($allow_change_type && phpbb::$auth->acl_get('f_sticky', $forum_id) && $topic_data['topic_type'] != POST_STICKY) ? '<option value="make_sticky">' . phpbb::$user->lang['MAKE_STICKY'] . '</option>' : '';
$topic_mod .= ($allow_change_type && phpbb::$auth->acl_get('f_announce', $forum_id) && $topic_data['topic_type'] != POST_ANNOUNCE) ? '<option value="make_announce">' . phpbb::$user->lang['MAKE_ANNOUNCE'] . '</option>' : '';
$topic_mod .= ($allow_change_type && phpbb::$auth->acl_get('f_announce', $forum_id) && $topic_data['topic_type'] != POST_GLOBAL) ? '<option value="make_global">' . phpbb::$user->lang['MAKE_GLOBAL'] . '</option>' : '';
$topic_mod .= (phpbb::$auth->acl_get('m_', $forum_id)) ? '<option value="topic_logs">' . phpbb::$user->lang['VIEW_TOPIC_LOGS'] . '</option>' : '';

// If we've got a hightlight set pass it on to pagination.
$pagination = generate_pagination(append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f=$forum_id&amp;t=$topic_id" . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '') . (($highlight_match) ? "&amp;hilit=$highlight" : '')), $total_posts, phpbb::$config['posts_per_page'], $start);
...
phpbb::$template->assign_vars(array(
	...
	'U_MCP' 		=> (phpbb::$auth->acl_get('m_', $forum_id)) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=main&amp;mode=topic_view&amp;f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start") . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : ''), true, phpbb::$user->session_id) : '',
	...
	'S_TOPIC_ACTION' 		=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start")),
	'S_TOPIC_MOD' 			=> ($topic_mod != '') ? '<select name="action" id="quick-mod-select">' . $topic_mod . '</select>' : '',
	'S_MOD_ACTION' 			=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start") . "&amp;quickmod=1&amp;redirect=" . urlencode(str_replace('&amp;', '&', $viewtopic_url)), true, phpbb::$user->session_id),

	'S_VIEWTOPIC'			=> true,
	'S_DISPLAY_SEARCHBOX'	=> (phpbb::$auth->acl_get('u_search') && phpbb::$auth->acl_get('f_search', $forum_id) && phpbb::$config['load_search']) ? true : false,
	'S_SEARCHBOX_ACTION'	=> append_sid(phpbb::$phpbb_root_path . "search.$phpEx"),
	'S_SEARCH_LOCAL_HIDDEN_FIELDS'	=> build_hidden_fields($s_search_hidden_fields),

	'S_DISPLAY_POST_INFO'	=> ($topic_data['forum_type'] == FORUM_POST && (phpbb::$auth->acl_get('f_post', $forum_id) || phpbb::$user->data['user_id'] == ANONYMOUS)) ? true : false,
	'S_DISPLAY_REPLY_INFO'	=> ($topic_data['forum_type'] == FORUM_POST && (phpbb::$auth->acl_get('f_reply', $forum_id) || phpbb::$user->data['user_id'] == ANONYMOUS)) ? true : false,
	'S_ENABLE_FEEDS_TOPIC'	=> (phpbb::$config['feed_topic'] && !phpbb_optionget(FORUM_OPTION_FEED_EXCLUDE, $topic_data['forum_options'])) ? true : false,

	'U_TOPIC'				=> "{$server_path}viewtopic.$phpEx?f=$forum_id&amp;t=$topic_id",
	'U_FORUM'				=> $server_path,
	'U_VIEW_TOPIC' 			=> $viewtopic_url,
	'U_VIEW_FORUM' 			=> append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $forum_id),
	'U_VIEW_OLDER_TOPIC'	=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f=$forum_id&amp;t=$topic_id&amp;view=previous"),
	'U_VIEW_NEWER_TOPIC'	=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f=$forum_id&amp;t=$topic_id&amp;view=next"),
	'U_PRINT_TOPIC'			=> (phpbb::$auth->acl_get('f_print', $forum_id)) ? $viewtopic_url . '&amp;view=print' : '',
	'U_EMAIL_TOPIC'			=> (phpbb::$auth->acl_get('f_email', $forum_id) && phpbb::$config['email_enable']) ? append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, "mode=email&amp;t=$topic_id") : '',

	'U_WATCH_TOPIC' 		=> $s_watching_topic['link'],
	'L_WATCH_TOPIC' 		=> $s_watching_topic['title'],
	'S_WATCHING_TOPIC'		=> $s_watching_topic['is_watching'],

	'U_BOOKMARK_TOPIC'		=> (phpbb::$user->data['is_registered'] && phpbb::$config['allow_bookmarks']) ? $viewtopic_url . '&amp;bookmark=1&amp;hash=' . generate_link_hash("topic_$topic_id") : '',
	'L_BOOKMARK_TOPIC'		=> (phpbb::$user->data['is_registered'] && phpbb::$config['allow_bookmarks'] && $topic_data['bookmarked']) ? phpbb::$user->lang['BOOKMARK_TOPIC_REMOVE'] : phpbb::$user->lang['BOOKMARK_TOPIC'],

	'U_POST_NEW_TOPIC' 		=> (phpbb::$auth->acl_get('f_post', $forum_id) || phpbb::$user->data['user_id'] == ANONYMOUS) ? append_sid(phpbb::$phpbb_root_path . "posting." . phpbb::$phpEx, "mode=post&amp;f=$forum_id") : '',
	'U_POST_REPLY_TOPIC' 	=> (phpbb::$auth->acl_get('f_reply', $forum_id) || phpbb::$user->data['user_id'] == ANONYMOUS) ? append_sid(phpbb::$phpbb_root_path . "posting." . phpbb::$phpEx, "mode=reply&amp;f=$forum_id&amp;t=$topic_id") : '',
	'U_BUMP_TOPIC'			=> (bump_topic_allowed($forum_id, $topic_data['topic_bumped'], $topic_data['topic_last_post_time'], $topic_data['topic_poster'], $topic_data['topic_last_poster_id'])) ? append_sid(phpbb::$phpbb_root_path . "posting." . phpbb::$phpEx, "mode=bump&amp;f=$forum_id&amp;t=$topic_id&amp;hash=" . generate_link_hash("topic_$topic_id")) : '')
);
...
if (!empty($topic_data['poll_start']))
{
	...
	else
	{
		...
		if (isset($_COOKIE[phpbb::$config['cookie_name'] . '_poll_' . $topic_id]))
		{
			$cur_voted_id = explode(',', $_COOKIE[phpbb::$config['cookie_name'] . '_poll_' . $topic_id]);
			$cur_voted_id = array_map('intval', $cur_voted_id);
		}
	}

	// Can not vote at all if no vote permission
	$s_can_vote = (phpbb::$auth->acl_get('f_vote', $forum_id) &&
		(($topic_data['poll_length'] != 0 && $topic_data['poll_start'] + $topic_data['poll_length'] > time()) || $topic_data['poll_length'] == 0) &&
		$topic_data['topic_status'] != ITEM_LOCKED &&
		$topic_data['forum_status'] != ITEM_LOCKED &&
		(!sizeof($cur_voted_id) ||
		(phpbb::$auth->acl_get('f_votechg', $forum_id) && $topic_data['poll_vote_change']))) ? true : false;
	...
	if ($update && $s_can_vote)
	{
		...
		{
			$redirect_url = append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start"));
		...
		if (phpbb::$user->data['user_id'] == ANONYMOUS && !phpbb::$user->data['is_bot'])
		{
...
$sql = 'SELECT p.post_id
	...
	WHERE p.topic_id = $topic_id
		" . ((!phpbb::$auth->acl_get('m_approve', $forum_id)) ? 'AND p.post_approved = 1' : '') . "
	...
	if (!isset($user_cache[$poster_id]))
	{
		...
		else
		{
			...
			$user_cache[$poster_id] = array(
				...
				'online'		=> false,
				'profile'		=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, "mode=viewprofile&amp;u=$poster_id"),
				'www'			=> $row['user_website'],
				'aim'			=> ($row['user_aim'] && phpbb::$auth->acl_get('u_sendim')) ? append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, "mode=contact&amp;action=aim&amp;u=$poster_id") : '',
				'msn'			=> ($row['user_msnm'] && phpbb::$auth->acl_get('u_sendim')) ? append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, "mode=contact&amp;action=msnm&amp;u=$poster_id") : '',
				'yim'			=> ($row['user_yim']) ? 'http://edit.yahoo.com/config/send_webmesg?.target=' . urlencode($row['user_yim']) . '&amp;.src=pg' : '',
				'jabber'		=> ($row['user_jabber'] && phpbb::$auth->acl_get('u_sendim')) ? append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, "mode=contact&amp;action=jabber&amp;u=$poster_id") : '',
				'search'		=> (phpbb::$auth->acl_get('u_search')) ? append_sid(phpbb::$phpbb_root_path . "search." . phpbb::$phpEx, "author_id=$poster_id&amp;sr=posts") : '',
			...
			if ((!empty($row['user_allow_viewemail']) && phpbb::$auth->acl_get('u_sendemail')) || phpbb::$auth->acl_get('a_email'))
			{
				$user_cache[$poster_id]['email'] = (phpbb::$config['board_email_form'] && phpbb::$config['email_enable']) ? append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, "mode=email&amp;u=$poster_id") : ((phpbb::$config['board_hide_emails'] && !phpbb::$auth->acl_get('a_email')) ? '' : 'mailto:' . $row['user_email']);
			}
			else
			{
...
if (phpbb::$config['load_onlinetrack'] && sizeof($id_cache))
{
	...
	while ($row = phpbb::$db->sql_fetchrow($result))
	{
		$user_cache[$row['session_user_id']]['online'] = (time() - $update_time < $row['online_time'] && (($row['viewonline']) || phpbb::$auth->acl_get('u_viewonline'))) ? true : false;
	}
	phpbb::$db->sql_freeresult($result);
}
unset($id_cache);
...
{
	if (phpbb::$auth->acl_get('u_download') && phpbb::$auth->acl_get('f_download', $forum_id))
	{
		$sql = 'SELECT *
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	if ($topic_data['topic_bumped'] && $row['post_id'] == $topic_data['topic_last_post_id'] && isset($user_cache[$topic_data['topic_bumper']]) )
	{
		...
		$l_bumped_by = sprintf(phpbb::$user->lang['BUMPED_BY'], $user_cache[$topic_data['topic_bumper']]['username'], phpbb::$user->format_date($topic_data['topic_last_post_time'], false, true));
	}
	else
	{
		$l_bumped_by = '';
	...
	$edit_allowed = (phpbb::$user->data['is_registered'] && (phpbb::$auth->acl_get('m_edit', $forum_id) || (
		phpbb::$user->data['user_id'] == $poster_id &&
		phpbb::$auth->acl_get('f_edit', $forum_id) &&
		!$row['post_edit_locked'] &&
		($row['post_time'] > time() - (phpbb::$config['edit_time'] * 60) || !phpbb::$config['edit_time'])
	)));

	$delete_allowed = (phpbb::$user->data['is_registered'] && (phpbb::$auth->acl_get('m_delete', $forum_id) || (
		phpbb::$user->data['user_id'] == $poster_id &&
		phpbb::$auth->acl_get('f_delete', $forum_id) &&
		$topic_data['topic_last_post_id'] == $row['post_id'] &&
		($row['post_time'] > time() - (phpbb::$config['delete_time'] * 60) || !phpbb::$config['delete_time']) &&
	...
	$postrow = array(
		...
		'MINI_POST_IMG'			=> ($post_unread) ? phpbb::$user->img('icon_post_target_unread', 'UNREAD_POST') : phpbb::$user->img('icon_post_target', 'POST'),
		...
		'U_EDIT'			=> ($edit_allowed) ? append_sid(phpbb::$phpbb_root_path . "posting." . phpbb::$phpEx, "mode=edit&amp;f=$forum_id&amp;p={$row['post_id']}") : '',
		'U_QUOTE'			=> (phpbb::$auth->acl_get('f_reply', $forum_id)) ? append_sid(phpbb::$phpbb_root_path . "posting." . phpbb::$phpEx, "mode=quote&amp;f=$forum_id&amp;p={$row['post_id']}") : '',
		'U_INFO'			=> (phpbb::$auth->acl_get('m_info', $forum_id)) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=main&amp;mode=post_details&amp;f=$forum_id&amp;p=" . $row['post_id'], true, phpbb::$user->session_id) : '',
		'U_DELETE'			=> ($delete_allowed) ? append_sid(phpbb::$phpbb_root_path . "posting." . phpbb::$phpEx, "mode=delete&amp;f=$forum_id&amp;p={$row['post_id']}") : '',

		'U_PROFILE'		=> $user_cache[$poster_id]['profile'],
		'U_SEARCH'		=> $user_cache[$poster_id]['search'],
		'U_PM'			=> ($poster_id != ANONYMOUS && phpbb::$config['allow_privmsg'] && phpbb::$auth->acl_get('u_sendpm') && ($user_cache[$poster_id]['allow_pm'] || phpbb::$auth->acl_gets('a_', 'm_') || phpbb::$auth->acl_getf_global('m_'))) ? append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm&amp;mode=compose&amp;action=quotepost&amp;p=' . $row['post_id']) : '',
		...
		'U_REPORT'			=> (phpbb::$auth->acl_get('f_report', $forum_id)) ? append_sid(phpbb::$phpbb_root_path . "report." . phpbb::$phpEx, 'f=' . $forum_id . '&amp;p=' . $row['post_id']) : '',
		'U_MCP_REPORT'		=> (phpbb::$auth->acl_get('m_report', $forum_id)) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=reports&amp;mode=report_details&amp;f=' . $forum_id . '&amp;p=' . $row['post_id'], true, phpbb::$user->session_id) : '',
		'U_MCP_APPROVE'		=> (phpbb::$auth->acl_get('m_approve', $forum_id)) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=queue&amp;mode=approve_details&amp;f=' . $forum_id . '&amp;p=' . $row['post_id'], true, phpbb::$user->session_id) : '',
		'U_MINI_POST'		=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'p=' . $row['post_id']) . (($topic_data['topic_type'] == POST_GLOBAL) ? '&amp;f=' . $forum_id : '') . '#p' . $row['post_id'],
		'U_NEXT_POST_ID'	=> ($i < $i_total && isset($rowset[$post_list[$i + 1]])) ? $rowset[$post_list[$i + 1]]['post_id'] : '',
		'U_PREV_POST_ID'	=> $prev_post_id,
		'U_NOTES'			=> (phpbb::$auth->acl_getf_global('m_')) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=notes&amp;mode=user_notes&amp;u=' . $poster_id, true, phpbb::$user->session_id) : '',
		'U_WARN'			=> (phpbb::$auth->acl_get('m_warn') && $poster_id != phpbb::$user->data['user_id'] && $poster_id != ANONYMOUS) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=warn&amp;mode=warn_post&amp;f=' . $forum_id . '&amp;p=' . $row['post_id'], true, phpbb::$user->session_id) : '',
		...
		'S_POST_REPORTED'	=> ($row['post_reported'] && phpbb::$auth->acl_get('m_report', $forum_id)) ? true : false,
...
if (isset(phpbb::$user->data['session_page']) && !phpbb::$user->data['is_bot'] && (strpos(phpbb::$user->data['session_page'], '&t=' . $topic_id) === false || isset(phpbb::$user->data['session_created'])))
{
	$sql = 'UPDATE ' . TOPICS_TABLE . '
...
if ($topic_data['topic_type'] == POST_GLOBAL)
{
	...
	$result = phpbb::$db->sql_query_limit($sql, 1);
	$topic_data['forum_last_post_time'] = (int) phpbb::$db->sql_fetchfield('forum_last_post_time');
	phpbb::$db->sql_freeresult($result);
	...
	$result = phpbb::$db->sql_query($sql);
	$topic_data['forum_mark_time'] = (int) phpbb::$db->sql_fetchfield('forum_mark_time');
	phpbb::$db->sql_freeresult($result);
}
...
if ($all_marked_read)
{
	...
	{
		phpbb::$template->assign_vars(array(
			'U_VIEW_UNREAD_POST'	=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f=$forum_id&amp;t=$topic_id&amp;view=unread") . '#unread',
		));
	}
}
...
$s_quick_reply = false;
if (phpbb::$user->data['is_registered'] && phpbb::$config['allow_quick_reply'] && ($topic_data['forum_flags'] & FORUM_FLAG_QUICK_REPLY) && phpbb::$auth->acl_get('f_reply', $forum_id))
{
	// Quick reply enabled forum
	$s_quick_reply = (($topic_data['forum_status'] == ITEM_UNLOCKED && $topic_data['topic_status'] == ITEM_UNLOCKED) || phpbb::$auth->acl_get('m_edit', $forum_id)) ? true : false;
}
...
if ($s_can_vote || $s_quick_reply)
{
	...
	if ($s_quick_reply)
	{
		$s_attach_sig	= phpbb::$config['allow_sig'] && phpbb::$user->optionget('attachsig') && phpbb::$auth->acl_get('f_sigs', $forum_id) && phpbb::$auth->acl_get('u_sig');
		$s_smilies		= phpbb::$config['allow_smilies'] && phpbb::$user->optionget('smilies') && phpbb::$auth->acl_get('f_smilies', $forum_id);
		$s_bbcode		= phpbb::$config['allow_bbcode'] && phpbb::$user->optionget('bbcode') && phpbb::$auth->acl_get('f_bbcode', $forum_id);
		$s_notify		= phpbb::$config['allow_topic_notify'] && (phpbb::$user->data['user_notify'] || $s_watching_topic['is_watching']);
		...
		(!phpbb::$config['allow_post_links'])	? $qr_hidden_fields['disable_magic_url'] = 1	: true;
		...
		phpbb::$template->assign_vars(array(
			'S_QUICK_REPLY'			=> true,
			'U_QR_ACTION'			=> append_sid(phpbb::$phpbb_root_path . "posting." . phpbb::$phpEx, "mode=reply&amp;f=$forum_id&amp;t=$topic_id"),
...
// Output the page
page_header(phpbb::$user->lang['VIEW_TOPIC'] . ' - ' . $topic_data['topic_title'], true, $forum_id);

phpbb::$template->set_filenames(array(
	'body' => ($view == 'print') ? 'viewtopic_print.html' : 'viewtopic_body.html')
);
make_jumpbox(append_sid(phpbb::$phpbb_root_path . "viewforum.$phpEx"), $forum_id);
