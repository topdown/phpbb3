...
include_once($phpbb_root_path . 'system/db/dbal.' . $phpEx);
...
class dbal_mysqli extends dbal
{
	...
	function sql_server_info($raw = false, $use_cache = true)
	{
		...
		if (!$use_cache || empty($cache) || ($this->sql_server_version = phpbb::$cache->get('mysqli_version')) === false)
		{
			...
			{
				phpbb::$cache->put('mysqli_version', $this->sql_server_version);
			}
		}
	...
	*/
	public function sql_query($query = '', $cache_ttl = 0)
	{
		if ($query != '')
		{
			...
			$this->query_result = ($cache_ttl && method_exists(phpbb::$cache, 'sql_load')) ? phpbb::$cache->sql_load($query) : false;
			...
			if ($this->query_result === false)
			{
				...
				if ($cache_ttl && method_exists(phpbb::$cache, 'sql_save'))
				{
					phpbb::$cache->sql_save($query, $this->query_result, $cache_ttl);
				}
			}
	...
	*/
	...
	public function sql_fetchrow($query_id = false)
	{
		...
		{
			return phpbb::$cache->sql_fetchrow($query_id);
		}
	...
	public function sql_rowseek($rownum, &$query_id)
	{
		...
		if (!is_object($query_id) && isset(phpbb::$cache->sql_rowset[$query_id]))
		{
			return phpbb::$cache->sql_rowseek($rownum, $query_id);
		}
	...
	* Get last inserted id after insert statement
	...
	public function sql_freeresult($query_id = false)
	{
		...
		if (!is_object($query_id) && isset(phpbb::$cache->sql_rowset[$query_id]))
		{
			return phpbb::$cache->sql_freeresult($query_id);
		}
