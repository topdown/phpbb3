...
include_once(phpbb::$phpbb_root_path . 'system/db/dbal.' . phpbb::$phpEx);

if (!class_exists('phpbb_error_collector'))
{
	include(phpbb::$phpbb_root_path . 'system/includes/error_collector.' . phpbb::$phpEx);
}
...
class dbal_postgres extends dbal
{
	...
	*/
	function sql_server_info($raw = false, $use_cache = true)
	{
		...
		if (!$use_cache || empty($cache) || ($this->sql_server_version = phpbb::$cache->get('pgsql_version')) === false)
		{
			...
			{
				phpbb::$cache->put('pgsql_version', $this->sql_server_version);
			}
		}
	...
	*/
	function sql_query($query = '', $cache_ttl = 0)
	{
		if ($query != '')
		{
			...
			$this->query_result = ($cache_ttl && method_exists($cache, 'sql_load')) ? phpbb::$cache->sql_load($query) : false;
	...
	*/
	function sql_fetchrow($query_id = false)
	{
		...
		if (isset(phpbb::$cache->sql_rowset[$query_id]))
		{
