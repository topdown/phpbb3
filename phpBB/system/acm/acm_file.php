...
class acm
{
	...
	*/
	function acm()
	{
		...
		$this->cache_dir = phpbb::$phpbb_root_path . 'cache/';
	}
	...
	function save()
	{
		...
		if (!$this->_write('data_global'))
		{
			if (!function_exists('phpbb_is_writable'))
			{
				...
				include(phpbb::$phpbb_root_path . 'system/includes/functions.' . phpbb::$phpEx);
			}

			// Now, this occurred how often? ... phew, just tell the user then...
			if (!phpbb_is_writable($this->cache_dir))
			{
				// We need to use die() here, because else we may encounter an infinite loop (the message handler calls phpbb::$cache->unload())
				die('Fatal: ' . $this->cache_dir . ' is NOT writable.');
				exit;
			}

			die('Fatal: Not able to open ' . $this->cache_dir . 'data_global.' . phpbb::$phpEx);
			exit;
		}
	...
	*/
	function tidy()
	{
		...
		if (file_exists($this->cache_dir . 'data_global.' . phpbb::$phpEx))
		{
			if (!sizeof($this->vars))
			{
				$this->load();
	...
	*/
	function destroy($var_name, $table = '')
	{
		...
		if ($var_name[0] == '_')
		{
			$this->remove_file($this->cache_dir . 'data' . $var_name . "." . phpbb::$phpEx, true);
		}
	...
	*/
	function _exists($var_name)
	{
		if ($var_name[0] == '_')
		{
			...
			return file_exists($this->cache_dir . 'data' . $var_name . "." . phpbb::$phpEx);
		}
		else
		{
			if (!sizeof($this->vars))
	...
	*/
	function _read($filename)
	{
		...
		$file = "{$this->cache_dir}$filename." . phpbb::$phpEx;
