...
function mcp_forum_view($id, $mode, $action, $forum_info)
{
	...
	$url = append_sid(phpbb::$phpbb_root_path . "mcp.$phpEx?$url_extra");
	...
	phpbb::$template->assign_vars(array(
		...
		'S_CAN_REPORT'			=> phpbb::$auth->acl_get('m_report', $forum_id),
		'S_CAN_DELETE'			=> phpbb::$auth->acl_get('m_delete', $forum_id),
		'S_CAN_MERGE'			=> phpbb::$auth->acl_get('m_merge', $forum_id),
		'S_CAN_MOVE'			=> phpbb::$auth->acl_get('m_move', $forum_id),
		'S_CAN_FORK'			=> phpbb::$auth->acl_get('m_', $forum_id),
		'S_CAN_LOCK'			=> phpbb::$auth->acl_get('m_lock', $forum_id),
		'S_CAN_SYNC'			=> phpbb::$auth->acl_get('m_', $forum_id),
		'S_CAN_APPROVE'			=> phpbb::$auth->acl_get('m_approve', $forum_id),
		'S_MERGE_SELECT'		=> ($merge_select) ? true : false,
		'S_CAN_MAKE_NORMAL'		=> phpbb::$auth->acl_gets('f_sticky', 'f_announce', $forum_id),
		'S_CAN_MAKE_STICKY'		=> phpbb::$auth->acl_get('f_sticky', $forum_id),
		'S_CAN_MAKE_ANNOUNCE'	=> phpbb::$auth->acl_get('f_announce', $forum_id),

		'U_VIEW_FORUM'			=> append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $forum_id),
		'U_VIEW_FORUM_LOGS'		=> (phpbb::$auth->acl_gets('a_', 'm_', $forum_id) && $module->loaded('logs')) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=logs&amp;mode=forum_logs&amp;f=' . $forum_id) : '',
	...
	foreach ($topic_list as $topic_id)
	{
		...
		$topic_unapproved = (!$row['topic_approved'] && phpbb::$auth->acl_get('m_approve', $row['forum_id'])) ? true : false;
		$posts_unapproved = ($row['topic_approved'] && $row['topic_replies'] < $row['topic_replies_real'] && phpbb::$auth->acl_get('m_approve', $row['forum_id'])) ? true : false;
		$u_mcp_queue = ($topic_unapproved || $posts_unapproved) ? $url . '&amp;i=queue&amp;mode=' . (($topic_unapproved) ? 'approve_details' : 'unapproved_posts') . '&amp;t=' . $row['topic_id'] : '';

		$topic_row = array(
			'ATTACH_ICON_IMG'		=> (phpbb::$auth->acl_get('u_download') && phpbb::$auth->acl_get('f_download', $row['forum_id']) && $row['topic_attachment']) ? phpbb::$user->img('icon_topic_attach', phpbb::$user->lang['TOTAL_ATTACHMENTS']) : '',
			...
			'REPLIES'			=> (phpbb::$auth->acl_get('m_approve', $row['forum_id'])) ? $row['topic_replies_real'] : $row['topic_replies'],
			...
			'S_TOPIC_REPORTED'		=> (!empty($row['topic_reported']) && empty($row['topic_moved_id']) && phpbb::$auth->acl_get('m_report', $row['forum_id'])) ? true : false,
		...
		if ($row['topic_status'] == ITEM_MOVED)
		{
			$topic_row = array_merge($topic_row, array(
				'U_VIEW_TOPIC'		=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "t={$row['topic_moved_id']}"),
				'U_DELETE_TOPIC'	=> (phpbb::$auth->acl_get('m_delete', $forum_id)) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=$id&amp;f=$forum_id&amp;topic_id_list[]={$row['topic_id']}&amp;mode=forum_view&amp;action=delete_topic") : '',
				'S_MOVED_TOPIC'		=> true,
		...
		else
		{
			...
			$topic_row = array_merge($topic_row, array(
				'U_VIEW_TOPIC'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=$id&amp;f=$forum_id&amp;t={$row['topic_id']}&amp;mode=topic_view"),

				'S_SELECT_TOPIC'	=> ($merge_select && !in_array($row['topic_id'], $source_topic_ids)) ? true : false,
				'U_SELECT_TOPIC'	=> $u_select_topic,
				'U_MCP_QUEUE'		=> $u_mcp_queue,
				'U_MCP_REPORT'		=> (phpbb::$auth->acl_get('m_report', $forum_id)) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=main&amp;mode=topic_view&amp;t=' . $row['topic_id'] . '&amp;action=reports') : '',
