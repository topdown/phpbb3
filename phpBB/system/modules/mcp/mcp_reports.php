	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			case 'report_details':
				...
				phpbb::$template->assign_vars(array(
					'S_MCP_REPORT'			=> true,
					'S_CLOSE_ACTION'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=reports&amp;mode=report_details&amp;f=' . $post_info['forum_id'] . '&amp;p=' . $post_id),
					...
					'U_MCP_REPORTER_NOTES'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=notes&amp;mode=user_notes&amp;u=' . $report['user_id']),
					'U_MCP_USER_NOTES'			=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=notes&amp;mode=user_notes&amp;u=' . $post_info['user_id']),
					'U_MCP_WARN_REPORTER'		=> (phpbb::$auth->acl_get('m_warn')) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=warn&amp;mode=warn_user&amp;u=' . $report['user_id']) : '',
					...
					'U_VIEW_FORUM'				=> append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $post_info['forum_id']),
					...
					'RETURN_REPORTS'			=> sprintf(phpbb::$user->lang['RETURN_REPORTS'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=reports' . (($post_info['post_reported']) ? '&amp;mode=reports' : '&amp;mode=reports_closed') . '&amp;start=' . $start . '&amp;f=' . $post_info['forum_id']) . '">', '</a>'),
					...
					'U_LOOKUP_IP'			=> (phpbb::$auth->acl_get('m_info', $post_info['forum_id'])) ? $this->u_action . '&amp;r=' . $report_id . '&amp;p=' . $post_id . '&amp;f=' . $forum_id . '&amp;lookup=' . $post_info['poster_ip'] . '#ip' : '',
				));
				...
				if (sizeof($report_ids))
				{
					...
					while ($row = phpbb::$db->sql_fetchrow($result))
					{
						...
						phpbb::$template->assign_block_vars('postrow', array(
							'U_VIEWFORUM'				=> (!$global_topic) ? append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $row['forum_id']) : '',
							'U_VIEWPOST'				=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $row['forum_id'] . '&amp;p=' . $row['post_id']) . '#p' . $row['post_id'],
							'U_VIEW_DETAILS'			=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=reports&amp;start=$start&amp;mode=report_details&amp;f={$row['forum_id']}&amp;r={$row['report_id']}"),
...
function close_report($report_id_list, $mode, $action, $pm = false)
{
	...
	$sql = "SELECT r.$id_column
		...
		WHERE ' . phpbb::$db->sql_in_set('r.report_id', $report_id_list) . $pm_where;
	$result = phpbb::$db->sql_query($sql);
	...
	if ($pm)
	{
		if (!phpbb::$auth->acl_getf_global('m_report'))
		{
	...
	elseif ($action == 'delete' && strpos(phpbb::$user->data['session_page'], 'mode=pm_report_details') !== false)
	{
			...
			if (sizeof($close_report_posts))
			{
				if ($pm)
				{
					$sql = 'UPDATE ' . PRIVMSGS_TABLE . '
						...
						WHERE ' . phpbb::$db->sql_in_set('msg_id', $close_report_posts);
					phpbb::$db->sql_query($sql);
	...
	{
		confirm_box(false, phpbb::$user->lang[strtoupper($action) . "_{$pm_prefix}REPORT" . ((sizeof($report_id_list) == 1) ? '' : 'S') . '_CONFIRM'], $s_hidden_fields);
	}
	...
	else
	{
		...
		if (!$pm)
		{
			...
			{
				$return_forum = sprintf(phpbb::$user->lang['RETURN_FORUM'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . current($forum_ids)) . '">', '</a>') . '<br /><br />';
			}

			if (sizeof($topic_ids) === 1)
			{
				$return_topic = sprintf(phpbb::$user->lang['RETURN_TOPIC'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 't=' . current($topic_ids) . '&amp;f=' . current($forum_ids)) . '">', '</a>') . '<br /><br />';
			}
		}
