...
class mcp_pm_reports
{
	...
	function main($id, $mode)
	{
		...
		include_once(phpbb::$phpbb_root_path . 'system/includes/functions_privmsgs.' . phpbb::$phpEx);
		...
		switch ($action)
		{
			...
			case 'delete':
				...
				{
					include(phpbb::$phpbb_root_path . '/system/modules/mcp/mcp_reports.' . phpbb::$phpEx);
				}
		...
		switch ($mode)
		{
			case 'pm_report_details':
				...
				phpbb::$user->add_lang(array('posting', 'viewforum', 'viewtopic', 'ucp'));
				...
				if ($pm_info['message_attachment'] && phpbb::$auth->acl_get('u_pm_download'))
				{
					$sql = 'SELECT *
				...
				phpbb::$template->assign_vars(array(
					'S_MCP_REPORT'			=> true,
					'S_PM'					=> true,
					'S_CLOSE_ACTION'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=pm_reports&amp;mode=pm_report_details&amp;r=' . $report_id),
					'S_CAN_VIEWIP'			=> phpbb::$auth->acl_getf_global('m_info'),
					'S_POST_REPORTED'		=> $pm_info['message_reported'],
					'S_USER_NOTES'			=> true,

					'U_MCP_REPORT'				=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=pm_reports&amp;mode=pm_report_details&amp;r=' . $report_id),
					'U_MCP_REPORTER_NOTES'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=notes&amp;mode=user_notes&amp;u=' . $report['user_id']),
					'U_MCP_USER_NOTES'			=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=notes&amp;mode=user_notes&amp;u=' . $pm_info['author_id']),
					...
					'U_MCP_WARN_USER'			=> (phpbb::$auth->acl_get('m_warn')) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=warn&amp;mode=warn_user&amp;u=' . $pm_info['author_id']) : '',
					
					'EDIT_IMG'				=> phpbb::$user->img('icon_post_edit', phpbb::$user->lang['EDIT_POST']),
					'MINI_POST_IMG'			=> phpbb::$user->img('icon_post_target', 'POST'),

					'RETURN_REPORTS'			=> sprintf(phpbb::$user->lang['RETURN_REPORTS'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=pm_reports' . (($pm_info['message_reported']) ? '&amp;mode=pm_reports' : '&amp;mode=pm_reports_closed') . '&amp;start=' . $start) . '">', '</a>'),
					...
					'POST_SUBJECT'			=> ($pm_info['message_subject']) ? $pm_info['message_subject'] : phpbb::$user->lang['NO_SUBJECT'],
					'POST_DATE'				=> phpbb::$user->format_date($pm_info['message_time']),
					'POST_IP'				=> $pm_info['author_ip'],
					'POST_IPADDR'			=> (phpbb::$auth->acl_getf_global('m_info') && request_var('lookup', '')) ? @gethostbyaddr($pm_info['author_ip']) : '',
					'POST_ID'				=> $pm_info['msg_id'],

					'U_LOOKUP_IP'			=> (phpbb::$auth->acl_getf_global('m_info')) ? $this->u_action . '&amp;r=' . $report_id . '&amp;pm=' . $pm_id . '&amp;lookup=' . $pm_info['author_ip'] . '#ip' : '',
				));
			...
			case 'pm_reports_closed':
					...
					if (sizeof($pm_list))
					{
						...
						foreach ($pm_list as $message_id)
						{
							...
							phpbb::$template->assign_block_vars('postrow', array(
								'U_VIEW_DETAILS'			=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=pm_reports&amp;mode=pm_report_details&amp;r={$row['report_id']}"),
								...
								'PM_SUBJECT'			=> ($row['message_subject']) ? $row['message_subject'] : phpbb::$user->lang['NO_SUBJECT'],
								'PM_TIME'				=> phpbb::$user->format_date($row['message_time']),
				...
				phpbb::$template->assign_vars(array(
					'L_EXPLAIN'				=> ($mode == 'pm_reports') ? phpbb::$user->lang['MCP_PM_REPORTS_OPEN_EXPLAIN'] : phpbb::$user->lang['MCP_PM_REPORTS_CLOSED_EXPLAIN'],
					'L_TITLE'				=> ($mode == 'pm_reports') ? phpbb::$user->lang['MCP_PM_REPORTS_OPEN'] : phpbb::$user->lang['MCP_PM_REPORTS_CLOSED'],
					
					'S_PM'					=> true,
					'S_MCP_ACTION'			=> $this->u_action,
					'S_CLOSED'				=> ($mode == 'pm_reports_closed') ? true : false,

					'PAGINATION'			=> generate_pagination($this->u_action . "&amp;st=$sort_days&amp;sk=$sort_key&amp;sd=$sort_dir", $total, phpbb::$config['topics_per_page'], $start),
