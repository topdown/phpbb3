...
function mcp_topic_view($id, $mode, $action)
{
	...
	// Approve posts?
	if ($action == 'approve' && phpbb::$auth->acl_get('m_approve', $topic_info['forum_id']))
	{
		include(phpbb::$phpbb_root_path . '/system/modules/mcp/mcp_queue.' . phpbb::$phpEx);
	...
	if ($total == -1)
	{
		if (phpbb::$auth->acl_get('m_approve', $topic_info['forum_id']))
		{
	...
	$sql = 'SELECT u.username, u.username_clean, u.user_colour, p.*
		...
		WHERE ' . (($action == 'reports') ? 'p.post_reported = 1 AND ' : '') . '
			p.topic_id = ' . $topic_id . ' ' .
			((!phpbb::$auth->acl_get('m_approve', $topic_info['forum_id'])) ? ' AND p.post_approved = 1 ' : '') . '
	...
	{
		$extensions = phpbb::$cache->obtain_attach_extensions($topic_info['forum_id']);

		// Get attachments...
		if (phpbb::$auth->acl_get('u_download') && phpbb::$auth->acl_get('f_download', $topic_info['forum_id']))
		{
			$sql = 'SELECT *
	...
	foreach ($rowset as $i => $row)
	{
		...
		phpbb::$template->assign_block_vars('postrow', array(
			...
			'MESSAGE'		=> $message,
			'POST_ID'		=> $row['post_id'],
			'RETURN_TOPIC'	=> sprintf(phpbb::$user->lang['RETURN_TOPIC'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 't=' . $topic_id) . '">', '</a>'),
			...
			'U_MCP_APPROVE'		=> (phpbb::$auth->acl_get('m_approve', $topic_info['forum_id'])) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=queue&amp;mode=approve_details&amp;f=' . $topic_info['forum_id'] . '&amp;p=' . $row['post_id']) : '',
			'U_MCP_REPORT'		=> (phpbb::$auth->acl_get('m_report', $topic_info['forum_id'])) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=reports&amp;mode=report_details&amp;f=' . $topic_info['forum_id'] . '&amp;p=' . $row['post_id']) : '')
		);
	...
	if (phpbb::$auth->acl_gets('m_split', 'm_merge', (int) $topic_info['forum_id']))
	{
		include_once(phpbb::$phpbb_root_path . 'system/includes/functions_posting.' . phpbb::$phpEx);
		...
		if ($to_topic_id)
		{
			...
			else
			{
				...
				if (!$to_topic_info['enable_icons'] || phpbb::$auth->acl_get('!f_icons', $topic_info['forum_id']))
				{
					$s_topic_icons = false;
				}
			}
	...
	phpbb::$template->assign_vars(array(
		...
		'U_VIEW_TOPIC'		=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $topic_info['forum_id'] . '&amp;t=' . $topic_info['topic_id']),

		'TO_TOPIC_ID'		=> $to_topic_id,
		'TO_TOPIC_INFO'		=> ($to_topic_id) ? sprintf(phpbb::$user->lang['YOU_SELECTED_TOPIC'], $to_topic_id, '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $to_topic_info['forum_id'] . '&amp;t=' . $to_topic_id) . '">' . $to_topic_info['topic_title'] . '</a>') : '',
		...
		'S_CAN_SPLIT'		=> (phpbb::$auth->acl_get('m_split', $topic_info['forum_id'])) ? true : false,
		'S_CAN_MERGE'		=> (phpbb::$auth->acl_get('m_merge', $topic_info['forum_id'])) ? true : false,
		'S_CAN_DELETE'		=> (phpbb::$auth->acl_get('m_delete', $topic_info['forum_id'])) ? true : false,
		'S_CAN_APPROVE'		=> ($has_unapproved_posts && phpbb::$auth->acl_get('m_approve', $topic_info['forum_id'])) ? true : false,
		'S_CAN_LOCK'		=> (phpbb::$auth->acl_get('m_lock', $topic_info['forum_id'])) ? true : false,
		'S_CAN_REPORT'		=> (phpbb::$auth->acl_get('m_report', $topic_info['forum_id'])) ? true : false,
		...
		'RETURN_TOPIC'		=> sprintf(phpbb::$user->lang['RETURN_TOPIC'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f={$topic_info['forum_id']}&amp;t={$topic_info['topic_id']}&amp;start=$start") . '">', '</a>'),
		'RETURN_FORUM'		=> sprintf(phpbb::$user->lang['RETURN_FORUM'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, "f={$topic_info['forum_id']}&amp;start=$start") . '">', '</a>'),

		'PAGE_NUMBER'		=> on_page($total, $posts_per_page, $start),
		'PAGINATION'		=> (!$posts_per_page) ? '' : generate_pagination(append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=$id&amp;t={$topic_info['topic_id']}&amp;mode=$mode&amp;action=$action&amp;to_topic_id=$to_topic_id&amp;posts_per_page=$posts_per_page&amp;st=$sort_days&amp;sk=$sort_key&amp;sd=$sort_dir"), $total, $posts_per_page, $start),
	...
	if (confirm_box(true))
	{
		if ($action == 'split_beyond')
		{
			...
			while ($row = phpbb::$db->sql_fetchrow($result))
			{
				...
				if (!$row['post_approved'] && !phpbb::$auth->acl_get('m_approve', $row['forum_id']))
				{
//					continue;
				}
		...
		$return_link = sprintf(phpbb::$user->lang['RETURN_TOPIC'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $post_info['forum_id'] . '&amp;t=' . $post_info['topic_id']) . '">', '</a>') . '<br /><br />' . sprintf(phpbb::$user->lang['RETURN_NEW_TOPIC'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $to_forum_id . '&amp;t=' . $to_topic_id) . '">', '</a>');
	}
	else
	{
	...
	{
		meta_refresh(3, append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f=$to_forum_id&amp;t=$to_topic_id"));
...
function merge_posts($topic_id, $to_topic_id)
{
	...
	if (confirm_box(true))
	{
		...
		if ($row)
		{
			$return_link .= sprintf(phpbb::$user->lang['RETURN_TOPIC'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $row['forum_id'] . '&amp;t=' . $topic_id) . '">', '</a>');
		}
		else
		{
		...
		$return_link .= (($return_link) ? '<br /><br />' : '') . sprintf(phpbb::$user->lang['RETURN_NEW_TOPIC'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $to_forum_id . '&amp;t=' . $to_topic_id) . '">', '</a>');
	}
	else
	{
