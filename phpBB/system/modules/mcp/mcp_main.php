...
class mcp_main
{
	...
	function main($id, $mode)
	{
		...
		{
			case 'front':
				include(phpbb::$phpbb_root_path . '/system/modules/mcp/mcp_front.' . phpbb::$phpEx);
			...
			case 'forum_view':
				include(phpbb::$phpbb_root_path . '/system/modules/mcp/mcp_forum.' . phpbb::$phpEx);
			...
			case 'topic_view':
				include(phpbb::$phpbb_root_path . '/system/modules/mcp/mcp_topic.' . phpbb::$phpEx);
			...
			case 'post_details':
				include(phpbb::$phpbb_root_path . '/system/modules/mcp/mcp_post.' . phpbb::$phpEx);
...
function mcp_move_topic($topic_ids)
{
	...
	if ($to_forum_id)
	{
		...
		else
		{
			...
			else if (!phpbb::$auth->acl_get('f_post', $to_forum_id) || (!phpbb::$auth->acl_get('m_approve', $to_forum_id) && !phpbb::$auth->acl_get('f_noapprove', $to_forum_id)))
			{
	...
	else
	{
		...
		$message .= '<br /><br />' . sprintf(phpbb::$user->lang['RETURN_NEW_FORUM'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, "f=$to_forum_id") . '">', '</a>');
...
function mcp_delete_topic($topic_ids)
{
	...
	{
		$redirect = append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $forum_id);
...
function mcp_delete_post($post_ids)
{
	...
	if (confirm_box(true))
	{
		...
		{
			$return_link[] = sprintf(phpbb::$user->lang['RETURN_TOPIC'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f=$forum_id&amp;t=$topic_id") . '">', '</a>');
		}
		$return_link[] = sprintf(phpbb::$user->lang['RETURN_FORUM'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $forum_id) . '">', '</a>');
	...
	else
	{
		if ($affected_topics != 1 || $deleted_topics || !$topic_id)
		{
			$redirect = append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "f=$forum_id&i=main&mode=forum_view", false);
		}
...
function mcp_fork_topic($topic_ids)
{
	...
	if ($to_forum_id)
	{
		...
		else
		{
			...
			else if (!phpbb::$auth->acl_get('f_post', $to_forum_id))
			{
				$additional_msg = phpbb::$user->lang['USER_CANNOT_POST'];
			}
		}
	...
	if (confirm_box(true))
	{
		...
		foreach ($topic_data as $topic_id => $topic_row)
		{
			if (!isset($search_type) && $topic_row['enable_indexing'])
			{
				...
				{
					include(phpbb::$phpbb_root_path . "system/search/$search_type.$phpEx");
				}
	...
	else
	{
		...
		{
			$return_link .= '<br /><br />' . sprintf(phpbb::$user->lang['RETURN_NEW_FORUM'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $to_forum_id) . '">', '</a>');
		}
