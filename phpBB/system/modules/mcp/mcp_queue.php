	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			case 'approve_details':
				...
				$post_url = append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $post_info['forum_id'] . '&amp;p=' . $post_info['post_id'] . '#p' . $post_info['post_id']);
				$topic_url = append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $post_info['forum_id'] . '&amp;t=' . $post_info['topic_id']);

				phpbb::$template->assign_vars(array(
					'S_MCP_QUEUE'			=> true,
					'U_APPROVE_ACTION'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=queue&amp;p=$post_id&amp;f=$forum_id"),
					...
					'RETURN_QUEUE'			=> sprintf(phpbb::$user->lang['RETURN_QUEUE'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=queue' . (($topic_id) ? '&amp;mode=unapproved_topics' : '&amp;mode=unapproved_posts')) . "&amp;start=$start\">", '</a>'),
					...
					'U_LOOKUP_IP'			=> (phpbb::$auth->acl_get('m_info', $post_info['forum_id'])) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=queue&amp;mode=approve_details&amp;f=' . $post_info['forum_id'] . '&amp;p=' . $post_id . '&amp;lookup=' . $post_info['poster_ip']) . '#ip' : '',
				));
			...
			case 'unapproved_posts':
				...
				foreach ($rowset as $row)
				{
					...
					phpbb::$template->assign_block_vars('postrow', array(
						'U_TOPIC'			=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $row['forum_id'] . '&amp;t=' . $row['topic_id']),
						'U_VIEWFORUM'		=> (!$global_topic) ? append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $row['forum_id']) : '',
						'U_VIEWPOST'		=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $row['forum_id'] . '&amp;p=' . $row['post_id']) . (($mode == 'unapproved_posts') ? '#p' . $row['post_id'] : ''),
						'U_VIEW_DETAILS'	=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=queue&amp;start=$start&amp;mode=approve_details&amp;f={$row['forum_id']}&amp;p={$row['post_id']}" . (($mode == 'unapproved_topics') ? "&amp;t={$row['topic_id']}" : '')),
...
function approve_post($post_id_list, $id, $mode)
{
	...
	if (confirm_box(true))
	{
		...
		if (sizeof($post_id_list) == 1)
		{
			...
			$post_url = append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f={$post_data['forum_id']}&amp;t={$post_data['topic_id']}&amp;p={$post_data['post_id']}") . '#p' . $post_data['post_id'];
		}
		unset($post_info);
	...
	else
	{
		...
		if (phpbb::$config['email_enable'] || phpbb::$config['jab_enable'])
		{
			...
			foreach ($post_info as $post_id => $post_data)
			{
				...
				if (isset($disapprove_reason_lang))
				{
					...
					if (!isset($lang_reasons[$post_data['user_lang']]))
					{
						...
						if ($post_data['user_lang'] != phpbb::$user->lang_name && file_exists(phpbb::$phpbb_root_path . '/system/language/' . $post_data['user_lang'] . '/mcp.' . phpbb::$phpEx))
						{
							// Load up the language pack
							$lang = array();
							@include(phpbb::$phpbb_root_path . '/system/language/' . basename($post_data['user_lang']) . '/mcp.' . phpbb::$phpEx);
