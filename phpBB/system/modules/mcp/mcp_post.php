...
function mcp_post_details($id, $mode, $action)
{
	...
	$url = append_sid(phpbb::$phpbb_root_path . "mcp.$phpEx?" . extra_url());

	switch ($action)
	{
		case 'whois':

			if (phpbb::$auth->acl_get('m_info', $post_info['forum_id']))
			{
				$ip = request_var('ip', '');
				include(phpbb::$phpbb_root_path . 'system/includes/functions_user.' . phpbb::$phpEx);

				phpbb::$template->assign_vars(array(
					'RETURN_POST'	=> sprintf(phpbb::$user->lang['RETURN_POST'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=$id&amp;mode=$mode&amp;p=$post_id") . '">', '</a>'),
					'U_RETURN_POST'	=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=$id&amp;mode=$mode&amp;p=$post_id"),
		...
		case 'chgposter_ip':
			...
			if (phpbb::$auth->acl_get('m_chgposter', $post_info['forum_id']))
			{
	...
	{
		include_once(phpbb::$phpbb_root_path . 'system/includes/bbcode.' . phpbb::$phpEx);
	...
	if ($post_info['post_attachment'] && phpbb::$auth->acl_get('u_download') && phpbb::$auth->acl_get('f_download', $post_info['forum_id']))
	{
		$extensions = phpbb::$cache->obtain_attach_extensions($post_info['forum_id']);
	...
	phpbb::$template->assign_vars(array(
		...
		'U_APPROVE_ACTION'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=queue&amp;p=$post_id&amp;f={$post_info['forum_id']}"),

		'S_CAN_VIEWIP'			=> phpbb::$auth->acl_get('m_info', $post_info['forum_id']),
		'S_CAN_CHGPOSTER'		=> phpbb::$auth->acl_get('m_chgposter', $post_info['forum_id']),
		'S_CAN_LOCK_POST'		=> phpbb::$auth->acl_get('m_lock', $post_info['forum_id']),
		'S_CAN_DELETE_POST'		=> phpbb::$auth->acl_get('m_delete', $post_info['forum_id']),
		...
		'S_USER_NOTES'			=> true,
		'S_CLEAR_ALLOWED'		=> (phpbb::$auth->acl_get('a_clearlogs')) ? true : false,

		'U_EDIT'				=> (phpbb::$auth->acl_get('m_edit', $post_info['forum_id'])) ? append_sid(phpbb::$phpbb_root_path . "posting." . phpbb::$phpEx, "mode=edit&amp;f={$post_info['forum_id']}&amp;p={$post_info['post_id']}") : '',
		'U_FIND_USERNAME'		=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=searchuser&amp;form=mcp_chgposter&amp;field=username&amp;select_single=true'),
		'U_MCP_APPROVE'			=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=queue&amp;mode=approve_details&amp;f=' . $post_info['forum_id'] . '&amp;p=' . $post_id),
		'U_MCP_REPORT'			=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=reports&amp;mode=report_details&amp;f=' . $post_info['forum_id'] . '&amp;p=' . $post_id),
		'U_MCP_USER_NOTES'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=notes&amp;mode=user_notes&amp;u=' . $post_info['user_id']),
		'U_MCP_WARN_USER'		=> (phpbb::$auth->acl_get('m_warn')) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=warn&amp;mode=warn_user&amp;u=' . $post_info['user_id']) : '',
		'U_VIEW_POST'			=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $post_info['forum_id'] . '&amp;p=' . $post_info['post_id'] . '#p' . $post_info['post_id']),
		'U_VIEW_TOPIC'			=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $post_info['forum_id'] . '&amp;t=' . $post_info['topic_id']),

		'MINI_POST_IMG'			=> ($post_unread) ? phpbb::$user->img('icon_post_target_unread', 'UNREAD_POST') : phpbb::$user->img('icon_post_target', 'POST'),

		'RETURN_TOPIC'			=> sprintf(phpbb::$user->lang['RETURN_TOPIC'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f={$post_info['forum_id']}&amp;p=$post_id") . "#p$post_id\">", '</a>'),
		'RETURN_FORUM'			=> sprintf(phpbb::$user->lang['RETURN_FORUM'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, "f={$post_info['forum_id']}&amp;start={$start}") . '">', '</a>'),
		'REPORTED_IMG'			=> phpbb::$user->img('icon_topic_reported', phpbb::$user->lang['POST_REPORTED']),
		'UNAPPROVED_IMG'		=> phpbb::$user->img('icon_topic_unapproved', phpbb::$user->lang['POST_UNAPPROVED']),
		'EDIT_IMG'				=> phpbb::$user->img('icon_post_edit', phpbb::$user->lang['EDIT_POST']),
		...
		'POST_IPADDR'			=> (phpbb::$auth->acl_get('m_info', $post_info['forum_id']) && request_var('lookup', '')) ? @gethostbyaddr($post_info['poster_ip']) : '',
		'POST_ID'				=> $post_info['post_id'],

		'U_LOOKUP_IP'			=> (phpbb::$auth->acl_get('m_info', $post_info['forum_id'])) ? "$url&amp;i=$id&amp;mode=$mode&amp;lookup={$post_info['poster_ip']}#ip" : '',
		'U_WHOIS'				=> (phpbb::$auth->acl_get('m_info', $post_info['forum_id'])) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=$id&amp;mode=$mode&amp;action=whois&amp;p=$post_id&amp;ip={$post_info['poster_ip']}") : '',
	));
	...
	// Get Reports
	if (phpbb::$auth->acl_get('m_', $post_info['forum_id']))
	{
		...
		if ($row = phpbb::$db->sql_fetchrow($result))
		{
			...
			do
			{
				...
				phpbb::$template->assign_block_vars('reports', array(
					...
					'U_REPORTER'	=> ($row['user_id'] != ANONYMOUS) ? append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=viewprofile&amp;u=' . $row['user_id']) : '',
	...
	if (phpbb::$auth->acl_get('m_info', $post_info['forum_id']))
	{
		...
		if (sizeof($users_ary))
		{
			...
			foreach ($users_ary as $user_id => $user_row)
			{
				phpbb::$template->assign_block_vars('userrow', array(
					...
					'U_PROFILE'		=> ($user_id == ANONYMOUS) ? '' : append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=viewprofile&amp;u=' . $user_id),
					'U_SEARCHPOSTS' => append_sid(phpbb::$phpbb_root_path . "search." . phpbb::$phpEx, 'author_id=' . $user_id . '&amp;sr=topics'))
				);
			}
		}
		...
		while ($row = phpbb::$db->sql_fetchrow($result))
		{
			...
			phpbb::$template->assign_block_vars('iprow', array(
				...
				'U_WHOIS'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=$id&amp;mode=$mode&amp;action=whois&amp;p=$post_id&amp;ip={$row['poster_ip']}"))
			);
		}
		phpbb::$db->sql_freeresult($result);
...
function change_poster(&$post_info, $userdata)
{
	...
	{
		require(phpbb::$phpbb_root_path . "system/search/$search_type.$phpEx");
