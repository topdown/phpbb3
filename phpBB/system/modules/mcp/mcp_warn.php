	...
	*/
	function mcp_warn_front_view()
	{
		...
		phpbb::$template->assign_vars(array(
			'U_FIND_USERNAME'	=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=searchuser&amp;form=mcp&amp;field=username&amp;select_single=true'),
			'U_POST_ACTION'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=warn&amp;mode=warn_user'),
		));
		...
		foreach ($highest as $row)
		{
			phpbb::$template->assign_block_vars('highest', array(
				'U_NOTES'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=notes&amp;mode=user_notes&amp;u=' . $row['user_id']),

				'USERNAME_FULL'		=> get_username_string('full', $row['user_id'], $row['username'], $row['user_colour']),
				'USERNAME'			=> $row['username'],
				'USERNAME_COLOUR'	=> ($row['user_colour']) ? '#' . $row['user_colour'] : '',
				'U_USER'			=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=viewprofile&amp;u=' . $row['user_id']),
	...
	*/
	function mcp_warn_list_view($action)
	{
		...
		phpbb::$template->assign_vars(array(
			...
			'PAGINATION'		=> generate_pagination(append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=warn&amp;mode=list&amp;st=$st&amp;sk=$sk&amp;sd=$sd"), $user_count, phpbb::$config['topics_per_page'], $start),
	...
	*/
	function mcp_warn_post_view($action)
	{
		...
		if ($warning && $action == 'add_warning')
		{
			...
			$redirect = append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=notes&amp;mode=user_notes&amp;u=$user_id");
		...
		phpbb::$template->assign_vars(array(
			...
			'L_WARNING_POST_DEFAULT'	=> sprintf(phpbb::$user->lang['WARNING_POST_DEFAULT'], generate_board_url() . "/viewtopic.$phpEx?f=$forum_id&amp;p=$post_id#p$post_id"),
...
function add_warning($user_row, $warning, $send_pm = true, $post_id = 0)
{
	...
	if ($send_pm)
	{
		...
		$user_row['user_lang'] = (file_exists(phpbb::$phpbb_root_path . 'language/' . $user_row['user_lang'] . "/mcp.$phpEx")) ? $user_row['user_lang'] : phpbb::$config['default_lang'];
		include(phpbb::$phpbb_root_path . 'language/' . basename($user_row['user_lang']) . "/mcp.$phpEx");
