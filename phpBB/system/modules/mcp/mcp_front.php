...
function mcp_front_view($id, $mode, $action)
{
	...
	if ($module->loaded('queue'))
	{
		...
		if (!empty($forum_list))
		{
			...
			if ($total)
			{
				...
				while ($row = phpbb::$db->sql_fetchrow($result))
				{
					...
					phpbb::$template->assign_block_vars('unapproved', array(
						'U_POST_DETAILS'	=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=queue&amp;mode=approve_details&amp;f=' . $row['forum_id'] . '&amp;p=' . $row['post_id']),
						'U_MCP_FORUM'		=> (!$global_topic) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=main&amp;mode=forum_view&amp;f=' . $row['forum_id']) : '',
						'U_MCP_TOPIC'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=main&amp;mode=topic_view&amp;f=' . $row['forum_id'] . '&amp;t=' . $row['topic_id']),
						'U_FORUM'			=> (!$global_topic) ? append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $row['forum_id']) : '',
			...
			$s_hidden_fields = build_hidden_fields(array(
				'redirect'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=main' . (($forum_id) ? '&amp;f=' . $forum_id : ''))
			));

			phpbb::$template->assign_vars(array(
				'S_HIDDEN_FIELDS'		=> $s_hidden_fields,
				'S_MCP_QUEUE_ACTION'	=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, "i=queue"),
			));
			...
			if ($total)
			{
				...
				while ($row = phpbb::$db->sql_fetchrow($result))
				{
					...
					phpbb::$template->assign_block_vars('report', array(
						'U_POST_DETAILS'	=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'f=' . $row['forum_id'] . '&amp;p=' . $row['post_id'] . "&amp;i=reports&amp;mode=report_details"),
						'U_MCP_FORUM'		=> (!$global_topic) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'f=' . $row['forum_id'] . "&amp;i=$id&amp;mode=forum_view") : '',
						'U_MCP_TOPIC'		=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'f=' . $row['forum_id'] . '&amp;t=' . $row['topic_id'] . "&amp;i=$id&amp;mode=topic_view"),
						'U_FORUM'			=> (!$global_topic) ? append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $row['forum_id']) : '',
						'U_TOPIC'			=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $row['forum_id'] . '&amp;t=' . $row['topic_id']),
	...
	if ($module->loaded('pm_reports') && phpbb::$auth->acl_getf_global('m_report'))
	{
		phpbb::$template->assign_var('S_SHOW_PM_REPORTS', true);
		...
		if ($total)
		{
			...
			foreach ($pm_list as $message_id)
			{
				...
				phpbb::$template->assign_block_vars('pm_report', array(
					'U_PM_DETAILS'	=> append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'r=' . $row['report_id'] . "&amp;i=pm_reports&amp;mode=pm_report_details"),
		...
		{
			phpbb::$template->assign_vars(array(
				'L_PM_REPORTS_TOTAL'	=>	phpbb::$user->lang['PM_REPORTS_ZERO_TOTAL'],
		...
		{
			phpbb::$template->assign_vars(array(
				'L_PM_REPORTS_TOTAL'	=> ($total == 1) ? phpbb::$user->lang['PM_REPORT_TOTAL'] : sprintf(phpbb::$user->lang['PM_REPORTS_TOTAL'], $total),
	...
	phpbb::$template->assign_var('S_MCP_ACTION', append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx));
	make_jumpbox(append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=main&amp;mode=forum_view'), 0, false, 'm_', true);
}
