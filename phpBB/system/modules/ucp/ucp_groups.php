		...
		switch ($mode)
		{
			case 'membership':
				...
				if ($submit || isset($_POST['change_default']))
				{
					...
					switch ($action)
					{
						case 'change_default':
							...
							if (!phpbb::$auth->acl_get('u_chggrp'))
							{
						...
						case 'resign':
							...
							if ($group_id == phpbb::$user->data['group_id'] && !phpbb::$auth->acl_get('u_chggrp'))
							{
				...
				while ($row = phpbb::$db->sql_fetchrow($result))
				{
					...
					phpbb::$template->assign_block_vars($block, array(
						...
						'U_VIEW_GROUP'	=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=group&amp;g=' . $row['group_id']),
				...
				$sql_and = (phpbb::$auth->acl_gets('a_group', 'a_groupadd', 'a_groupdel')) ? '<> ' . GROUP_SPECIAL : 'NOT IN (' . GROUP_SPECIAL . ', ' . GROUP_HIDDEN . ')';
				...
				while ($row = phpbb::$db->sql_fetchrow($result))
				{
					...
					phpbb::$template->assign_block_vars('nonmember', array(
						...
						'GROUP_DESC'	=> ($row['group_type'] <> GROUP_SPECIAL) ? generate_text_for_display($row['group_desc'], $row['group_desc_uid'], $row['group_desc_bitfield'], $row['group_desc_options']) : phpbb::$user->lang['GROUP_IS_SPECIAL'],
						'GROUP_SPECIAL'	=> ($row['group_type'] <> GROUP_SPECIAL) ? false : true,
						'GROUP_CLOSED'	=> ($row['group_type'] <> GROUP_CLOSED || phpbb::$auth->acl_gets('a_group', 'a_groupadd', 'a_groupdel')) ? false : true,
						'GROUP_STATUS'	=> phpbb::$user->lang['GROUP_IS_' . $group_status],
				...
				phpbb::$template->assign_vars(array(
					'S_CHANGE_DEFAULT'	=> (phpbb::$auth->acl_get('u_chggrp')) ? true : false,
			...
			case 'manage':
				...
				if ($group_id)
				{
					...
					if (phpbb::$user->data['user_type'] != USER_FOUNDER && $group_row['group_founder_manage'])
					{
					...
					$avatar_img = (!empty($group_row['group_avatar'])) ? get_user_avatar($group_row['group_avatar'], $group_row['group_avatar_type'], $group_row['group_avatar_width'], $group_row['group_avatar_height'], 'GROUP_AVATAR') : '<img src="' . phpbb::$phpbb_root_path . 'adm/images/no_avatar.gif" alt="" />';
				...
				switch ($action)
				{
					case 'edit':
						...
						if ($update)
						{
							...
							else if ($avatar_select && phpbb::$config['allow_avatar_local'])
							{
								...
								if (is_dir(phpbb::$phpbb_root_path . phpbb::$config['avatar_gallery_path'] . '/' . $category))
								{
									...
									list($submit_ary['avatar_width'], $submit_ary['avatar_height']) = getimagesize(phpbb::$phpbb_root_path . phpbb::$config['avatar_gallery_path'] . '/' . $category . '/' . $avatar_select);
							...
							if (!sizeof($error))
							{
								...
								{
									phpbb::$cache->destroy('sql', GROUPS_TABLE);
						...
						if (phpbb::$config['allow_avatar'] && phpbb::$config['allow_avatar_local'] && $display_gallery)
						{
							avatar_gallery($category, $avatar_select, 4);
						}

						$avatars_enabled = (phpbb::$config['allow_avatar'] && (($can_upload && (phpbb::$config['allow_avatar_upload'] || phpbb::$config['allow_avatar_remote_upload'])) || (phpbb::$config['allow_avatar_local'] || phpbb::$config['allow_avatar_remote']))) ? true : false;

						phpbb::$template->assign_vars(array(
							'S_EDIT'			=> true,
							'S_INCLUDE_SWATCH'	=> true,
							'S_FORM_ENCTYPE'	=> (phpbb::$config['allow_avatar'] && $can_upload && (phpbb::$config['allow_avatar_upload'] || phpbb::$config['allow_avatar_remote_upload'])) ? ' enctype="multipart/form-data"' : '',
							...
							'S_UPLOAD_AVATAR_FILE'	=> (phpbb::$config['allow_avatar'] && phpbb::$config['allow_avatar_upload'] && $can_upload) ? true : false,
							'S_UPLOAD_AVATAR_URL'	=> (phpbb::$config['allow_avatar'] && phpbb::$config['allow_avatar_remote_upload'] && $can_upload) ? true : false,
							'S_LINK_AVATAR'			=> (phpbb::$config['allow_avatar'] && phpbb::$config['allow_avatar_remote']) ? true : false,
							...
							'U_SWATCH'			=> append_sid(phpbb::$phpbb_root_path . "adm/swatch." . phpbb::$phpEx, 'form=ucp&amp;name=group_colour'),
					...
					case 'list':
						...
						phpbb::$template->assign_vars(array(
							...
							'U_FIND_USERNAME'	=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=searchuser&amp;form=ucp&amp;field=usernames'),
						));
