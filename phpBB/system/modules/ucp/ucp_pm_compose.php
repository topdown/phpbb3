...
function compose_pm($id, $mode, $action, $user_folders = array())
{
	...
	$select_single = (phpbb::$config['allow_mass_pm'] && phpbb::$auth->acl_get('u_masspm')) ? false : true;
	...
	if ($cancel || ($current_time - $lastclick < 2 && $submit))
	{
		if ($msg_id)
		{
			redirect(append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm&amp;mode=view&amp;action=view_message&amp;p=' . $msg_id));
		}
		redirect(append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm'));
	}
	...
	if ($action != 'edit')
	{
		...
		if (phpbb::$config['allow_mass_pm'] && phpbb::$auth->acl_get('u_masspm_group'))
		{
			...
			$sql .= (phpbb::$auth->acl_gets('a_group', 'a_groupadd', 'a_groupdel')) ? ' WHERE ' : ' AND ';
		...
		phpbb::$template->assign_vars(array(
			'S_SHOW_PM_BOX'		=> true,
			'S_ALLOW_MASS_PM'	=> (phpbb::$config['allow_mass_pm'] && phpbb::$auth->acl_get('u_masspm')) ? true : false,
			'S_GROUP_OPTIONS'	=> (phpbb::$config['allow_mass_pm'] && phpbb::$auth->acl_get('u_masspm_group')) ? $group_options : '',
			'U_FIND_USERNAME'	=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, "mode=searchuser&amp;form=postform&amp;field=username_list&amp;select_single=$select_single"),
		));
	}
	...
	{
		case 'post':
			if (!phpbb::$auth->acl_get('u_sendpm'))
			{
		...
		case 'delete':
			if (!phpbb::$auth->acl_get('u_pm_delete'))
			{
	...
	if ($action == 'forward' && (!phpbb::$config['forward_pm'] || !phpbb::$auth->acl_get('u_pm_forward')))
	{
		trigger_error('NO_AUTH_FORWARD_MESSAGE');
	}

	if ($action == 'edit' && !phpbb::$auth->acl_get('u_pm_edit'))
	{
	...
	if ($sql)
	{
		...
		if ($action == 'quotepost')
		{
			if (($post['forum_id'] && !phpbb::$auth->acl_get('f_read', $post['forum_id'])) || (!$post['forum_id'] && !phpbb::$auth->acl_getf_global('f_read')))
			{
	...
	if (($to_group_id || isset($address_list['g'])) && (!phpbb::$config['allow_mass_pm'] || !phpbb::$auth->acl_get('u_masspm_group')))
	{
	...
	$s_action = append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, "i=$id&amp;mode=$mode&amp;action=$action", true, phpbb::$user->session_id);
	...
	if ($action == 'delete')
	{
		...
		else
		{
			...
			// phpbb::$phpbb_root_path . "ucp.$phpEx?i=pm&amp;mode=compose"
	...
	if ((!phpbb::$config['allow_mass_pm'] || !phpbb::$auth->acl_get('u_masspm_group')) && !empty($address_list['g']))
	{
		$address_list = array();
	...
	if ((!phpbb::$config['allow_mass_pm'] || !phpbb::$auth->acl_get('u_masspm')) && num_recipients($address_list) > 1)
	{
	...
	{
		$enable_sig		= (phpbb::$config['allow_sig'] && phpbb::$config['allow_sig_pm'] && phpbb::$auth->acl_get('u_sig') && phpbb::$user->optionget('attachsig'));
		$enable_smilies	= (phpbb::$config['allow_smilies'] && phpbb::$auth->acl_get('u_pm_smilies') && phpbb::$user->optionget('smilies'));
		$enable_bbcode	= (phpbb::$config['allow_bbcode'] && phpbb::$auth->acl_get('u_pm_bbcode') && phpbb::$user->optionget('bbcode'));
		$enable_urls	= true;
	}
	...
	if (phpbb::$auth->acl_get('u_savedrafts') && $action != 'delete')
	{
		$sql = 'SELECT draft_id
			FROM ' . DRAFTS_TABLE . '
			WHERE forum_id = 0
	...
	$smilies_status	= (phpbb::$config['allow_smilies'] && phpbb::$config['auth_smilies_pm'] && phpbb::$auth->acl_get('u_pm_smilies')) ? true : false;
	$img_status		= (phpbb::$config['auth_img_pm'] && phpbb::$auth->acl_get('u_pm_img')) ? true : false;
	$flash_status	= (phpbb::$config['auth_flash_pm'] && phpbb::$auth->acl_get('u_pm_flash')) ? true : false;
	$url_status		= (phpbb::$config['allow_post_links']) ? true : false;

	// Save Draft
	if ($save && phpbb::$auth->acl_get('u_savedrafts'))
	{
		...
		if ($subject && $message)
		{
			if (confirm_box(true))
			{
				...
				$redirect_url = append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, "i=pm&amp;mode=$mode");
	...
	// Load Draft
	if ($draft_id && phpbb::$auth->acl_get('u_savedrafts'))
	{
	...
	if ($submit || $preview || $refresh)
	{
		...
		$enable_sig			= (!phpbb::$config['allow_sig'] ||!phpbb::$config['allow_sig_pm']) ? false : ((isset($_POST['attach_sig'])) ? true : false);
		...
		if ($action != 'edit' && !$preview && !$refresh && phpbb::$config['flood_interval'] && !phpbb::$auth->acl_get('u_ignoreflood'))
		{
			// Flood check
		...
		if (!sizeof($error) && $submit)
		{
			...
			$return_message_url = append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm&amp;mode=view&amp;p=' . $msg_id);
			$inbox_folder_url = append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm&amp;folder=inbox');
			$outbox_folder_url = append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm&amp;folder=outbox');

			$folder_url = '';
			if (($folder_id > 0) && isset($user_folders[$folder_id]))
			{
				$folder_url = append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm&amp;folder=' . $folder_id);
			}
			...
			$message = phpbb::$user->lang['MESSAGE_STORED'] . '<br /><br />' . sprintf(phpbb::$user->lang['VIEW_PRIVATE_MESSAGE'], '<a href="' . $return_message_url . '">', '</a>');

			$last_click_type = 'CLICK_RETURN_FOLDER';
			if ($folder_url)
			{
				$message .= '<br /><br />' . sprintf(phpbb::$user->lang['CLICK_RETURN_FOLDER'], '<a href="' . $folder_url . '">', '</a>', $user_folders[$folder_id]['folder_name']);
				$last_click_type = 'CLICK_GOTO_FOLDER';
			}
			$message .= '<br /><br />' . sprintf(phpbb::$user->lang[$last_click_type], '<a href="' . $return_box_url . '">', '</a>', phpbb::$user->lang[$return_box_lang]);
	...
	if (($action == 'quote' || $action == 'quotepost') && !$preview && !$refresh && !$submit)
	{
		if ($action == 'quotepost')
		{
			...
			{
				$message_link = "[url=" . generate_board_url() . "/viewtopic.$phpEx?p={$post_id}#p{$post_id}]{phpbb::$user->lang['SUBJECT']}: {$message_subject}[/url]\n\n";
			}
			else
			{
				$message_link = phpbb::$user->lang['SUBJECT'] . ': ' . $message_subject . " (" . generate_board_url() . "/viewtopic.$phpEx?p={$post_id}#p{$post_id})\n\n";
			}
		}
		else
		{
	...
	if ($action == 'forward' && !$preview && !$refresh && !$submit)
	{
		...
		$forward_text[] = sprintf(phpbb::$user->lang['FWD_DATE'], phpbb::$user->format_date($message_time, false, true));
	...
	if (sizeof($address_list))
	{
		...
		foreach ($address_list as $type => $adr_ary)
		{
			foreach ($adr_ary as $id => $field)
			{
				...
				{
					$tpl_ary = array_merge($tpl_ary, array(
						'U_VIEW'		=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=group&amp;g=' . $id),
					));
				}
	...
	$bbcode_checked		= (isset($enable_bbcode)) ? !$enable_bbcode : ((phpbb::$config['allow_bbcode'] && phpbb::$auth->acl_get('u_pm_bbcode')) ? !phpbb::$user->optionget('bbcode') : 1);
	$smilies_checked	= (isset($enable_smilies)) ? !$enable_smilies : ((phpbb::$config['allow_smilies'] && phpbb::$auth->acl_get('u_pm_smilies')) ? !phpbb::$user->optionget('smilies') : 1);
	...
	$form_enctype = (@ini_get('file_uploads') == '0' || strtolower(@ini_get('file_uploads')) == 'off' || !phpbb::$config['allow_pm_attach'] || !phpbb::$auth->acl_get('u_pm_attach')) ? '' : ' enctype="multipart/form-data"';
	...
	phpbb::$template->assign_vars(array(
		...
		'MAX_RECIPIENTS'		=> (phpbb::$config['allow_mass_pm'] && (phpbb::$auth->acl_get('u_masspm') || phpbb::$auth->acl_get('u_masspm_group'))) ? $max_recipients : 0,
		...
		'S_SIG_ALLOWED'			=> (phpbb::$config['allow_sig'] && phpbb::$config['allow_sig_pm'] && phpbb::$auth->acl_get('u_sig')),
		'S_SIGNATURE_CHECKED'	=> ($sig_checked) ? ' checked="checked"' : '',
		'S_LINKS_ALLOWED'		=> $url_status,
		'S_MAGIC_URL_CHECKED'	=> ($urls_checked) ? ' checked="checked"' : '',
		'S_SAVE_ALLOWED'		=> (phpbb::$auth->acl_get('u_savedrafts') && $action != 'edit') ? true : false,
		'S_HAS_DRAFTS'			=> (phpbb::$auth->acl_get('u_savedrafts') && $drafts),
		...
		'U_PROGRESS_BAR'			=> append_sid(phpbb::$phpbb_root_path . "posting." . phpbb::$phpEx, 'f=0&amp;mode=popup'),
		'UA_PROGRESS_BAR'			=> addslashes(append_sid(phpbb::$phpbb_root_path . "posting." . phpbb::$phpEx, 'f=0&amp;mode=popup')),
	));
	...
	$allowed = (phpbb::$auth->acl_get('u_pm_attach') && phpbb::$config['allow_pm_attach'] && $form_enctype);
