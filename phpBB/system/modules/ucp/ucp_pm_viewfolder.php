...
function view_folder($id, $mode, $folder_id, $folder)
{
	...
	if (!$submit_export)
	{
		...
		// Minimise edits
		if (!phpbb::$auth->acl_get('u_pm_delete') && $key = array_search('delete_marked', $mark_options))
		{
		...
		if (sizeof($folder_info['pm_list']))
		{
			...
			foreach ($folder_info['pm_list'] as $message_id)
			{
				...
				// Generate all URIs ...
				$view_message_url = append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, "i=$id&amp;mode=view&amp;f=$folder_id&amp;p=$message_id");
				$remove_message_url = append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, "i=$id&amp;mode=compose&amp;action=delete&amp;p=$message_id");
				...
				phpbb::$template->assign_block_vars('messagerow', array(
					...
					'U_FOLDER'			=> (isset($folder[$row['folder_id']])) ? append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'folder=' . $row['folder_id']) : '',
					...
					'ATTACH_ICON_IMG'	=> (phpbb::$auth->acl_get('u_pm_download') && $row['message_attachment'] && phpbb::$config['allow_pm_attach']) ? phpbb::$user->img('icon_topic_attach', phpbb::$user->lang['TOTAL_ATTACHMENTS']) : '',
					...
					'U_MCP_REPORT'		=> (isset($row['report_id'])) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=pm_reports&amp;mode=pm_report_details&amp;r=' . $row['report_id']) : '',
			...
			phpbb::$template->assign_vars(array(
				...
				'REPORTED_IMG'			=> phpbb::$user->img('icon_topic_reported', 'PM_REPORTED'),
			...
			foreach ($folder_info['pm_list'] as $message_id)
			{
				...
				foreach ($_types as $ug_type)
				{
					if (isset($address_temp[$message_id][$ug_type]) && sizeof($address_temp[$message_id][$ug_type]))
					{
						...
						$sql .= phpbb::$db->sql_in_set(($ug_type == 'u') ? 'user_id' : 'group_id', array_map('intval', array_keys($address_temp[$message_id][$ug_type])));
				    ...
				    !(isset($address[$message_id]['u']) && sizeof($address[$message_id]['u'])))
				{
					...
					$address[$message_id]['u']['to'][] = phpbb::$user->lang['UNDISCLOSED_RECIPIENT'];
				}
				...
				$data[] = array(
					'subject'	=> censor_text($row['message_subject']),
					'sender'	=> $row['username'],
					// ISO 8601 date. For PHP4 we are able to hardcode the timezone because phpbb::$user->format_date() does not set it.
					'date'		=> phpbb::$user->format_date($row['message_time'], (PHP_VERSION >= 5) ? 'c' : "Y-m-d\TH:i:s+00:00", true),
...
function get_pm_from($folder_id, $folder, $user_id)
{
	...
	phpbb::$template->assign_vars(array(
		'PAGINATION'		=> generate_pagination(append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, "i=pm&amp;mode=view&amp;action=view_folder&amp;f=$folder_id&amp;$u_sort_param"), $pm_count, phpbb::$config['topics_per_page'], $start),
		...
		'POST_IMG'		=> (!phpbb::$auth->acl_get('u_sendpm')) ? phpbb::$user->img('button_topic_locked', 'POST_PM_LOCKED') : phpbb::$user->img('button_pm_new', 'POST_NEW_PM'),

		'S_NO_AUTH_SEND_MESSAGE'	=> !phpbb::$auth->acl_get('u_sendpm'),
		...
		'U_POST_NEW_TOPIC'	=> (phpbb::$auth->acl_get('u_sendpm')) ? append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm&amp;mode=compose') : '',
		'S_PM_ACTION'		=> append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, "i=pm&amp;mode=view&amp;action=view_folder&amp;f=$folder_id" . (($start !== 0) ? "&amp;start=$start" : '')),
	));
	...
	if (!empty($pm_reported) && phpbb::$auth->acl_getf_global('m_report'))
	{
		$sql = 'SELECT pm_id, report_id
			FROM ' . REPORTS_TABLE . '
			WHERE report_closed = 0
				AND ' . phpbb::$db->sql_in_set('pm_id', $pm_reported);
		$result = phpbb::$db->sql_query($sql);
