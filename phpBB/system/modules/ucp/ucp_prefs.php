		...
		switch ($mode)
		{
			case 'personal':
				...
				if ($submit)
				{
					...
					if (!sizeof($error))
					{
						...
						$sql_ary = array(
							...
							'user_allow_viewonline'	=> (phpbb::$auth->acl_get('u_hideonline')) ? !$data['hideonline'] : phpbb::$user->data['user_allow_viewonline'],
				...
				phpbb::$template->assign_vars(array(
					...
					'S_CAN_HIDE_ONLINE'		=> (phpbb::$auth->acl_get('u_hideonline')) ? true : false,
			...
			case 'view':
				...
				if ($submit)
				{
					...
					if (!sizeof($error))
					{
						...
						if (phpbb::$auth->acl_get('u_chgcensors'))
						{
				...
				phpbb::$template->assign_vars(array(
					...
					'S_CHANGE_CENSORS'		=> (phpbb::$auth->acl_get('u_chgcensors') && phpbb::$config['allow_nocensors']) ? true : false,
