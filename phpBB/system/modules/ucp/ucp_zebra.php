...
class ucp_zebra
{
	...
	function main($id, $mode)
	{
					...
					if ($data['add'])
					{
						...
						if (sizeof($data['add']))
						{
							...
							while ($row = phpbb::$db->sql_fetchrow($result))
							{
								...
								{
									$error[] = phpbb::$user->lang['NOT_ADDED_' . $l_mode . '_BOTS'];
								}
								else
								{
							...
							if (sizeof($user_id_ary))
							{
								...
								{
									$perms = array();
									foreach (phpbb::$auth->acl_get_list($user_id_ary, array('a_', 'm_')) as $forum_id => $forum_ary)
									{
		...
		phpbb::$template->assign_vars(array(
			...
			'U_FIND_USERNAME'	=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=searchuser&amp;form=ucp&amp;field=add'),
