	...
	function main($id, $mode)
	{
		...
		if ($change_lang || $user_lang != phpbb::$config['default_lang'])
		{
			...
			if (!validate_language_iso_name($use_lang))
			{
				...
				phpbb::$user->lang_name = $user_lang = $use_lang;
				phpbb::$user->lang = array();
				phpbb::$user->data['user_lang'] = phpbb::$user->lang_name;
		...
		if (!$agreed || ($coppa === false && phpbb::$config['coppa_enable']) || ($coppa && !phpbb::$config['coppa_enable']))
		{
			...
			if ($coppa === false && phpbb::$config['coppa_enable'])
			{
				...
				phpbb::$template->assign_vars(array(
					...
					'U_COPPA_NO'		=> append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'mode=register&amp;coppa=0' . $add_lang),
					'U_COPPA_YES'		=> append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'mode=register&amp;coppa=1' . $add_lang),

					'S_SHOW_COPPA'		=> true,
					'S_HIDDEN_FIELDS'	=> build_hidden_fields($s_hidden_fields),
					'S_UCP_ACTION'		=> append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'mode=register' . $add_lang),
				));
			}
			else
			{
				phpbb::$template->assign_vars(array(
					...
					'S_REGISTRATION'	=> true,
					'S_HIDDEN_FIELDS'	=> build_hidden_fields($s_hidden_fields),
					'S_UCP_ACTION'		=> append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'mode=register' . $add_lang . $add_coppa),
					)
				);
			}
			unset($lang_row);
		...
		if ($submit)
		{
			...
			if (!sizeof($error))
			{
				...
				if (phpbb::$config['new_member_post_limit'])
				{
					$user_row['user_new'] = 1;
				}
				...
				if (phpbb::$config['enable_confirm'] && isset($captcha))
				{
					$captcha->reset();
				}
		...
		phpbb::$template->assign_vars(array(
			...
			'S_UCP_ACTION'		=> append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'mode=register'),
		));
