...
class acp_language
{
	...
	function main($id, $mode)
	{
		...
		global $phpbb_admin_path, $table_prefix, $safe_mode, $file_uploads;
		...
		$selected_lang_file = request_var('language_file', '|common.' . phpbb::$phpEx);
		...
		if ($submit && $action == 'upload_data' && request_var('test_connection', ''))
		{
			...
			include_once(phpbb::$phpbb_root_path . 'system/includes/functions_transfer.' . phpbb::$phpEx);
		...
		switch ($action)
		{
			...
			case 'upload_data':
				...
				switch ($this->language_directory)
				{
					case 'email':
						// Get email templates
						$email_files = filelist(phpbb::$phpbb_root_path . 'language/' . $row['lang_iso'], 'email', 'txt');
					...
					case 'acp':
						// Get acp files
						$acp_files = filelist(phpbb::$phpbb_root_path . 'language/' . $row['lang_iso'], 'acp', phpbb::$phpEx);
					...
					case 'mods':
						// Get mod files
						$mods_files = filelist(phpbb::$phpbb_root_path . 'language/' . $row['lang_iso'], 'mods', phpbb::$phpEx);
				...
				if (!$safe_mode)
				{
					...
					{
						$dir = phpbb::$phpbb_root_path . 'store/' . $dir;
				...
				$fp = @fopen(phpbb::$phpbb_root_path . $filename, 'wb');
				...
				if ($action == 'download_file')
				{
					...
					$fp = @fopen(phpbb::$phpbb_root_path . $filename, 'rb');
				...
				else if ($action == 'upload_data')
				{
					...
					if (file_exists(phpbb::$phpbb_root_path . 'store/' . $lang_path . $file))
					{
						@unlink(phpbb::$phpbb_root_path . 'store/' . $lang_path . $file);
					}
			...
			case 'details':
				...
				// Get email templates
				$email_files = filelist(phpbb::$phpbb_root_path . 'language/' . phpbb::$config['default_lang'], 'email', 'txt');
				$email_files = $email_files['email/'];

				// Get acp files
				$acp_files = filelist(phpbb::$phpbb_root_path . 'language/' . phpbb::$config['default_lang'], 'acp', phpbb::$phpEx);
				$acp_files = $acp_files['acp/'];

				// Get mod files
				$mods_files = filelist(phpbb::$phpbb_root_path . 'language/' . phpbb::$config['default_lang'], 'mods', phpbb::$phpEx);
				$mods_files = (isset($mods_files['mods/'])) ? $mods_files['mods/'] : array();
				...
				if (isset($_POST['remove_store']))
				{
					...
					if (file_exists(phpbb::$phpbb_root_path . $store_filename))
					{
						@unlink(phpbb::$phpbb_root_path . $store_filename);
					}
				}
				...
				if ($lang_iso != phpbb::$config['default_lang'])
				{
					...
					{
						if (file_exists(phpbb::$phpbb_root_path . $this->get_filename($lang_iso, '', $file)))
						{
					...
					{
						if (file_exists(phpbb::$phpbb_root_path . $this->get_filename($lang_iso, 'acp', $file)))
						{
					...
					if (sizeof($mods_files))
					{
						...
						{
							if (file_exists(phpbb::$phpbb_root_path . $this->get_filename($lang_iso, 'mods', $file)))
							{
					...
					{
						if (!file_exists(phpbb::$phpbb_root_path . $this->get_filename($lang_iso, 'email', $file)))
						{
				...
				// Main language files
				$s_lang_options = '<option value="|common.' . $phpEx . '" class="sep">' . phpbb::$user->lang['LANGUAGE_FILES'] . '</option>';
				foreach ($this->main_files as $file)
				{
					...
					$prefix = (file_exists(phpbb::$phpbb_root_path . $this->get_filename($lang_iso, '', $file, true, true))) ? '* ' : '';
				...
				// Help Files
				$s_lang_options .= '<option value="|common.' . $phpEx . '" class="sep">' . phpbb::$user->lang['HELP_FILES'] . '</option>';
				foreach ($this->main_files as $file)
				{
					if (strpos($file, 'help_') !== 0)
					{
				...
				foreach ($check_files as $check)
				{
					...
					$s_lang_options .= '<option value="|common.' . $phpEx . '" class="sep">' . phpbb::$user->lang[strtoupper($check) . '_FILES'] . '</option>';

					foreach (${$check . '_files'} as $file)
					{
						$prefix = (file_exists(phpbb::$phpbb_root_path . $this->get_filename($lang_iso, $check, $file, true, true))) ? '* ' : '';
				...
				$file_from_store = (file_exists(phpbb::$phpbb_root_path . $this->get_filename($lang_iso, $this->language_directory, $this->language_file, true, true))) ? true : false;
				$no_store_filename = $this->get_filename($lang_iso, $this->language_directory, $this->language_file);

				if (!$file_from_store && !file_exists(phpbb::$phpbb_root_path . $no_store_filename))
				{
				...
				else
				{
					if ($is_email_file)
					{
						$lang = file_get_contents(phpbb::$phpbb_root_path . $this->get_filename($lang_iso, $this->language_directory, $this->language_file, $file_from_store));
					}
					else
					{
						$help = array();
						include(phpbb::$phpbb_root_path . $this->get_filename($lang_iso, $this->language_directory, $this->language_file, $file_from_store));
			...
			case 'install':
				...
				if (!$lang_iso || !file_exists(phpbb::$phpbb_root_path . "language/$lang_iso/iso.txt"))
				{
					trigger_error(phpbb::$user->lang['LANGUAGE_PACK_NOT_EXIST'] . adm_back_link($this->u_action), E_USER_WARNING);
				}

				$file = file(phpbb::$phpbb_root_path . "language/$lang_iso/iso.txt");
				...
				{
					if (@file_exists(phpbb::$phpbb_root_path . "styles/{$imageset_row['imageset_path']}/imageset/{$lang_pack['iso']}/imageset.cfg"))
					{
						$cfg_data_imageset_data = parse_cfg_file(phpbb::$phpbb_root_path . "styles/{$imageset_row['imageset_path']}/imageset/{$lang_pack['iso']}/imageset.cfg");
			...
			case 'download':
				...
				include_once(phpbb::$phpbb_root_path . 'system/includes/functions_compress.' . phpbb::$phpEx);

				if ($use_method == '.zip')
				{
					$compress = new compress_zip('w', phpbb::$phpbb_root_path . 'store/lang_' . $row['lang_iso'] . $use_method);
				}
				else
				{
					$compress = new compress_tar('w', phpbb::$phpbb_root_path . 'store/lang_' . $row['lang_iso'] . $use_method, $use_method);
				}

				// Get email templates
				$email_templates = filelist(phpbb::$phpbb_root_path . 'language/' . $row['lang_iso'], 'email', 'txt');
				...
				// Get mod files
				$mod_files = filelist(phpbb::$phpbb_root_path . 'language/' . $row['lang_iso'], 'mods', phpbb::$phpEx);
				...
				if (file_exists(phpbb::$phpbb_root_path . 'language/' . $row['lang_iso'] . '/search_ignore_words.' . phpbb::$phpEx))
				{
					$this->add_to_archive($compress, array("search_ignore_words.$phpEx"), $row['lang_iso']);
				}

				if (file_exists(phpbb::$phpbb_root_path . 'language/' . $row['lang_iso'] . '/search_synonyms.' . phpbb::$phpEx))
				{
				...
				@unlink(phpbb::$phpbb_root_path . 'store/lang_' . $row['lang_iso'] . $use_method);
		...
		$new_ary = $iso = array();
		$dp = @opendir(phpbb::$phpbb_root_path . "language");

		if ($dp)
		{
			while (($file = readdir($dp)) !== false)
			{
				if ($file[0] == '.' || !is_dir(phpbb::$phpbb_root_path . 'language/' . $file))
				{
					continue;
				}

				if (file_exists(phpbb::$phpbb_root_path . "language/$file/iso.txt"))
				{
					if (!in_array($file, $installed))
					{
						if ($iso = file(phpbb::$phpbb_root_path . "language/$file/iso.txt"))
						{
							if (sizeof($iso) == 3)
							{
								$new_ary[$file] = array(
...
';
		...
		$this->main_files = array("captcha_qa." . phpbb::$phpEx, "captcha_recaptcha." . phpbb::$phpEx, "common." . phpbb::$phpEx, "groups." . phpbb::$phpEx, "install." . phpbb::$phpEx, "mcp." . phpbb::$phpEx, "memberlist." . phpbb::$phpEx, "posting." . phpbb::$phpEx, "search." . phpbb::$phpEx, "ucp." . phpbb::$phpEx, "viewforum." . phpbb::$phpEx, "viewtopic." . phpbb::$phpEx, "help_bbcode." . phpbb::$phpEx, "help_faq.$phpEx");
	}
	...
	*/
	function get_filename($lang_iso, $directory, $filename, $check_store = false, $only_return_filename = false)
	{
		...
		if ($check_store)
		{
			$check_store_filename = ($safe_mode) ? "store/langfile_{$lang_iso}" . (($directory) ? '_' . $directory : '') . "_{$filename}" : "store/system/language/$lang_iso/" . (($directory) ? $directory . '/' : '') . $filename;

			if (!$only_return_filename && file_exists(phpbb::$phpbb_root_path . $check_store_filename))
			{
	...
	*/
	function add_to_archive(&$compress, $filelist, $lang_iso, $directory = '')
	{
		...
		foreach ($filelist as $file)
		{
			...
			// Add file to archive
			$compress->add_custom_file(phpbb::$phpbb_root_path . $source, $destination);
		}
	}
	...
	*/
	function compare_language_files($source_lang, $dest_lang, $directory, $file)
	{
		...
		$lang = array();
		include(phpbb::$phpbb_root_path . "language/{$source_lang}/" . (($directory) ? $directory . '/' : '') . $file);
		$lang_entry_src = $lang;

		$lang = array();

		if (!file_exists(phpbb::$phpbb_root_path . $this->get_filename($dest_lang, $directory, $file, true)))
		{
			return array();
		}

		include(phpbb::$phpbb_root_path . $this->get_filename($dest_lang, $directory, $file, true));
