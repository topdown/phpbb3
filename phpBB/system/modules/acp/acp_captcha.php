...
class acp_captcha
{
	...
	function main($id, $mode)
	{
		...
		$selected = (isset($captchas['available'][$selected]) || isset($captchas['unavailable'][$selected])) ? $selected : phpbb::$config['captcha_plugin'];
		...
		else
		{
			...
			if ($submit && check_form_key($form_key))
			{
				...
				if ($selected !== phpbb::$config['captcha_plugin'])
				{
					...
					{
						trigger_error(phpbb::$user->lang['CAPTCHA_UNAVAILABLE'] . adm_back_link($this->u_action), E_USER_WARNING);
					}
				}
			...
			{
				trigger_error(phpbb::$user->lang['FORM_INVALID'] . adm_back_link(), E_USER_WARNING);
			}
			else
			{
				$captcha_select = '';
				...
				foreach ($config_vars as $config_var => $options)
				{
					phpbb::$template->assign_var($options['tpl'], (isset($_POST[$config_var])) ? request_var($config_var, $options['default']) : phpbb::$config[$config_var]) ;
				}
