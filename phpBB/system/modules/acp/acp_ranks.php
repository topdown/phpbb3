	...
	function main($id, $mode)
	{
		...
		switch ($action)
		{
			...
			case 'add':
				...
				$imglist = filelist(phpbb::$phpbb_root_path . phpbb::$config['ranks_path'], '');
				...
				foreach ($imglist as $path => $img_ary)
				{
					...
					foreach ($img_ary as $img)
					{
						...
						$filename_list .= '<option value="' . htmlspecialchars($img) . '"' . $selected . '>' . $img . ((in_array($img, $existing_imgs)) ? ' ' . phpbb::$user->lang['RANK_IMAGE_IN_USE'] : '') . '</option>';
					}
				}
				...
				phpbb::$template->assign_vars(array(
					...
					'RANKS_PATH'		=> phpbb::$phpbb_root_path . phpbb::$config['ranks_path'],
					'U_ACTION'			=> $this->u_action . '&amp;id=' . $rank_id,

					'RANK_TITLE'		=> (isset($ranks['rank_title'])) ? $ranks['rank_title'] : '',
					'S_FILENAME_LIST'	=> $filename_list,
					'RANK_IMAGE'		=> ($edit_img) ? phpbb::$phpbb_root_path . phpbb::$config['ranks_path'] . '/' . $edit_img : $phpbb_admin_path . 'images/spacer.gif',
		...
		while ($row = phpbb::$db->sql_fetchrow($result))
		{
			phpbb::$template->assign_block_vars('ranks', array(
				...
				'RANK_IMAGE'		=> phpbb::$phpbb_root_path . phpbb::$config['ranks_path'] . '/' . $row['rank_image'],
