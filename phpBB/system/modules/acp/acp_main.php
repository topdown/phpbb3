...
class acp_main
{
	...
	function main($id, $mode)
	{
		...
		if (phpbb::$user->data['user_perm_from'] && phpbb::$auth->acl_get('a_switchperm'))
		{
			...
			$perm_from .= ($user_row['user_id'] != ANONYMOUS) ? '<a href="' . append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=viewprofile&amp;u=' . $user_row['user_id']) . '">' : '';
			...
			phpbb::$template->assign_vars(array(
				...
				'U_RESTORE_PERMISSIONS'		=> append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'mode=restore_perm'),
				'PERM_FROM'					=> $perm_from,
				'L_PERMISSIONS_TRANSFERRED_EXPLAIN'	=> sprintf(phpbb::$user->lang['PERMISSIONS_TRANSFERRED_EXPLAIN'], $perm_from, append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'mode=restore_perm')),
			));
		...
		if ($action)
		{
			if ($action === 'admlogout')
			{
				...
				$redirect_url = append_sid(phpbb::$phpbb_root_path . "index.$phpEx");
			...
			else
			{
				switch ($action)
				{
					...
					case 'stats':
						...
						{
							include(phpbb::$phpbb_root_path . "system/includes/functions_user.$phpEx");
						}
					...
					case 'purge_cache':
						if ((int) phpbb::$user->data['user_type'] !== USER_FOUNDER)
						{
							trigger_error(phpbb::$user->lang['NO_AUTH_OPERATION'] . adm_back_link($this->u_action), E_USER_WARNING);
						}
					...
					case 'purge_sessions':
						...
						$reinsert_ary = array(
								...
								'session_page'			=> (string) substr(phpbb::$user->page['page'], 0, 199),
								'session_forum_id'		=> phpbb::$user->page['forum'],
								'session_user_id'		=> (int) phpbb::$user->data['user_id'],
								'session_start'			=> (int) phpbb::$user->data['session_start'],
								'session_last_visit'	=> (int) phpbb::$user->data['session_last_visit'],
								'session_time'			=> (int) phpbb::$user->time_now,
								'session_browser'		=> (string) trim(substr(phpbb::$user->browser, 0, 149)),
								'session_forwarded_for'	=> (string) phpbb::$user->forwarded_for,
								'session_ip'			=> (string) phpbb::$user->ip,
								'session_autologin'		=> (int) phpbb::$user->data['session_autologin'],
								'session_admin'			=> 1,
								'session_viewonline'	=> (int) phpbb::$user->data['session_viewonline'],
						);

						$sql = 'INSERT INTO ' . SESSIONS_TABLE . ' ' . phpbb::$db->sql_build_array('INSERT', $reinsert_ary);
						phpbb::$db->sql_query($sql);
		...
		if (phpbb::$auth->acl_get('a_server') && version_compare(PHP_VERSION, '5.2.0', '<'))
		{
			phpbb::$template->assign_vars(array(
				'S_PHP_VERSION_OLD'	=> true,
				'L_PHP_VERSION_OLD'	=> sprintf(phpbb::$user->lang['PHP_VERSION_OLD'], '<a href="http://www.phpbb.com/community/viewtopic.php?f=14&amp;t=1958605">', '</a>'),
			));
		}
		...
		{
			phpbb::$template->assign_var('S_VERSIONCHECK_FAIL', true);
		}
		else
		{
		...
		if ($avatar_dir = @opendir(phpbb::$phpbb_root_path . phpbb::$config['avatar_path']))
		{
			while (($file = readdir($avatar_dir)) !== false)
			{
				if ($file[0] != '.' && $file != 'CVS' && strpos($file, 'index.') === false)
				{
					$avatar_dir_size += filesize(phpbb::$phpbb_root_path . phpbb::$config['avatar_path'] . '/' . $file);
				}
			}
			closedir($avatar_dir);
		...
		phpbb::$template->assign_vars(array(
			...
			'GZIP_COMPRESSION'	=> (phpbb::$config['gzip_compress'] && @extension_loaded('zlib')) ? phpbb::$user->lang['ON'] : phpbb::$user->lang['OFF'],
			'DATABASE_INFO'		=> phpbb::$db->sql_server_info(),
			'BOARD_VERSION'		=> phpbb::$config['version'],

			'U_ACTION'			=> $this->u_action,
			'U_ADMIN_LOG'		=> append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=logs&amp;mode=admin'),
			'U_INACTIVE_USERS'	=> append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=inactive&amp;mode=list'),
			'U_VERSIONCHECK'	=> append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=update&amp;mode=version_check'),
			'U_VERSIONCHECK_FORCE'	=> append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=1&amp;versioncheck_force=1'),

			'S_ACTION_OPTIONS'	=> (phpbb::$auth->acl_get('a_board')) ? true : false,
		...
		if (phpbb::$auth->acl_get('a_viewlogs'))
		{
		...
		if (file_exists(phpbb::$phpbb_root_path . 'install') && !is_file(phpbb::$phpbb_root_path . 'install'))
		{
			phpbb::$template->assign_var('S_REMOVE_INSTALL', true);
		}

		if (!defined('PHPBB_DISABLE_CONFIG_CHECK') && file_exists(phpbb::$phpbb_root_path . 'config.' . phpbb::$phpEx) && phpbb_is_writable(phpbb::$phpbb_root_path . 'config.' . phpbb::$phpEx))
		{
			// World-Writable? (000x)
			phpbb::$template->assign_var('S_WRITABLE_CONFIG', (bool) (@fileperms(phpbb::$phpbb_root_path . 'config.' . phpbb::$phpEx) & 0x0002));
		}
