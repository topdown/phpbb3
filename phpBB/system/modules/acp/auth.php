...
class auth_admin extends auth
{
			...
			if (isset($auth_ary['users']) && sizeof($auth_ary['users']))
			{
				...
				while ($row = phpbb::$db->sql_fetchrow($result))
				{
					phpbb::$template->assign_block_vars('role_mask.users', array(
						...
						'U_PROFILE'		=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, "mode=viewprofile&amp;u={$row['user_id']}"))
					);
				}
				phpbb::$db->sql_freeresult($result);
			}
			...
			if (isset($auth_ary['groups']) && sizeof($auth_ary['groups']))
			{
				...
				while ($row = phpbb::$db->sql_fetchrow($result))
				{
					phpbb::$template->assign_block_vars('role_mask.groups', array(
						...
						'U_PROFILE'		=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, "mode=group&amp;g={$row['group_id']}"))
					);
				}
				phpbb::$db->sql_freeresult($result);
			}
	...
	*/
	function acl_add_option($options)
	{
		...
		phpbb::$cache->destroy('_acl_options');
			...
			while (list($permission, $allowed) = each($cat_array['permissions']))
			{
				if ($s_view)
				{
					phpbb::$template->assign_block_vars($tpl_cat . '.' . $tpl_mask, array(
						...
						'U_TRACE'		=> ($show_trace) ? append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, "i=permissions&amp;mode=trace&amp;u=$ug_id&amp;f=$forum_id&amp;auth=$permission") : '',
						'UA_TRACE'		=> ($show_trace) ? append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, "i=permissions&mode=trace&u=$ug_id&f=$forum_id&auth=$permission", false) : '',
