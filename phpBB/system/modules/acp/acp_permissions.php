...
class acp_permissions
{
	...
	function main($id, $mode)
	{
		...
		if ($mode == 'trace')
		{
			...
			if ($user_id && isset($auth_admin->acl_options['id'][$permission]) && phpbb::$auth->acl_get('a_viewauth'))
			{
		...
		if ($mode == 'setting_forum_copy')
		{
			...
			if (phpbb::$auth->acl_get('a_fauth') && phpbb::$auth->acl_get('a_authusers') && phpbb::$auth->acl_get('a_authgroups') && phpbb::$auth->acl_get('a_mauth'))
			{
		...
		foreach ($permission_victim as $victim)
		{
			switch ($victim)
			{
				...
				case 'user':
					...
					phpbb::$template->assign_vars(array(
						'S_SELECT_USER'			=> true,
						'U_FIND_USERNAME'		=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=searchuser&amp;form=select_victim&amp;field=username&amp;select_single=true'),
					));
				...
				case 'usergroup_view':
					...
					phpbb::$template->assign_vars(array(
						...
						'U_FIND_USERNAME'			=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=searchuser&amp;form=add_user&amp;field=username&amp;select_single=true'),
					));
			...
			phpbb::$template->assign_vars(array(
				...
				'S_CAN_SELECT_USER'		=> (phpbb::$auth->acl_get('a_authusers')) ? true : false,
				'S_CAN_SELECT_GROUP'	=> (phpbb::$auth->acl_get('a_authgroups')) ? true : false,
	...
	*/
	function build_permission_dropdown($options, $default_option, $permission_scope)
	{
		...
		{
			if (!phpbb::$auth->acl_get('a_' . str_replace('_', '', $setting) . 'auth'))
			{
				continue;
			}
	...
	*/
	function set_permissions($mode, $permission_type, &$auth_admin, &$user_id, &$group_id)
	{
		...
		if (!phpbb::$auth->acl_get('a_' . str_replace('_', '', $permission_type) . 'auth') || !phpbb::$auth->acl_get('a_auth' . $ug_type . 's'))
		{
	...
	*/
	function permission_trace($user_id, $forum_id, $permission)
	{
		...
		if (sizeof($groups))
		{
			...
			$hold_ary = phpbb::$auth->acl_group_raw_data(array_keys($groups), $permission, $forum_id);
			...
			foreach ($groups as $id => $row)
			{
				switch ($row['auth_setting'])
				{
					...
					case ACL_YES:
						$information = ($total == ACL_YES) ? phpbb::$user->lang['TRACE_GROUP_YES_TOTAL_YES' . $add_key] : (($total == ACL_NEVER) ? phpbb::$user->lang['TRACE_GROUP_YES_TOTAL_NEVER' . $add_key] : phpbb::$user->lang['TRACE_GROUP_YES_TOTAL_NO' . $add_key]);
						$total = ($total == ACL_NO) ? ACL_YES : $total;
					break;

					case ACL_NEVER:
						$information = ($total == ACL_YES) ? phpbb::$user->lang['TRACE_GROUP_NEVER_TOTAL_YES' . $add_key] : (($total == ACL_NEVER) ? phpbb::$user->lang['TRACE_GROUP_NEVER_TOTAL_NEVER' . $add_key] : phpbb::$user->lang['TRACE_GROUP_NEVER_TOTAL_NO' . $add_key]);
						$total = ACL_NEVER;
					break;
				}
		...
		$hold_ary = phpbb::$auth->acl_user_raw_data($user_id, $permission, $forum_id);
		$auth_setting = (!sizeof($hold_ary)) ? ACL_NO : $hold_ary[$user_id][$forum_id][$permission];

		switch ($auth_setting)
		{
			case ACL_NO:
				$information = ($total == ACL_NO) ? phpbb::$user->lang['TRACE_USER_NO_TOTAL_NO' . $add_key] : phpbb::$user->lang['TRACE_USER_KEPT' . $add_key];
				$total = ($total == ACL_NO) ? ACL_NEVER : $total;
			break;

			case ACL_YES:
				$information = ($total == ACL_YES) ? phpbb::$user->lang['TRACE_USER_YES_TOTAL_YES' . $add_key] : (($total == ACL_NEVER) ? phpbb::$user->lang['TRACE_USER_YES_TOTAL_NEVER' . $add_key] : phpbb::$user->lang['TRACE_USER_YES_TOTAL_NO' . $add_key]);
				$total = ($total == ACL_NO) ? ACL_YES : $total;
			break;

			case ACL_NEVER:
				$information = ($total == ACL_YES) ? phpbb::$user->lang['TRACE_USER_NEVER_TOTAL_YES' . $add_key] : (($total == ACL_NEVER) ? phpbb::$user->lang['TRACE_USER_NEVER_TOTAL_NEVER' . $add_key] : phpbb::$user->lang['TRACE_USER_NEVER_TOTAL_NO' . $add_key]);
				$total = ACL_NEVER;
			break;
		}
		...
		if ($forum_id != 0 && isset(phpbb::$auth->acl_options['global'][$permission]))
		{
			if ($user_id != phpbb::$user->data['user_id'])
			{
				$auth2 = new auth();
			...
			{
				$auth_setting = phpbb::$auth->acl_get($permission);
			}

			if ($auth_setting)
			{
				$information = ($total == ACL_YES) ? phpbb::$user->lang['TRACE_USER_GLOBAL_YES_TOTAL_YES'] : phpbb::$user->lang['TRACE_USER_GLOBAL_YES_TOTAL_NEVER'];
				$total = ACL_YES;
			}
			else
			{
	...
	function copy_forum_permissions()
	{
		...
		if ($submit)
		{
			...
			if (confirm_box(true))
			{
				...
				{
					trigger_error(phpbb::$user->lang['SELECTED_FORUM_NOT_EXIST'] . adm_back_link($this->u_action), E_USER_WARNING);
				}
			}
			else
			{
				...
				confirm_box(false, phpbb::$user->lang['COPY_PERMISSIONS_CONFIRM'], $s_hidden_fields);
			}
		}
