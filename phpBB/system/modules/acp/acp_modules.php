...
class acp_modules
{
	...
	{
		global $module, $phpbb_admin_path;
				...
				if (confirm_box(true))
				{
					...
					if (isset($fileinfo['modes'][$module_mode]))
					{
						$module_data = array(
							...
							'module_enabled' => 0,
							'module_display' => (isset($fileinfo['modes'][$module_mode]['display']))
								? $fileinfo['modes'][$module_mode]['display'] : 1,
							'parent_id' => $this->parent_id,
			...
			case 'add':
				...
				if ($submit)
				{
					...
					if (!sizeof($errors))
					{
						...
						trigger_error((($action == 'add') ? phpbb::$user->lang['MODULE_ADDED']
								: phpbb::$user->lang['MODULE_EDITED']) . adm_back_link($this->u_action . '&amp;parent_id=' . $this->parent_id));
					}
				}
				...
				foreach ($module_infos as $option => $values)
				{
					...
					// Name options
					$s_name_options .= '<option value="' . $option . '"' . (($option == $module_data['module_basename'])
						? ' selected="selected"'
						: '') . '>' . $this->lang_name($values['title']) . ' [' . $this->module_class . '_' . $option . ']</option>';
					...
					foreach ($values['modes'] as $m_mode => $m_values)
					{
						...
						{
							$s_mode_options .= '<option value="' . $m_mode . '"' . (($m_mode == $module_data['module_mode'])
								? ' selected="selected"'
								: '') . '>' . $this->lang_name($m_values['title']) . '</option>';
						}
				...
				$s_cat_option = '<option value="0"' . (($module_data['parent_id'] == 0) ? ' selected="selected"'
					: '') . '>' . phpbb::$user->lang['NO_PARENT'] . '</option>';

				phpbb::$template->assign_vars(array_merge(array(
							'S_EDIT_MODULE' => true,
							'S_IS_CAT' => $is_cat,
							'S_CAT_OPTIONS' => $s_cat_option . $this->make_module_select($module_data['parent_id'], ($action == 'edit')
									? $module_row['module_id'] : false, false, false, false, true),
			...
			do
			{
				...
				else
				{
					$module_image = (!$row['module_basename'] || $row['left_id'] + 1 != $row['right_id'])
						? '<img src="images/icon_subfolder.gif" alt="' . phpbb::$user->lang['CATEGORY'] . '" />'
						: '<img src="images/icon_folder.gif" alt="' . phpbb::$user->lang['MODULE'] . '" />';
				}
	...
	function get_module_infos($module = '', $module_class = false)
	{
		...
		$directory = phpbb::$phpbb_root_path . 'system/includes/' . $module_class . '/info/';
		$fileinfo = array();
		...
		if (!$module)
		{
			...
			while (($file = readdir($dh)) !== false)
			{
				...
				{
					$class = str_replace("." . phpbb::$phpEx, '', $file) . '_info';
		...
		else
		{
			...
			if (!class_exists($class))
			{
				include($directory . $filename . '.' . phpbb::$phpEx);
			}
	...
	function make_module_select($select_id = false, $ignore_id = false, $ignore_acl = false, $ignore_nonpost = false, $ignore_emptycat = true, $ignore_noncat = false)
	{
		...
		while ($row = phpbb::$db->sql_fetchrow($result))
		{
			...
			$selected = (is_array($select_id)) ? ((in_array($row['module_id'], $select_id)) ? ' selected="selected"'
				: '') : (($row['module_id'] == $select_id) ? ' selected="selected"' : '');

			$langname = $this->lang_name($row['module_langname']);
			$module_list .= '<option value="' . $row['module_id'] . '"' . $selected . ((!$row['module_enabled'])
				? ' class="disabled"' : '') . '>' . $padding . $langname . '</option>';
	...
	function remove_cache_file()
	{
		...
		phpbb::$cache->destroy('_modules_' . $p_class);

		// Additionally remove sql cache
		phpbb::$cache->destroy('sql', MODULES_TABLE);
	}

	/**
	 * Return correct language name

	 * @param $module_langname
	 * @return string $module_langname
	 */
	function lang_name($module_langname)
	{


		return (!empty(phpbb::$user->lang[$module_langname])) ? phpbb::$user->lang[$module_langname] : $module_langname;
	}
	...
	function move_module_by($module_row, $action = 'move_up', $steps = 1)
	{
		...
		$sql = 'SELECT module_id, left_id, right_id, module_langname
			...
			WHERE module_class = '" . phpbb::$db->sql_escape($this->module_class) . "'
				...
				AND ' . (($action == 'move_up')
			? 'right_id < ' . (int) $module_row['right_id'] . ' ORDER BY right_id DESC'
			: 'left_id > ' . (int) $module_row['left_id'] . ' ORDER BY left_id ASC');
