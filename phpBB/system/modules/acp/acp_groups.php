...
class acp_groups
{
	...
	function main($id, $mode)
	{
		...
		switch ($action)
		{
			...
			case 'delete':
				...
				if (confirm_box(true))
				{
					...
					{
						case 'delete':
							if (!phpbb::$auth->acl_get('a_groupdel'))
							{
								trigger_error(phpbb::$user->lang['NO_AUTH_OPERATION'] . adm_back_link($this->u_action), E_USER_WARNING);
							}
			...
			case 'add':
				...
				if ($action == 'add' && !phpbb::$auth->acl_get('a_groupadd'))
				{
					trigger_error(phpbb::$user->lang['NO_AUTH_OPERATION'] . adm_back_link($this->u_action), E_USER_WARNING);
				}
				...
				if ($update)
				{
					...
					if (!sizeof($error))
					{
						...
						if (!($error = group_create($group_id, $group_type, $group_name, $group_desc, $group_attributes, $allow_desc_bbcode, $allow_desc_urls, $allow_desc_smilies)))
						{
							...
							if ($group_perm_from && $action == 'add' && phpbb::$auth->acl_get('a_authgroups') && phpbb::$auth->acl_gets('a_aauth', 'a_fauth', 'a_mauth', 'a_uauth'))
							{
								...
								if ($check_row && (phpbb::$user->data['user_type'] == USER_FOUNDER || $check_row['group_founder_manage'] == 0))
								{
				...
				switch ($back_link)
				{
					case 'acp_users_groups':
						$u_back = append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=users&amp;mode=groups&amp;u=' . request_var('u', 0));
					break;
				...
				phpbb::$template->assign_vars(array(
					...
					'S_GROUP_PERM'		=> ($action == 'add' && phpbb::$auth->acl_get('a_authgroups') && phpbb::$auth->acl_gets('a_aauth', 'a_fauth', 'a_mauth', 'a_uauth')) ? true : false,
					...
					'S_USER_FOUNDER'	=> (phpbb::$user->data['user_type'] == USER_FOUNDER) ? true : false,
					...
					'S_GROUP_OPTIONS'		=> group_select_options(false, false, ((phpbb::$user->data['user_type'] == USER_FOUNDER) ? false : 0)),
					'AVATAR'				=> $avatar_img,
					...
					'U_BACK'			=> $u_back,
					'U_SWATCH'			=> append_sid("{$phpbb_admin_path}swatch." . phpbb::$phpEx, 'form=settings&amp;name=group_colour'),
			...
			case 'list':
				...
				while ($row = phpbb::$db->sql_fetchrow($result))
				{
					phpbb::$template->assign_block_vars('leader', array(
						'U_USER_EDIT'		=> append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, "i=users&amp;action=edit&amp;u={$row['user_id']}"),
				...
				phpbb::$template->assign_vars(array(
					...
					'U_BACK'			=> $this->u_action,
					'U_FIND_USERNAME'	=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=searchuser&amp;form=list&amp;field=usernames'),
		...
		phpbb::$template->assign_vars(array(
			...
			'S_GROUP_ADD'	=> (phpbb::$auth->acl_get('a_groupadd')) ? true : false)
		);
		...
		foreach ($cached_group_data as $type => $row_ary)
		{
			...
			foreach ($row_ary as $group_id => $row)
			{
				...
				phpbb::$template->assign_block_vars('groups', array(
					...
					'U_DELETE'		=> (phpbb::$auth->acl_get('a_groupdel')) ? "{$this->u_action}&amp;action=delete&amp;g=$group_id" : '',
