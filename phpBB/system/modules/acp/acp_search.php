...
class acp_search
{
				...
				if (!$this->init_search($cfg_array['search_type'], $search, $error))
				{
					if (confirm_box(true))
					{
						if (!method_exists($search, 'init') || !($error = $search->init()))
						{
							...
							$extra_message = '<br />' . phpbb::$user->lang['SWITCHED_SEARCH_BACKEND'] . '<br /><a href="' . append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=search&amp;mode=index') . '">&raquo; ' . phpbb::$user->lang['GO_TO_SEARCH_INDEX'] . '</a>';
						}
						else
						{
	...
	function index($id, $mode)
	{
			...
			switch ($action)
			{
				case 'delete':
					if (method_exists($this->search, 'delete_index'))
					{
						...
						if ($error = $this->search->delete_index($this, append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, "i=$id&mode=$mode&action=delete", false)))
						{
							$this->state = array('');
							$this->save_state();
				...
				case 'create':
					if (method_exists($this->search, 'create_index'))
					{
						...
						if ($error = $this->search->create_index($this, append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, "i=$id&mode=$mode&action=create", false)))
						{
							$this->state = array('');
							$this->save_state();
							trigger_error($error . adm_back_link($this->u_action) . $this->close_popup_js(), E_USER_WARNING);
					...
					else
					{
						...
						while (still_on_time() && $post_counter <= $this->max_post_id)
						{
							...
							$buffer = phpbb::$db->sql_buffer_nested_transactions();

							if ($buffer)
							{
								$rows = phpbb::$db->sql_fetchrowset($result);
							...
							$i = 0;
							while ($row = ($buffer ? $rows[$i++] : phpbb::$db->sql_fetchrow($result)))
							{
		...
		phpbb::$template->assign_vars(array(
			'S_INDEX'				=> true,
			'U_ACTION'				=> $this->u_action,
			'U_PROGRESS_BAR'		=> append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, "i=$id&amp;mode=$mode&amp;action=progress_bar"),
			'UA_PROGRESS_BAR'		=> addslashes(append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, "i=$id&amp;mode=$mode&amp;action=progress_bar")),
		));
	...
	function display_progress_bar($type)
	{
		...
		adm_page_header(phpbb::$user->lang[$l_type]);
	...
	function get_search_types()
	{
		...
		$dp = @opendir(phpbb::$phpbb_root_path . 'system/search');
	...
	*/
	function init_search($type, &$search, &$error)
	{
		...
		if (!preg_match('#^\w+$#', $type) || !file_exists(phpbb::$phpbb_root_path . "system/search/$type.$phpEx"))
		{
			$error = phpbb::$user->lang['NO_SUCH_SEARCH_MODULE'];
			return $error;
		}

		include_once(phpbb::$phpbb_root_path . "system/search/$type.$phpEx");
