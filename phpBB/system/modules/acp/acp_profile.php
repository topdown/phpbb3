	...
	function main($id, $mode)
	{
				...
				switch ($step)
				{
					...
					case 1:
						...
						phpbb::$template->assign_vars(array(
							...
							'FIELD_TYPE'		=> phpbb::$user->lang['FIELD_' . strtoupper($cp->profile_types[$field_type])],
						...
						if ($field_type == FIELD_STRING || $field_type == FIELD_TEXT)
						{
							phpbb::$template->assign_vars(array(
								...
								'L_DEFAULT_VALUE_EXPLAIN'	=> phpbb::$user->lang[strtoupper($cp->profile_types[$field_type]) . '_DEFAULT_VALUE_EXPLAIN'],
						...
						if ($field_type == FIELD_BOOL || $field_type == FIELD_DROPDOWN)
						{
							...
							phpbb::$template->assign_vars(array(
								...
								'L_LANG_OPTIONS_EXPLAIN'	=> phpbb::$user->lang[strtoupper($cp->profile_types[$field_type]) . '_ENTRIES_EXPLAIN'],
		...
		while ($row = phpbb::$db->sql_fetchrow($result))
		{
			...
			phpbb::$template->assign_block_vars('fields', array(
				...
				'FIELD_TYPE'		=> phpbb::$user->lang['FIELD_' . strtoupper($cp->profile_types[$row['field_type']])],
