		...
		switch ($action)
		{
			...
			case 'delete':
				...
				if (confirm_box(true))
				{
					...
					$result = phpbb::$db->sql_query($sql);
					$other_reason_id = (int) phpbb::$db->sql_fetchfield('reason_id');
					phpbb::$db->sql_freeresult($result);
					...
					switch (phpbb::$db->sql_layer)
					{
						...
						case 'mysql':
							...
							$sql = 'UPDATE ' . REPORTS_TABLE . '
								SET reason_id = ' . $other_reason_id . ", report_text = CONCAT('" . phpbb::$db->sql_escape($reason_row['reason_description']) . "\n\n', report_text)
								WHERE reason_id = $reason_id";
						break;
						...
						case 'mssqlnative':
							...
							$sql = "DECLARE @ptrval binary(16)
									...
									UPDATETEXT " . REPORTS_TABLE . ".report_text @ptrval 0 0 '" . phpbb::$db->sql_escape($reason_row['reason_description']) . "\n\n'
						...
						case 'sqlite':
							// Change the reports using this reason to 'other'
							$sql = 'UPDATE ' . REPORTS_TABLE . '
								SET reason_id = ' . $other_reason_id . ", report_text = '" . phpbb::$db->sql_escape($reason_row['reason_description']) . "\n\n' || report_text
								WHERE reason_id = $reason_id";
						break;
					}
