...
class acp_board
{
	...
	function main($id, $mode)
	{
		...
		*/
			...
			case 'post':
				$display_vars = array(
					...
					'vars'	=> array(
						...
						'delete_time'			=> array('lang' => 'DELETE_TIME',			'validate' => 'int:0',		'type' => 'text:5:5', 'explain' => true, 'append' => ' ' . phpbb::$user->lang['MINUTES']),
			...
			case 'registration':
				$display_vars = array(
					...
					'vars'	=> array(
						...
						'new_member_post_limit'	=> array('lang' => 'NEW_MEMBER_POST_LIMIT', 'validate' => 'int:0:255', 'type' => 'text:4:4', 'explain' => true, 'append' => ' ' . phpbb::$user->lang['POSTS']),
			...
			case 'security':
				$display_vars = array(
					...
					'vars'	=> array(
						...
						'ip_login_limit_time'	=> array('lang' => 'IP_LOGIN_LIMIT_TIME',	'validate' => 'int:0',	'type' => 'text:5:5', 'explain' => true, 'append' => ' ' . phpbb::$user->lang['SECONDS']),
		...
		if ($mode == 'feed' && $submit)
		{
			phpbb::$cache->destroy('_feed_news_forum_ids');
			phpbb::$cache->destroy('_feed_excluded_forum_ids');
		...
		if ($mode == 'auth')
		{
			...
			$dp = @opendir(phpbb::$phpbb_root_path . 'system/includes/auth');
			...
			{
				if ($method && file_exists(phpbb::$phpbb_root_path . 'system/auth/auth_' . $method . '.' . phpbb::$phpEx))
				{
	...
	*/
	function quick_reply($value, $key)
	{
		...
		return h_radio('config[allow_quick_reply]', $radio_ary, $value) .
			'<br /><br /><input class="button2" type="submit" id="' . $key . '_enable" name="' . $key . '_enable" value="' . phpbb::$user->lang['ALLOW_QUICK_REPLY_BUTTON'] . '" />';
	}
	...
	*/
	function dateformat_select($value, $key)
	{
		...
		phpbb::$user->timezone = phpbb::$config['board_timezone'] * 3600;
		phpbb::$user->dst = phpbb::$config['board_dst'] * 3600;
	...
	*/
	...
	function store_feed_forums($option, $key)
	{
		...
		$sql = 'UPDATE ' . FORUMS_TABLE . '
			...
			WHERE ' . phpbb::$db->sql_bit_and('forum_options', $option, '<> 0');
		...
		if (sizeof($values))
		{
			...
			$sql = 'UPDATE ' . FORUMS_TABLE . '
				SET forum_options = forum_options + ' . (1 << $option) . '
				WHERE ' . phpbb::$db->sql_in_set('forum_id', $values);
			phpbb::$db->sql_query($sql);
		}
