...
class acp_database
{
	...
	{
		global $table_prefix, $phpbb_admin_path;
			...
			case 'restore':
				...
				switch ($action)
				{
					case 'submit':
						...
						$file_name = phpbb::$phpbb_root_path . 'store/' . $matches[0];
					...
					default:
						...
						$dir = phpbb::$phpbb_root_path . 'store/';
						$dh = @opendir($dir);
						...
						if (!empty($backup_files))
						{
							...
							foreach ($backup_files as $name => $file)
							{
								phpbb::$template->assign_block_vars('files', array(
									'FILE'		=> $file,
									'NAME'		=> phpbb::$user->format_date($name, 'd-m-Y H:i:s', true),
									'SUPPORTED'	=> true,
								));
							}
						}
...
class base_extractor
{
	...
	function base_extractor($download = false, $store = false, $format, $filename, $time)
	{
		...
		if ($store == true)
		{
			...
			$file = phpbb::$phpbb_root_path . 'store/' . $filename . $ext;
...
class mysql_extractor extends base_extractor
{
	...
	function write_table($table_name)
	{
		...
		if ($new_extract === null)
		{
			if (phpbb::$db->sql_layer === 'mysqli' || version_compare(phpbb::$db->sql_server_info(true), '3.23.20', '>='))
			{
				$new_extract = true;
			}
			else
...
class postgres_extractor extends base_extractor
{
	...
	function write_table($table_name)
	{
		...
		while ($row = phpbb::$db->sql_fetchrow($result))
		{
			...
			$def_row = phpbb::$db->sql_fetchrow($def_res);
...
class mssql_extractor extends base_extractor
{
	...
	function write_data($table_name)
	{
		...
		else if(phpbb::$db->sql_layer === 'mssqlnative')
		{
	...
	function write_data_mssqlnative($table_name)
	{
		...
		phpbb::$db->mssqlnative_set_query_options(array('Scrollable' => SQLSRV_CURSOR_STATIC));
		$result = phpbb::$db->sql_query($sql);

		$retrieved_data = phpbb::$db->mssqlnative_num_rows($result);
		...
		$sql = "SELECT * FROM $table_name";
		$result_fields = phpbb::$db->sql_query_limit($sql, 1);
		...
		phpbb::$db->sql_freeresult($result_fields);
