...
#';
				...
				switch ($action)
				{
					...
					case 'refresh':
						...
						if (confirm_box(true))
						{
							...
							if ($template_row['template_storedb'] && file_exists(phpbb::$phpbb_root_path . "styles/{$template_row['template_path']}/template/"))
							{
								...
								{
									//									if (@filemtime(phpbb::$phpbb_root_path . "styles/{$template_row['template_path']}/template/" . $row['template_filename']) > $row['template_mtime'])
									//									{
								...
								$template_refreshed = phpbb::$user->lang['TEMPLATE_REFRESHED'] . '<br />';
								add_log('admin', 'LOG_TEMPLATE_REFRESHED', $template_row['template_name']);
							}

							$this->clear_template_cache($template_row);

							trigger_error($template_refreshed . phpbb::$user->lang['TEMPLATE_CACHE_CLEARED'] . adm_back_link($this->u_action));
						}
						else
						{
							confirm_box(false, ($template_row['template_storedb'])
									? phpbb::$user->lang['CONFIRM_TEMPLATE_REFRESH']
									: phpbb::$user->lang['CONFIRM_TEMPLATE_CLEAR_CACHE'], build_hidden_fields(array(
										'i' => $id,
										'mode' => $mode,
										'action' => $action,
										'id' => $style_id
			...
			case 'theme':
				...
				switch ($action)
				{
					...
					case 'refresh':
						...
						if (confirm_box(true))
						{
							if ($theme_row['theme_storedb'] && file_exists(phpbb::$phpbb_root_path . "styles/{$theme_row['theme_path']}/theme/stylesheet.css"))
							{
								// Save CSS contents
								$sql_ary = array(
									'theme_mtime' => (int) filemtime(phpbb::$phpbb_root_path . "styles/{$theme_row['theme_path']}/theme/stylesheet.css"),
				...
				switch ($action)
				{
					case 'refresh':
						...
						if (confirm_box(true))
						{
							...
							$cfg_data_imageset = parse_cfg_file(phpbb::$phpbb_root_path . "styles/{$imageset_row['imageset_path']}/imageset/imageset.cfg");
							...
							while ($row = phpbb::$db->sql_fetchrow($result))
							{
								if (@file_exists(phpbb::$phpbb_root_path . "styles/{$imageset_row['imageset_path']}/imageset/{$row['lang_dir']}/imageset.cfg"))
								{
									$cfg_data_imageset_data = parse_cfg_file(phpbb::$phpbb_root_path . "styles/{$imageset_row['imageset_path']}/imageset/{$row['lang_dir']}/imageset.cfg");
							...
							phpbb::$cache->destroy('sql', STYLES_IMAGESET_DATA_TABLE);
							phpbb::$cache->destroy('imageset_site_logo_md5');
	...
	function frontend($mode, $options, $actions)
	{
		...
		while ($row = phpbb::$db->sql_fetchrow($result))
		{
			...
			phpbb::$template->assign_block_vars('installed', array(
					'S_DEFAULT_STYLE' => ($mode == 'style' && $row['style_id'] == phpbb::$config['default_style']) ? true
						: false,
					'U_EDIT' => $this->u_action . '&amp;action=' . (($mode == 'style') ? 'details'
						: 'edit') . '&amp;id=' . $row[$mode . '_id'],
					...
					'U_PREVIEW' => ($mode == 'style')
						? append_sid(phpbb::$phpbb_root_path . "index." . phpbb::$phpEx, "$mode=" . $row[$mode . '_id']) : '',

					'NAME' => $row[$mode . '_name'],
					'STYLE_COUNT' => ($mode == 'style' && isset($style_count[$row['style_id']]))
						? $style_count[$row['style_id']] : 0,
				)
			);
		}
		phpbb::$db->sql_freeresult($result);
		...
		$dp = @opendir(phpbb::$phpbb_root_path . "styles");

		if ($dp)
		{
			while (($file = readdir($dp)) !== false)
			{
				if ($file[0] == '.' || !is_dir(phpbb::$phpbb_root_path . 'styles/' . $file))
				{
					continue;
				}

				$subpath = ($mode != 'style') ? "$mode/" : '';
				if (file_exists(phpbb::$phpbb_root_path . "styles/$file/$subpath$mode.cfg"))
				{
					if ($cfg = file(phpbb::$phpbb_root_path . "styles/$file/$subpath$mode.cfg"))
					{
	...
	function edit_template($template_id)
	{
		...
		if ($save_changes && $template_file)
		{
			...
			$file = phpbb::$phpbb_root_path . "styles/{$template_info['template_path']}/template/$template_file";
			$additional = '';
			...
			if (!$safe_mode && !$template_info['template_storedb'] && file_exists($file) && phpbb_is_writable($file))
			{
				if (!($fp = @fopen($file, 'wb')))
				{
					...
					trigger_error(sprintf(phpbb::$user->lang['TEMPLATE_FILE_NOT_WRITABLE'], htmlspecialchars($template_file)) . adm_back_link($this->u_action), E_USER_WARNING);
				}
			...
			else
			{
				...
				if (!$template_info['template_storedb'])
				{
					...
					$additional .= '<br />' . phpbb::$user->lang['EDIT_TEMPLATE_STORED_DB'];
				}

				// Update the template_data table entry for this template file
				$sql = 'UPDATE ' . STYLES_TEMPLATE_DATA_TABLE . "
					SET template_data = '" . phpbb::$db->sql_escape($template_data) . "', template_mtime = " . time() . "
					WHERE template_id = $template_id
						AND template_filename = '" . phpbb::$db->sql_escape($template_file) . "'";
				phpbb::$db->sql_query($sql);
			...
			phpbb::$cache->destroy('sql', STYLES_TABLE);
		...
		if (!$template_info['template_storedb'])
		{
			$template_path = phpbb::$phpbb_root_path . "styles/{$template_info['template_path']}/template";
	...
	function template_cache($template_id)
	{
		...
		if ($source && file_exists(phpbb::$phpbb_root_path . "cache/{$cache_prefix}_$source.html.$phpEx"))
		{
			adm_page_header(phpbb::$user->lang['TEMPLATE_CACHE']);
			...
			$code = str_replace(array("\r\n", "\r"), array("\n", "\n"), file_get_contents(phpbb::$phpbb_root_path . "cache/{$cache_prefix}_$source.html.$phpEx"));
		...
		foreach ($file_ary as $file)
		{
			...
			if (!file_exists(phpbb::$phpbb_root_path . "cache/$filename"))
			{
				continue;
			}

			$file_tpl = phpbb::$phpbb_root_path . "styles/{$template_row['template_path']}/template/$tpl_file.html";
			$inherited = false;
			...
			if (isset($template_row['template_inherits_id']) && $template_row['template_inherits_id'])
			{
				if (!$template_row['template_storedb'])
				{
					...
					{
						$file_tpl = phpbb::$phpbb_root_path . "styles/{$template_row['template_inherit_path']}/template/$tpl_file.html";
						$inherited = true;
					}
				}
				else
			...
			phpbb::$template->assign_block_vars('file', array(
					...
					'CACHED' => phpbb::$user->format_date(filemtime(phpbb::$phpbb_root_path . "cache/$filename")),
					'FILENAME' => $file,
					'FILENAME_PATH' => $file_tpl,
					'FILESIZE' => get_formatted_filesize(filesize(phpbb::$phpbb_root_path . "cache/$filename")),
					'MODIFIED' => phpbb::$user->format_date((!$template_row['template_storedb']) ? filemtime($file_tpl)
							: $filemtime[$file . '.html'])
				)
			);
		}
	...
	function edit_theme($theme_id)
	{
		...
		{
			// Get the filesystem location of the current file
			$file = phpbb::$phpbb_root_path . "styles/{$theme_info['theme_path']}/theme/$theme_file";
			$additional = '';
			...
			phpbb::$cache->destroy('sql', STYLES_THEME_TABLE);
			add_log('admin', (!$theme_info['theme_storedb']) ? 'LOG_THEME_EDIT_FILE'
					: 'LOG_THEME_EDIT', $theme_info['theme_name'], (!$theme_info['theme_storedb']) ? $theme_file : '');
		...
		if (!$theme_info['theme_storedb'])
		{
			$theme_path = phpbb::$phpbb_root_path . "styles/{$theme_info['theme_path']}/theme";
	...
	function edit_imageset($imageset_id)
	{
		...
		if ($update && isset($_POST['imgpath']) && $valid_name)
		{
			...
			if ($imgpath && !file_exists(phpbb::$phpbb_root_path . "styles/$imageset_path/imageset/$imgpath"))
			{
			...
			if ($imgsize && $imgpath)
			{
				...
				{
					list($imgwidth_file, $imgheight_file) = getimagesize(phpbb::$phpbb_root_path . "styles/$imageset_path/imageset/$imgpath");
		...
		$dir = phpbb::$phpbb_root_path . "styles/$imageset_path/imageset";
		$dp = @opendir($dir);
		...
		$image_request = '../styles/' . $imageset_path . '/imageset/' . ($image_lang ? $imgnamelang . '/'
			: '') . $image_filename;
		...
		phpbb::$template->assign_vars(array(
				...
				'ERROR' => !$valid_name,
				'IMG_SRC' => ($image_found) ? '../styles/' . $imageset_path . '/imageset/' . $img_val
					: 'images/no_image.png',
	...
	function remove($mode, $style_id)
	{
		...
		if ($mode === 'template' && ($conflicts = $this->check_inheritance($mode, $style_id)))
		{
			...
			$msg = phpbb::$user->lang[$l_type . '_DELETE_DEPENDENT'];
	...
	function display_component_options($component, $component_id, $style_row = false, $style_id = false)
	{
		...
		if (($component != 'style') && empty($component_in_use))
		{
			...
			$s_options .= '<option value="-1" selected="selected">' . phpbb::$user->lang['DELETE_' . strtoupper($component)] . '</option>';
			$s_options .= '<option value="0">' . sprintf(phpbb::$user->lang['KEEP_' . strtoupper($component)], $row[$component . '_name']) . '</option>';
		}
		else
		{
			$sql = "SELECT {$component}_id, {$component}_name
			...
			while ($row = phpbb::$db->sql_fetchrow($result))
			{
				...
				{
					$s_options .= '<option value="' . $row[$component . '_id'] . '">' . sprintf(phpbb::$user->lang['REPLACE_WITH_OPTION'], $row[$component . '_name']) . '</option>';
				}
				else if ($component != 'style')
				{
					$s_keep_option = '<option value="0" selected="selected">' . sprintf(phpbb::$user->lang['KEEP_' . strtoupper($component)], $row[$component . '_name']) . '</option>';
				}
			}
			phpbb::$db->sql_freeresult($result);
		...
		if (!$style_row)
		{
			phpbb::$template->assign_var('S_REPLACE_' . strtoupper($component) . '_OPTIONS', $s_options);
		}
		else
		{
			phpbb::$template->assign_var('S_REPLACE_OPTIONS', $s_options);
			if ($component == 'style')
			{
				$components = array('template', 'theme', 'imageset');
				foreach ($components as $component)
	...
	function export($mode, $style_id)
	{
		...
		if ($update && !sizeof($error))
		{
			...
			if ($mode == 'template' || $inc_template)
			{
				...
				if ($style_row['template_inherits_id'])
				{
					...
					$result = phpbb::$db->sql_query($sql);
					$use_template_name = (string) phpbb::$db->sql_fetchfield('template_name');
					phpbb::$db->sql_freeresult($result);
				}
			...
			if ($mode == 'theme' || $inc_theme)
			{
				...
				// Read old cfg file
				$items = phpbb::$cache->obtain_cfg_items($style_row);
				$items = $items['theme'];
			...
			if ($mode == 'imageset' || $inc_imageset)
			{
				...
				while ($row = phpbb::$db->sql_fetchrow($result))
				{
					$imageset_main[$row['image_name']] = $row['image_filename'] . ($row['image_height']
						? '*' . $row['image_height'] : '') . ($row['image_width'] ? '*' . $row['image_width'] : '');
				}
				phpbb::$db->sql_freeresult($result);
				...
				$imageset_root = phpbb::$phpbb_root_path . "styles/{$style_row['imageset_path']}/imageset/";
				...
				while ($row = phpbb::$db->sql_fetchrow($result))
				{
					$imageset_lang[$row['image_lang']][$row['image_name']] = $row['image_filename'] . ($row['image_height']
						? '*' . $row['image_height'] : '') . ($row['image_width'] ? '*' . $row['image_width'] : '');
				}
				phpbb::$db->sql_freeresult($result);
			...
			if (!sizeof($error))
			{
				include(phpbb::$phpbb_root_path . 'system/includes/functions_compress.' . phpbb::$phpEx);
				...
				if ($format == 'zip')
				{
					$compress = new compress_zip('w', phpbb::$phpbb_root_path . "store/$path$ext");
				}
				else
				{
					$compress = new compress_tar('w', phpbb::$phpbb_root_path . "store/$path$ext", $ext);
				}
				...
				if (!$store)
				{
					...
					@unlink(phpbb::$phpbb_root_path . "store/$path$ext");
					exit;
				}
		...
		{
			$format_buttons .= '<label><input type="radio"' . ((!$format_buttons) ? ' id="format"'
				: '') . ' class="radio" value="' . $method . '" name="format"' . (($method == $format)
				? ' checked="checked"' : '') . ' /> ' . $method . '</label>';
		}
	...
	function details($mode, $style_id)
	{
		...
		if ($update)
		{
			...
			$sql = "SELECT {$mode}_id, {$mode}_name
				FROM $sql_from
				WHERE {$mode}_id <> $style_id
				AND LOWER({$mode}_name) = '" . phpbb::$db->sql_escape(strtolower($name)) . "'";
			$result = phpbb::$db->sql_query($sql);
			...
			if ($mode === 'theme' || $mode === 'template')
			{
				...
				$check = phpbb::$phpbb_root_path . "styles/" . $style_row["{$mode}_path"] . (($mode === 'theme')
					? '/theme/stylesheet.css' : '/template');
				if (($style_row["{$mode}_storedb"] != $store_db) && !$store_db && ($safe_mode || !phpbb_is_writable($check)))
				{
					$error[] = phpbb::$user->lang['EDIT_' . strtoupper($mode) . '_STORED_DB'];
					$store_db = 1;
				}
				...
				if ($mode == 'theme')
				{
					$cfg = parse_cfg_file(phpbb::$phpbb_root_path . "styles/" . $style_row["{$mode}_path"] . "/theme/theme.cfg");

					if (isset($cfg['parse_css_file']) && $cfg['parse_css_file'] && !$store_db)
					{
						$error[] = phpbb::$user->lang['EDIT_THEME_STORE_PARSED'];
						$store_db = 1;
					}
				}
			}
			...
			switch ($mode)
			{
				...
				case 'theme':
					...
					if ($style_row['theme_storedb'] != $store_db)
					{
						...
						else if (!$store_db && !$safe_mode && phpbb_is_writable(phpbb::$phpbb_root_path . "styles/{$style_row['theme_path']}/theme/stylesheet.css"))
						{
							$store_db = 1;
							$theme_data = $style_row['theme_data'];

							if ($fp = @fopen(phpbb::$phpbb_root_path . "styles/{$style_row['theme_path']}/theme/stylesheet.css", 'wb'))
							{
								$store_db = (@fwrite($fp, str_replace("styles/{$style_row['theme_path']}/theme/", './', $theme_data)))
									? 0 : 1;
							}
							fclose($fp);
						}
						...
						$sql_ary += array(
							'theme_mtime' => ($store_db)
								? filemtime(phpbb::$phpbb_root_path . "styles/{$style_row['theme_path']}/theme/stylesheet.css")
								: 0,
				...
				case 'template':
					...
					if ($style_row['template_storedb'] != $store_db)
					{
						...
						{
							$error[] = (sprintf(phpbb::$user->lang["{$l_type}_INHERITS"], $super['template_name']));
							$sql_ary = array();
						}
						else
						{
							if (!$store_db && !$safe_mode && phpbb_is_writable(phpbb::$phpbb_root_path . "styles/{$style_row['template_path']}/template"))
							{
		...
		phpbb::$template->assign_vars(array(
				...
				'S_STORE_DB_DISABLED' => (isset($style_row[$mode . '_inherits_id']))
					? $style_row[$mode . '_inherits_id'] : 0,
				...
				'S_SUPERTEMPLATE' => (isset($style_row[$mode . '_inherits_id']) && $style_row[$mode . '_inherits_id'])
					? $super['template_name'] : 0,
				...
				'L_LOCATION_EXPLAIN' => ($mode == 'template' || $mode == 'theme')
					? phpbb::$user->lang[$l_type . '_LOCATION_EXPLAIN'] : '',
	...
	function load_css_file($path, $filename)
	{
		...
		$file = phpbb::$phpbb_root_path . "styles/$path/theme/$filename";
	 ...
	 * @param string $root_path should only be used in case you want to use a different root path than phpbb::$phpbb_root_path . "styles/{$theme_row['theme_path']}"
	 *
	...
	function db_theme_data($theme_row, $stylesheet = false, $root_path = '')
	{
		...
		if (!$root_path)
		{
			$root_path = phpbb::$phpbb_root_path . 'styles/' . $theme_row['theme_path'];
		}
	...
	function store_templates($mode, $style_id, $template_path, $filelist)
	{
		...
		foreach ($filelist as $pathfile => $file_ary)
		{
			foreach ($file_ary as $file)
			{
				if (!($fp = @fopen(phpbb::$phpbb_root_path . "styles/$template_path$pathfile$file", 'r')))
				{
					trigger_error("Could not open {$phpbb_root_path}styles/$template_path$pathfile$file", E_USER_ERROR);
				}

				$filesize = filesize(phpbb::$phpbb_root_path . "styles/$template_path$pathfile$file");
		...
		foreach ($filelist as $pathfile => $file_ary)
		{
			foreach ($file_ary as $file)
			{
				...
				$sql_ary = array(
					...
					'template_mtime' => (int) filemtime(phpbb::$phpbb_root_path . "styles/$template_path$pathfile$file"),
					'template_data' => (string) file_get_contents(phpbb::$phpbb_root_path . "styles/$template_path$pathfile$file"),
				);

				if ($mode == 'insert')
				{
					$sql = 'INSERT INTO ' . STYLES_TEMPLATE_DATA_TABLE . ' ' . phpbb::$db->sql_build_array('INSERT', $sql_ary);
				}
				else
				{
					$sql = 'UPDATE ' . STYLES_TEMPLATE_DATA_TABLE . ' SET ' . phpbb::$db->sql_build_array('UPDATE', $sql_ary) . "
						WHERE template_id = $style_id
							AND template_filename = '" . phpbb::$db->sql_escape("$pathfile$file") . "'";
				}
				phpbb::$db->sql_query($sql);
			}
		}
	...
	function template_cache_filelist($template_path)
	{
		...
		if (!($dp = @opendir(phpbb::$phpbb_root_path . "cache")))
		{
		...
		while ($file = readdir($dp))
		{
			...
			if (is_file(phpbb::$phpbb_root_path . 'cache/' . $file) && (strpos($file, $cache_prefix) === 0))
			{
	...
	function clear_template_cache($template_row, $file_ary = false)
	{
		...
		foreach ($file_ary as $file)
		{
			...
			$file = phpbb::$phpbb_root_path . "cache/{$cache_prefix}_$file.html.$phpEx";
	...
	function install($mode)
	{
		...
		if ($install_path)
		{
			$root_path = phpbb::$phpbb_root_path . 'styles/' . $install_path . '/';
			...
			switch ($mode)
			{
				case 'style':
					...
					foreach ($element_ary as $element => $table)
					{
						...
						$this->test_installed($element, $error, ($
							{'reqd_' . $element}) ? phpbb::$phpbb_root_path . 'styles/' . $reqd_template . '/' : $root_path, $
							{'reqd_' . $element}, $style_row[$element . '_id'], $style_row[$element . '_name'], $style_row[$element . '_copyright']);
						...
						$cfg_file = phpbb::$phpbb_root_path . 'styles/' . $install_path . '/' . $element . '/' . $element . '.cfg';
		...
		if ($update && !sizeof($error))
		{
			if ($mode == 'style')
			{
				foreach ($element_ary as $element => $table)
				{
					$
					{$element . '_root_path'} = ($
					{'reqd_' . $element}) ? phpbb::$phpbb_root_path . 'styles/' . $
					{'reqd_' . $element} . '/' : false;
					$
					{$element . '_path'} = ($
					{'reqd_' . $element}) ? $
			...
			if (!sizeof($error))
			{
				...
				trigger_error(phpbb::$user->lang[$l_type . $message] . adm_back_link($this->u_action));
			}
		}
	...
	function add($mode)
	{
		...
		if ($basis)
		{
			...
			if (!sizeof($error))
			{
				$style_row['template_id'] = (isset($row['template_id'])) ? $row['template_id']
					: $style_row['template_id'];
				$style_row['theme_id'] = (isset($row['theme_id'])) ? $row['theme_id'] : $style_row['theme_id'];
				$style_row['imageset_id'] = (isset($row['imageset_id'])) ? $row['imageset_id']
					: $style_row['imageset_id'];
			}
		}
	 ...
	 * @param $default
	 * @param $style_row
	 * @param bool $template_root_path
	 * @param bool $template_path
	 * @param bool $theme_root_path
	 * @param bool $theme_path
	 * @param bool $imageset_root_path
	 * @param bool $imageset_path
	 * @return bool
	 */
	function install_style(&$error, $action, $root_path, &$id, $name, $path, $copyright, $active, $default, &$style_row, $template_root_path = false, $template_path = false, $theme_root_path = false, $theme_path = false, $imageset_root_path = false, $imageset_path = false)
	{
		...
		foreach ($element_ary as $element)
		{
			...
			{
				$this->install_element($element, $error, $action, ($
					{$element . '_root_path'}) ? $
					{$element . '_root_path'}
						: $root_path, $style_row[$element . '_id'], $style_row[$element . '_name'], ($
					{$element . '_path'}) ? $
					{$element . '_path'} : $path, $style_row[$element . '_copyright']);
			}
		}
	...
	function install_element($mode, &$error, $action, $root_path, &$id, $name, $path, $copyright, $store_db = 0)
	{
		...
		if (isset($cfg_data['inherit_from']) && $cfg_data['inherit_from'])
		{
			...
			$sql = "SELECT {$mode}_id, {$mode}_name, {$mode}_path, {$mode}_storedb $select_bf
				FROM $sql_from
				WHERE {$mode}_name = '" . phpbb::$db->sql_escape($cfg_data['inherit_from']) . "'
			...
			if (!$row)
			{
				$error[] = sprintf(phpbb::$user->lang[$l_type . '_ERR_REQUIRED_OR_INCOMPLETE'], $cfg_data['inherit_from']);
			}
			else
			{
		...
		switch ($mode)
		{
			...
			case 'theme':
				...
				$sql_ary += array(
					...
					'theme_mtime' => (int) filemtime(phpbb::$phpbb_root_path . "styles/$path/theme/stylesheet.css")
				);
				break;
	 ...
	 * @return array|bool false if no component inherits, array with name, path and id for each subtemplate otherwise
	 */
	 ...
	 * @param string $mode The element type to check - only template is supported
	 * @param int $id The template id
	 * @return bool false if the component does not inherit, array with name, path and id otherwise
	 */
	...
	function _store_in_db($mode, $id, $path)
	{
		...
		$filelist = filelist(phpbb::$phpbb_root_path . "styles/{$path}/template", '', 'html');
		$this->store_templates('insert', $id, $path, $filelist);
	...
	function _store_in_fs($mode, $id, $path)
	{
		...
		$store_db = 0;
		$error = array();
		if (!$safe_mode && phpbb_is_writable(phpbb::$phpbb_root_path . "styles/{$path}/template"))
		{
			$sql = 'SELECT *
					FROM ' . STYLES_TEMPLATE_DATA_TABLE . "
					WHERE template_id = $id";
			...
			while ($row = phpbb::$db->sql_fetchrow($result))
			{
				if (!($fp = @fopen(phpbb::$phpbb_root_path . "styles/{$path}/template/" . $row['template_filename'], 'wb')))
				{
					$store_db = 1;
					$error[] = phpbb::$user->lang['EDIT_TEMPLATE_STORED_DB'];
					break;
				}
