	...
	function main($id, $mode)
	{
		...
		switch ($action)
		{
			...
			case 'delete':
				...
				if (!phpbb::$auth->acl_get('a_forumdel'))
				{
			...
			case 'add':
				...
				if (!phpbb::$auth->acl_get('a_forumadd'))
				{
				...
				case 'add':
					...
					if (!sizeof($errors))
					{
						...
						if ($forum_perm_from && $forum_perm_from != $forum_data['forum_id'] &&
							($action != 'edit' || empty($forum_id) || (phpbb::$auth->acl_get('a_fauth') && phpbb::$auth->acl_get('a_authusers') && phpbb::$auth->acl_get('a_authgroups') && phpbb::$auth->acl_get('a_mauth'))))
						{
...
/* Commented out because of questionable UI workflow - re-visit for 3.0.7
						else if (!$this->parent_id && $action != 'edit' && phpbb::$auth->acl_get('a_fauth') && phpbb::$auth->acl_get('a_authusers') && phpbb::$auth->acl_get('a_authgroups') && phpbb::$auth->acl_get('a_mauth'))
						{
...
*/
						...
						if (phpbb::$auth->acl_get('a_fauth') && !$copied_permissions)
						{
							$message .= '<br /><br />' . sprintf(phpbb::$user->lang['REDIRECT_ACL'], '<a href="' . append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=permissions' . $acl_url) . '">', '</a>');
						}

						// redirect directly to permission settings screen if authed
						if ($action == 'add' && !$copied_permissions && phpbb::$auth->acl_get('a_fauth'))
						{
							meta_refresh(4, append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=permissions' . $acl_url));
						}
		...
		switch ($action)
		{
			...
			case 'edit':
				...
				phpbb::$template->assign_vars(array(
					...
					'FORUM_IMAGE_SRC'			=> ($forum_data['forum_image']) ? phpbb::$phpbb_root_path . $forum_data['forum_image'] : '',
					...
					'S_CAN_COPY_PERMISSIONS'	=> ($action != 'edit' || empty($forum_id) || (phpbb::$auth->acl_get('a_fauth') && phpbb::$auth->acl_get('a_authusers') && phpbb::$auth->acl_get('a_authgroups') && phpbb::$auth->acl_get('a_mauth'))) ? true : false,
				));
			...
			case 'copy_perm':
				...
				if (!empty($forum_perm_from) && $forum_perm_from != $forum_id)
				{
					...
					$message = phpbb::$user->lang['FORUM_UPDATED'];

					// Redirect to permissions
					if (phpbb::$auth->acl_get('a_fauth'))
					{
						$message .= '<br /><br />' . sprintf(phpbb::$user->lang['REDIRECT_ACL'], '<a href="' . append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=permissions' . $acl_url) . '">', '</a>');
					}
		...
		if ($row = phpbb::$db->sql_fetchrow($result))
		{
			do
			{
				...
				phpbb::$template->assign_block_vars('forums', array(
					...
					'FORUM_IMAGE'		=> ($row['forum_image']) ? '<img src="' . phpbb::$phpbb_root_path . $row['forum_image'] . '" alt="" />' : '',
	...
	*/
	function update_forum_data(&$forum_data)
	{
		...
		if (!empty($forum_data['forum_image']) && !file_exists(phpbb::$phpbb_root_path . $forum_data['forum_image']))
		{
			$errors[] = phpbb::$user->lang['FORUM_IMAGE_NO_EXIST'];
		}
		...
		if (!isset($forum_data_sql['forum_id']))
		{
			...
			if ($forum_data_sql['parent_id'])
			{
				...
				if (!$row)
				{
					trigger_error(phpbb::$user->lang['PARENT_NOT_EXIST'] . adm_back_link($this->u_action . '&amp;parent_id=' . $this->parent_id), E_USER_WARNING);
				}
				...
				if ($row['right_id'] - $row['left_id'] > 1)
				{
					...
					if ($action_subforums == 'delete')
					{
						...
						if (sizeof($forum_ids))
						{
							...
							phpbb::$cache->destroy('_extensions');
						}
					}
	...
	*/
	function display_progress_bar($start, $total)
	{
		...
		adm_page_header(phpbb::$user->lang['SYNC_IN_PROGRESS']);
	...
	*/
	function copy_permission_page($forum_data)
	{
		...
		$l_acl = sprintf(phpbb::$user->lang['COPY_TO_ACL'], '<a href="' . append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=permissions' . $acl_url) . '">', '</a>');

		$this->tpl_name = 'acp_forums_copy_perm';

		phpbb::$template->assign_vars(array(
			'U_ACL'				=> append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=permissions' . $acl_url),
			'L_ACL_LINK'		=> $l_acl,
