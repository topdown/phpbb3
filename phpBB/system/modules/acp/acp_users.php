...
class acp_users
{
	...
	function main($id, $mode)
	{
		...
		global $phpbb_admin_path ,$table_prefix, $file_uploads;
		...
		if (!$username && !$user_id)
		{
			...
			phpbb::$template->assign_vars(array(
				...
				'S_SELECT_USER'		=> true,
				'U_FIND_USERNAME'	=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=searchuser&amp;form=select_user&amp;field=username&amp;select_single=true'),
			));
		...
		phpbb::$template->assign_vars(array(
			...
			'U_MODE_SELECT'		=> append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, "i=$id&amp;u=$user_id"),
		...
		if (phpbb::$user->data['user_type'] != USER_FOUNDER && $user_row['user_type'] == USER_FOUNDER)
		{
		...
		switch ($mode)
		{
			case 'overview':
				...
				if ($submit)
				{
					...
					if ($delete && $user_row['user_type'] != USER_FOUNDER)
					{
						...
						{
							trigger_error(phpbb::$user->lang['NO_MODE'] . adm_back_link($this->u_action . '&amp;u=' . $user_id), E_USER_WARNING);
						}
					}
					...
					switch ($action)
					{
						...
						case 'banip':
							...
							if ($user_id == ANONYMOUS)
							{
								trigger_error(phpbb::$user->lang['CANNOT_BAN_ANONYMOUS'] . adm_back_link($this->u_action . '&amp;u=' . $user_id), E_USER_WARNING);
							}
						...
						case 'deloutbox':
							...
							if (confirm_box(true))
							{
								...
								if ($row = phpbb::$db->sql_fetchrow($result))
								{
									...
									{
										include(phpbb::$phpbb_root_path . 'system/includes/functions_privmsgs.' . phpbb::$phpEx);
									}
								...
								trigger_error(phpbb::$user->lang['USER_OUTBOX_' . $lang] . adm_back_link($this->u_action . '&amp;u=' . $user_id));
							}
							else
							{
								confirm_box(false, phpbb::$user->lang['CONFIRM_OPERATION'], build_hidden_fields(array(
						...
						case 'leave_nr':
							...
							if (confirm_box(true))
							{
								...
								trigger_error(phpbb::$user->lang['USER_LIFTED_NR'] . adm_back_link($this->u_action . '&amp;u=' . $user_id));
							}
							else
							{
								confirm_box(false, phpbb::$user->lang['CONFIRM_OPERATION'], build_hidden_fields(array(
					...
					if (!sizeof($error))
					{
						...
						if ($user_row['user_type'] != USER_FOUNDER || phpbb::$user->data['user_type'] == USER_FOUNDER)
						{
						...
						phpbb::$auth->acl_clear_prefetch($user_id);
				...
				else
				{
					...
					if (phpbb::$config['email_enable'] && ($user_row['user_type'] == USER_NORMAL || $user_row['user_type'] == USER_INACTIVE))
					{
				...
				phpbb::$template->assign_vars(array(
					...
					'S_FOUNDER'					=> (phpbb::$user->data['user_type'] == USER_FOUNDER) ? true : false,
					...
					'U_SWITCH_PERMISSIONS'	=> (phpbb::$auth->acl_get('a_switchperm') && phpbb::$user->data['user_id'] != $user_row['user_id']) ? append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, "mode=switch_perm&amp;u={$user_row['user_id']}&amp;hash=" . generate_link_hash('switchperm')) : '',
			...
			case 'feedback':
				...
				if (($deletemark || $deleteall) && phpbb::$auth->acl_get('a_clearlogs'))
				{
					if (!check_form_key($form_name))
					{
						trigger_error(phpbb::$user->lang['FORM_INVALID'] . adm_back_link($this->u_action . '&amp;u=' . $user_id), E_USER_WARNING);
				...
				phpbb::$template->assign_vars(array(
					...
					'S_CLEARLOGS'	=> phpbb::$auth->acl_get('a_clearlogs'))
				);
			...
			case 'warnings':
				...
				if ($deletemark || $deleteall || $confirm)
				{
					if (confirm_box(true))
					{
						...
						if ($deletemark && $marked)
						{
							$where_sql = ' AND ' . phpbb::$db->sql_in_set('warning_id', array_values($marked));
						}
						...
						if ($where_sql || $deleteall)
						{
							...
							{
								$num_warnings = (int) phpbb::$db->sql_affectedrows();
				...
				while ($row = phpbb::$db->sql_fetchrow($result))
				{
					if (!$row['log_operation'])
					{
						...
						$row['action'] = phpbb::$user->lang['USER_WARNING_LOG_DELETED'];
					}
					else
					{
						$row['action'] = (isset(phpbb::$user->lang[$row['log_operation']])) ? phpbb::$user->lang[$row['log_operation']] : '{' . ucfirst(str_replace('_', ' ', $row['log_operation'])) . '}';
					...
					phpbb::$template->assign_block_vars('warn', array(
						'ID'		=> $row['warning_id'],
						...
						'DATE'		=> phpbb::$user->format_date($row['warning_time']),
					));
				}
				phpbb::$db->sql_freeresult($result);
						...
						if ($user_row['session_id'])
						{
								...
								|| $user_row['user_allow_viewonline'] && !$sql_ary['user_allow_viewonline'])
							{
								...
								$sql = 'UPDATE ' . SESSIONS_TABLE . '
									SET ' . phpbb::$db->sql_build_array('UPDATE', $session_sql_ary) . "
									WHERE session_user_id = $user_id";
								phpbb::$db->sql_query($sql);
			...
			case 'avatar':
				...
				$can_upload = (file_exists(phpbb::$phpbb_root_path . phpbb::$config['avatar_path']) && phpbb_is_writable(phpbb::$phpbb_root_path . phpbb::$config['avatar_path']) && $file_uploads) ? true : false;
				...
				if (!phpbb::$config['allow_avatar'] && $user_row['user_avatar_type'])
				{
					$error[] = phpbb::$user->lang['USER_AVATAR_NOT_ALLOWED'];
				}
				else if ((($user_row['user_avatar_type'] == AVATAR_UPLOAD) && !phpbb::$config['allow_avatar_upload']) ||
				 (($user_row['user_avatar_type'] == AVATAR_REMOTE) && !phpbb::$config['allow_avatar_remote']) ||
				 (($user_row['user_avatar_type'] == AVATAR_GALLERY) && !phpbb::$config['allow_avatar_local']))
				{
					$error[] = phpbb::$user->lang['USER_AVATAR_TYPE_NOT_ALLOWED'];
				}
				...
				phpbb::$template->assign_vars(array(
					...
					'S_UPLOAD_FILE'		=> (phpbb::$config['allow_avatar'] && $can_upload && phpbb::$config['allow_avatar_upload']) ? true : false,
					'S_REMOTE_UPLOAD'	=> (phpbb::$config['allow_avatar'] && $can_upload && phpbb::$config['allow_avatar_remote_upload']) ? true : false,
					'S_ALLOW_REMOTE'	=> (phpbb::$config['allow_avatar'] && phpbb::$config['allow_avatar_remote']) ? true : false,
					'S_DISPLAY_GALLERY'	=> (phpbb::$config['allow_avatar'] && phpbb::$config['allow_avatar_local'] && !$display_gallery) ? true : false,
					'S_IN_GALLERY'		=> (phpbb::$config['allow_avatar'] && phpbb::$config['allow_avatar_local'] && $display_gallery) ? true : false,
			...
			case 'sig':
				...
				include_once(phpbb::$phpbb_root_path . 'system/includes/functions_posting.' . phpbb::$phpEx);
				include_once(phpbb::$phpbb_root_path . 'system/includes/functions_display.' . phpbb::$phpEx);

				$enable_bbcode	= (phpbb::$config['allow_sig_bbcode']) ? (bool) $this->optionget($user_row, 'sig_bbcode') : false;
				$enable_smilies	= (phpbb::$config['allow_sig_smilies']) ? (bool) $this->optionget($user_row, 'sig_smilies') : false;
				$enable_urls	= (phpbb::$config['allow_sig_links']) ? (bool) $this->optionget($user_row, 'sig_links') : false;
				...
				if ($submit || $preview)
				{
					include_once(phpbb::$phpbb_root_path . 'system/includes/message_parser.' . phpbb::$phpEx);
			...
			case 'attach':
				...
				while ($row = phpbb::$db->sql_fetchrow($result))
				{
					if ($row['in_message'])
					{
						$view_topic = append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, "i=pm&amp;p={$row['post_msg_id']}");
					}
					else
					{
						$view_topic = append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "t={$row['topic_id']}&amp;p={$row['post_msg_id']}") . '#p' . $row['post_msg_id'];
					}
					...
					phpbb::$template->assign_block_vars('attach', array(
						...
						'U_DOWNLOAD'		=> append_sid(phpbb::$phpbb_root_path . "download/file." . phpbb::$phpEx, 'mode=view&amp;id=' . $row['attach_id']),
			...
			case 'groups':
				...
				if ($group_id)
				{
					...
					if (phpbb::$user->data['user_type'] != USER_FOUNDER && $founder_manage)
					{
				...
				while ($row = phpbb::$db->sql_fetchrow($result))
				{
					...
					if (phpbb::$user->data['user_type'] != USER_FOUNDER && $row['group_founder_manage'])
					{
						continue;
					}
				...
				foreach ($group_data as $group_type => $data_ary)
				{
					...
					{
						phpbb::$template->assign_block_vars('group', array(
							'U_EDIT_GROUP'		=> append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, "i=groups&amp;mode=manage&amp;action=edit&amp;u=$user_id&amp;g={$data['group_id']}&amp;back_link=acp_users_groups"),
			...
			case 'perm':
				...
				include_once(phpbb::$phpbb_root_path . '/system/modules/acp/auth.' . phpbb::$phpEx);
				...
				phpbb::$template->assign_vars(array(
					...
					'U_USER_FORUM_PERMISSIONS'	=> append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=permissions&amp;mode=setting_user_local&amp;user_id[]=' . $user_id))
				);
