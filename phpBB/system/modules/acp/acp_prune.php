	...
	*/
	function prune_users($id, $mode)
	{
			...
			else
			{
				...
				foreach ($user_ids as $user_id)
				{
					phpbb::$template->assign_block_vars('users', array(
						...
						'U_PROFILE'			=> append_sid(phpbb::$phpbb_root_path . 'memberlist.' . $phpEx, 'mode=viewprofile&amp;u=' . $user_id),
						'U_USER_ADMIN'		=> (phpbb::$auth->acl_get('a_user')) ? append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=users&amp;mode=overview&amp;u=' . $user_id, true, phpbb::$user->session_id) : '',
					));
				}
		...
		phpbb::$template->assign_vars(array(
			...
			'U_FIND_USERNAME'	=> append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=searchuser&amp;form=acp_prune&amp;field=users'),
		));
	}
