...
class acp_icons
{
	...
	function main($id, $mode)
	{
		...
		if ($action == 'edit' || $action == 'add' || $action == 'import')
		{
			...
			foreach ($imglist as $path => $img_ary)
			{
				...
				{
					$img_size = getimagesize(phpbb::$phpbb_root_path . $img_path . '/' . $path . $img);
			...
			if ($dir = @opendir(phpbb::$phpbb_root_path . $img_path))
			{
				while (($file = readdir($dir)) !== false)
				{
					if (is_file(phpbb::$phpbb_root_path . $img_path . '/' . $file) && preg_match('#\.pak$#i', $file))
					{
						$_paks[] = $file;
					}
				}
		...
		switch ($action)
		{
			...
			case 'add':
				...
				foreach ($data as $img => $img_row)
				{
					phpbb::$template->assign_block_vars('items', array(
						...
						'IMG_SRC'	=> phpbb::$phpbb_root_path . $img_path . '/' . $img,
				...
				if ($action == 'add' && $mode == 'smilies' && sizeof($smilies))
				{
					phpbb::$template->assign_vars(array(
						...
						'IMG_SRC'			=> phpbb::$phpbb_root_path . $img_path . '/' . $default_row['smiley_url'],
						'IMG_PATH'			=> $img_path,
			...
			case 'modify':
				...
				if ($mode == 'smilies' && $action == 'create')
				{
					...
					{
						trigger_error(sprintf(phpbb::$user->lang['TOO_MANY_SMILIES'], SMILEY_LIMIT) . adm_back_link($this->u_action), E_USER_WARNING);
					}
				}
				...
				foreach ($images as $image)
				{
					...
					else if (!file_exists(phpbb::$phpbb_root_path . $img_path . '/' . $image))
					{
					...
					else
					{
						...
						{
							$img_size = getimagesize(phpbb::$phpbb_root_path . $img_path . '/' . $image);
				...
				phpbb::$cache->destroy('_icons');
				phpbb::$cache->destroy('sql', $table);
			...
			case 'import':
				...
				if ($pak != '')
				{
					...
					if (!($pak_ary = @file(phpbb::$phpbb_root_path . $img_path . '/' . $pak)))
					{
		...
		$result = phpbb::$db->sql_query_limit($sql, phpbb::$config['smilies_per_page'], $pagination_start);
		...
		while ($row = phpbb::$db->sql_fetchrow($result))
		{
			...
			phpbb::$template->assign_block_vars('items', array(
				...
				'ALT_TEXT'		=> $alt_text,
				'IMG_SRC'		=> phpbb::$phpbb_root_path . $img_path . '/' . $row[$fields . '_url'],
		...
		phpbb::$template->assign_var('PAGINATION',
			generate_pagination($this->u_action, $item_count, phpbb::$config['smilies_per_page'], $pagination_start, true)
		);
	}
	...
	/* private */ function item_count($table)
	{
		...
		$sql = "SELECT COUNT(*) AS item_count
			FROM $table";
		$result = phpbb::$db->sql_query($sql);
		$item_count = (int) phpbb::$db->sql_fetchfield('item_count');
		phpbb::$db->sql_freeresult($result);
