...
* @ignore
*/
include_once(phpbb::$phpbb_root_path . 'system/search/search.' . phpbb::$phpEx);
	...
	*/
	function fulltext_native(&$error)
	{
		...
		*/
		...
		{
			include(phpbb::$phpbb_root_path . 'system/utf/utf_normalizer.' . phpbb::$phpEx);
		}
	...
	function split_keywords($keywords, $terms)
	{
		...
		foreach ($query as $word)
		{
			...
			if (is_array($word))
			{
				...
				{
					trigger_error(sprintf(phpbb::$user->lang['WORDS_IN_NO_POST'], implode(', ', $non_common_words)));
				}
			...
			else if (!$ignore_no_id)
			{
				if (!isset($common_ids[$word]))
				{
					...
					if ($len >= $this->word_length['min'] && $len <= $this->word_length['max'])
					{
						trigger_error(sprintf(phpbb::$user->lang['WORD_IN_NO_POST'], $word));
					}
					else
					{
						$this->common_words[] = $word;
		...
		*/
		...
		if (sizeof($author_ary))
		{
			if ($author_name)
			{
				...
				$sql_author = '(' . phpbb::$db->sql_in_set('p.poster_id', array_diff($author_ary, array(ANONYMOUS)), false, true) . ' OR p.post_username ' . $author_name . ')';
			}
			else
			{
...
//					$select = 'SQL_CALC_FOUND_ROWS ' . $select;
					...
					else
					{
						...
						$sql .= ' FROM ' . TOPICS_TABLE . ' t, ' . POSTS_TABLE . " p
							WHERE $sql_author
								...
								$sql_time" . ((phpbb::$db->sql_layer == 'sqlite') ? ')' : '');
					}
					$result = phpbb::$db->sql_query($sql);
	...
	*/
	function index_stats()
	{
		...
		return array(
			phpbb::$user->lang['TOTAL_WORDS']		=> $this->stats['total_words'],
			phpbb::$user->lang['TOTAL_MATCHES']	=> $this->stats['total_matches']);
	}
		...
		*/
		...
		do
		{
			...
			*/
			...
			{
				$conv_loaded[$idx] = 1;
				$file = phpbb::$phpbb_root_path . 'system/utf/data/search_indexer_' . $idx . '.' . $phpEx;
	...
	*/
	function acp()
	{
		...
		*/
		...
		$tpl = '
		<dl>
			<dt><label for="fulltext_native_load_upd">' . phpbb::$user->lang['YES_SEARCH_UPDATE'] . ':</label><br /><span>' . phpbb::$user->lang['YES_SEARCH_UPDATE_EXPLAIN'] . '</span></dt>
			<dd><label><input type="radio" id="fulltext_native_load_upd" name="config[fulltext_native_load_upd]" value="1"' . ((phpbb::$config['fulltext_native_load_upd']) ? ' checked="checked"' : '') . ' class="radio" /> ' . phpbb::$user->lang['YES'] . '</label><label><input type="radio" name="config[fulltext_native_load_upd]" value="0"' . ((!phpbb::$config['fulltext_native_load_upd']) ? ' checked="checked"' : '') . ' class="radio" /> ' . phpbb::$user->lang['NO'] . '</label></dd>
		</dl>
		<dl>
			<dt><label for="fulltext_native_min_chars">' . phpbb::$user->lang['MIN_SEARCH_CHARS'] . ':</label><br /><span>' . phpbb::$user->lang['MIN_SEARCH_CHARS_EXPLAIN'] . '</span></dt>
		...
		<dl>
			<dt><label for="fulltext_native_max_chars">' . phpbb::$user->lang['MAX_SEARCH_CHARS'] . ':</label><br /><span>' . phpbb::$user->lang['MAX_SEARCH_CHARS_EXPLAIN'] . '</span></dt>
		...
		<dl>
			<dt><label for="fulltext_native_common_thres">' . phpbb::$user->lang['COMMON_WORD_THRESHOLD'] . ':</label><br /><span>' . phpbb::$user->lang['COMMON_WORD_THRESHOLD_EXPLAIN'] . '</span></dt>
			<dd><input id="fulltext_native_common_thres" type="text" size="3" maxlength="3" name="config[fulltext_native_common_thres]" value="' . (double) phpbb::$config['fulltext_native_common_thres'] . '" /> %</dd>
		</dl>
		';
