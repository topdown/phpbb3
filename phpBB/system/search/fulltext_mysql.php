...
class fulltext_mysql extends search_backend
{
	...
	*/
	function init()
	{
		...
		if (phpbb::$db->sql_layer != 'mysql4' && phpbb::$db->sql_layer != 'mysqli')
		{
			return phpbb::$user->lang['FULLTEXT_MYSQL_INCOMPATIBLE_VERSION'];
		}
	...
	*/
	function keyword_search($type, $fields, $terms, $sort_by_sql, $sort_key, $sort_dir, $sort_days, $ex_fid_ary, $m_approve_fid_ary, $topic_id, $author_ary, $author_name, &$id_ary, $start, $per_page)
	{
		...
		if (sizeof($author_ary) && $author_name)
		{
			...
			$sql_author = ' AND (' . phpbb::$db->sql_in_set('p.poster_id', array_diff($author_ary, array(ANONYMOUS)), false, true) . ' OR p.post_username ' . $author_name . ')';
		}
		else if (sizeof($author_ary))
		{
			$sql_author = ' AND ' . phpbb::$db->sql_in_set('p.poster_id', $author_ary);
		}
		else
		{
			$sql_author = '';
	...
	*/
	function create_index($acp_module, $u_action)
	{
		...
		{
			if (phpbb::$db->sql_layer == 'mysqli' || version_compare(phpbb::$db->sql_server_info(true), '4.1.3', '>='))
			{
	...
	*/
	...
	function get_stats()
	{
		...
		if (strpos(phpbb::$db->sql_layer, 'mysql') === false)
		{
			$this->stats = array();
			return;
		}
	...
	*/
	function acp()
	{
		...
		$tpl = '
		<dl>
			<dt><label>' . phpbb::$user->lang['FULLTEXT_MYSQL_PCRE'] . '</label><br /><span>' . phpbb::$user->lang['FULLTEXT_MYSQL_PCRE_EXPLAIN'] . '</span></dt>
			<dd>' . (($this->pcre_properties) ? phpbb::$user->lang['YES'] : phpbb::$user->lang['NO']) . ' (PHP ' . PHP_VERSION . ')</dd>
		</dl>
		<dl>
			<dt><label>' . phpbb::$user->lang['FULLTEXT_MYSQL_MBSTRING'] . '</label><br /><span>' . phpbb::$user->lang['FULLTEXT_MYSQL_MBSTRING_EXPLAIN'] . '</span></dt>
			<dd>' . (($this->mbstring_regex) ? phpbb::$user->lang['YES'] : phpbb::$user->lang['NO']). '</dd>
		</dl>
		<dl>
			<dt><label>' . phpbb::$user->lang['MIN_SEARCH_CHARS'] . ':</label><br /><span>' . phpbb::$user->lang['FULLTEXT_MYSQL_MIN_SEARCH_CHARS_EXPLAIN'] . '</span></dt>
			<dd>' . phpbb::$config['fulltext_mysql_min_word_len'] . '</dd>
		</dl>
		<dl>
			<dt><label>' . phpbb::$user->lang['MAX_SEARCH_CHARS'] . ':</label><br /><span>' . phpbb::$user->lang['FULLTEXT_MYSQL_MAX_SEARCH_CHARS_EXPLAIN'] . '</span></dt>
			<dd>' . phpbb::$config['fulltext_mysql_max_word_len'] . '</dd>
		</dl>
		';
