...
class search_backend
{
	...
	*/
	function get_ignore_words()
	{
		if (!sizeof($this->ignore_words))
		{
			...
			if (file_exists("{phpbb::$user->lang_path}{phpbb::$user->lang_name}/search_ignore_words.$phpEx"))
			{
				// include the file containing ignore words
				include("{phpbb::$user->lang_path}{phpbb::$user->lang_name}/search_ignore_words.$phpEx");
			}
	...
	*/
	function get_synonyms()
	{
		if (!sizeof($this->match_synonym))
		{
			...
			if (file_exists("{phpbb::$user->lang_path}{phpbb::$user->lang_name}/search_synonyms.$phpEx"))
			{
				// include the file containing synonyms
				include("{phpbb::$user->lang_path}{phpbb::$user->lang_name}/search_synonyms.$phpEx");
			}
	...
	*/
	function obtain_ids($search_key, &$result_count, &$id_ary, $start, $per_page, $sort_dir)
	{
		...
		if (!($stored_ids = phpbb::$cache->get('_search_results_' . $search_key)))
		{
	...
	*/
	function save_ids($search_key, $keywords, $author_ary, $result_count, &$id_ary, $start, $sort_dir)
	{
		...
		if (!($store = phpbb::$cache->get('_search_results_' . $search_key)))
		{
		...
		if (is_array($store_ids))
		{
			...
			phpbb::$cache->put('_search_results_' . $search_key, $store, phpbb::$config['search_store_results']);
	...
	*/
	function destroy_cache($words, $authors = false)
	{
		...
		if (sizeof($words))
		{
			...
			{
				phpbb::$cache->destroy('_search_results_' . $row['search_key']);
			}
			phpbb::$db->sql_freeresult($result);
		}
