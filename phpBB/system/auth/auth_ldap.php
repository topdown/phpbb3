...
function init_ldap()
{
	...
	$search = @ldap_search(
		$ldap,
		htmlspecialchars_decode(phpbb::$config['ldap_base_dn']),
		ldap_user_filter(phpbb::$user->data['username']),
		(empty(phpbb::$config['ldap_email'])) ?
			array(htmlspecialchars_decode(phpbb::$config['ldap_uid'])) :
			array(htmlspecialchars_decode(phpbb::$config['ldap_uid']), htmlspecialchars_decode(phpbb::$config['ldap_email'])),
		0,
		1
	);
	...
	if ($search === false)
	{
		return phpbb::$user->lang['LDAP_SEARCH_FAILED'];
	}
	...
	if (!empty(phpbb::$config['ldap_email']) && !isset($result[0][htmlspecialchars_decode(phpbb::$config['ldap_email'])]))
	{
	...
	if (is_array($ldap_result) && sizeof($ldap_result) > 1)
	{
		if (@ldap_bind($ldap, $ldap_result[0]['dn'], htmlspecialchars_decode($password)))
		{
			...
			else
			{
				...
				$sql = 'SELECT group_id
					FROM ' . GROUPS_TABLE . "
					WHERE group_name = '" . phpbb::$db->sql_escape('REGISTERED') . "'
				...
				$ldap_user_row = array(
					...
					'user_email'	=> (!empty(phpbb::$config['ldap_email'])) ? utf8_htmlspecialchars($ldap_result[0][htmlspecialchars_decode(phpbb::$config['ldap_email'])][0]) : '',
					'group_id'		=> (int) $row['group_id'],
					'user_type'		=> USER_NORMAL,
					'user_ip'		=> phpbb::$user->ip,
					'user_new'		=> (phpbb::$config['new_member_post_limit']) ? 1 : 0,
				);
...
function ldap_user_filter($username)
{
	...
	{
		$_filter = (phpbb::$config['ldap_user_filter'][0] == '(' && substr(phpbb::$config['ldap_user_filter'], -1) == ')') ? phpbb::$config['ldap_user_filter'] : phpbb::$config['ldap_user_filter'];
