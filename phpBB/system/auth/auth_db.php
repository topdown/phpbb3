...
function login_db($username, $password, $ip = '', $browser = '', $forwarded_for = '')
{
	...
	$sql = 'SELECT user_id, username, user_password, user_passchg, user_pass_convert, user_email, user_type, user_login_attempts
		FROM ' . USERS_TABLE . "
		WHERE username_clean = '" . phpbb::$db->sql_escape($username_clean) . "'";
	$result = phpbb::$db->sql_query($sql);
	$row = phpbb::$db->sql_fetchrow($result);
	phpbb::$db->sql_freeresult($result);

	if (($ip && !phpbb::$config['ip_login_limit_use_forwarded']) ||
		($forwarded_for && phpbb::$config['ip_login_limit_use_forwarded']))
	{
		$sql = 'SELECT COUNT(*) AS attempts
			FROM ' . LOGIN_ATTEMPT_TABLE . '
			WHERE attempt_time > ' . (time() - (int) phpbb::$config['ip_login_limit_time']);
		if (phpbb::$config['ip_login_limit_use_forwarded'])
		{
			$sql .= " AND attempt_forwarded_for = '" . phpbb::$db->sql_escape($forwarded_for) . "'";
		}
		else
		{
			$sql .= " AND attempt_ip = '" . phpbb::$db->sql_escape($ip) . "' ";
		}
		...
		$sql = 'INSERT INTO ' . LOGIN_ATTEMPT_TABLE . phpbb::$db->sql_build_array('INSERT', $attempt_data);
		$result = phpbb::$db->sql_query($sql);
	}
	else
	{
	...
	$show_captcha = (phpbb::$config['max_login_attempts'] && $row['user_login_attempts'] >= phpbb::$config['max_login_attempts']) ||
		(phpbb::$config['ip_login_limit_max'] && $attempts >= phpbb::$config['ip_login_limit_max']);
	...
	if ($row['user_pass_convert'])
	{
		...
		if ($password == $password_new_format)
		{
			if (!function_exists('utf8_to_cp1252'))
			{
				...
				include(phpbb::$phpbb_root_path . 'system/utf/data/recode_basic.' . phpbb::$phpEx);
			}
