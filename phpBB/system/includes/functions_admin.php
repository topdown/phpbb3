...
function recalc_nested_sets(&$new_id, $pkey, $table, $parent_id = 0, $where = array())
{
	...
	while ($row = phpbb::$db->sql_fetchrow($result))
	{
		...
		{
			phpbb::$db->sql_query('UPDATE ' . $table . ' SET ' . phpbb::$db->sql_build_array('UPDATE', array('left_id' => $new_id)) . " WHERE $pkey = {$row[$pkey]}");
		}
		$new_id++;
		...
		{
			phpbb::$db->sql_query('UPDATE ' . $table . ' SET ' . phpbb::$db->sql_build_array('UPDATE', array('right_id' => $new_id)) . " WHERE $pkey = {$row[$pkey]}");
		}
		$new_id++;
	}
...
function make_forum_select($select_id = false, $ignore_id = false, $ignore_acl = false, $ignore_nonpost = false, $ignore_emptycat = true, $only_acl_post = false, $return_array = false)
{
	...
	while ($row = phpbb::$db->sql_fetchrow($result))
	{
		...
		if (!$ignore_acl && phpbb::$auth->acl_gets(array('f_list', 'a_forum', 'a_forumadd', 'a_forumdel'), $row['forum_id']))
		{
			if ($only_acl_post && !phpbb::$auth->acl_get('f_post', $row['forum_id']) || (!phpbb::$auth->acl_get('m_approve', $row['forum_id']) && !phpbb::$auth->acl_get('f_noapprove', $row['forum_id'])))
			{
				$disabled = true;
			}
		}
...
function get_forum_list($acl_list = 'f_list', $id_only = true, $postable_only = false, $no_cache = false)
{
	...
	foreach ($forum_rows as $row)
	{
		...
		if ($acl_list == '' || ($acl_list != '' && phpbb::$auth->acl_gets($acl_list, $row['forum_id'])))
		{
...
function copy_forum_permissions($src_forum_id, $dest_forum_ids, $clear_dest_perms = true, $add_log = true)
{
	...
	$result = phpbb::$db->sql_query($sql);
	$src_forum_name = phpbb::$db->sql_fetchfield('forum_name');
	phpbb::$db->sql_freeresult($result);
	...
	$sql = 'SELECT forum_id, forum_name
		FROM ' . FORUMS_TABLE . '
		WHERE ' . phpbb::$db->sql_in_set('forum_id', $dest_forum_ids);
	$result = phpbb::$db->sql_query($sql);
...
function delete_attachments($mode, $ids, $resync = true)
{
	...
	if (sizeof($message_ids))
	{
		...
		$sql = 'SELECT post_msg_id
			FROM ' . ATTACHMENTS_TABLE . '
			WHERE ' . phpbb::$db->sql_in_set('post_msg_id', $message_ids) . '
				AND in_message = 1
				AND is_orphan = 0';
		$result = phpbb::$db->sql_query($sql);
...
function phpbb_unlink($filename, $mode = 'file', $entry_removed = false)
{
	...
	$sql = 'SELECT COUNT(attach_id) AS num_entries
		FROM ' . ATTACHMENTS_TABLE . "
		WHERE physical_filename = '" . phpbb::$db->sql_escape(utf8_basename($filename)) . "'";
	$result = phpbb::$db->sql_query($sql);
	...
	return @unlink(phpbb::$phpbb_root_path . phpbb::$config['upload_path'] . '/' . $filename);
}
...
function cache_moderators()
{
	...
	$hold_ary = phpbb::$auth->acl_user_raw_data(false, 'm_%', false);
	...
	// Now to the groups...
	$hold_ary = phpbb::$auth->acl_group_raw_data(false, 'm_%', false);
...
function view_log($mode, &$log, &$log_count, $limit = 0, $offset = 0, $forum_id = 0, $topic_id = 0, $user_id = 0, $limit_days = 0, $sort_by = 'l.log_time DESC', $keywords = '')
{
	...
	$profile_url = (defined('IN_ADMIN')) ? append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, 'i=users&amp;mode=overview') : append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=viewprofile');
	...
	if (!empty($keywords))
	{
		...
		for ($i = 0, $num_keywords = sizeof($keywords); $i < $num_keywords; $i++)
		{
			...
			$keywords[$i] = phpbb::$db->sql_like_expression(phpbb::$db->any_char . $keywords[$i] . phpbb::$db->any_char);
		}

		$keywords_pattern = '#' . implode('|', $keywords_pattern) . '#ui';

		$operations = array();
		foreach (phpbb::$user->lang as $key => $value)
		{
		...
		if (!empty($operations))
		{
			$sql_keywords .= phpbb::$db->sql_in_set('l.log_operation', $operations) . ' OR ';
		}
	...
	while ($row = phpbb::$db->sql_fetchrow($result))
	{
		...
		$log[$i] = array(
			...
			'viewforum'			=> ($row['forum_id'] && phpbb::$auth->acl_get('f_read', $row['forum_id'])) ? append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $row['forum_id']) : false,
	...
	if (sizeof($topic_id_list))
	{
		...
		while ($row = phpbb::$db->sql_fetchrow($result))
		{
			if (!$row['forum_id'])
			{
				if (phpbb::$auth->acl_getf_global('f_read'))
				{
					if (!$default_forum_id)
					{
						...
						$default_forum_id = (int) phpbb::$db->sql_fetchfield('forum_id', false, $f_result);
			...
			if (phpbb::$auth->acl_gets('a_', 'm_', $row['forum_id']))
			{
		...
		{
			$log[$key]['viewtopic'] = (isset($is_auth[$row['topic_id']])) ? append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $is_auth[$row['topic_id']] . '&amp;t=' . $row['topic_id']) : false;
			$log[$key]['viewlogs'] = (isset($is_mod[$row['topic_id']])) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=logs&amp;mode=topic_logs&amp;t=' . $row['topic_id'], true, phpbb::$user->session_id) : false;
		}
	}
...
function update_foes($group_id = false, $user_id = false)
{
	...
	if (is_array($group_id) && sizeof($group_id))
	{
		...
		switch (phpbb::$db->sql_layer)
		{
			case 'mysqli':
			case 'mysql4':
				$sql = 'DELETE ' . ((phpbb::$db->sql_layer === 'mysqli' || version_compare(phpbb::$db->sql_server_info(true), '4.1', '>=')) ? 'z.*' : ZEBRA_TABLE) . '
	...
	$perms = array();
	foreach (phpbb::$auth->acl_get_list(false, array('a_', 'm_'), false) as $forum_id => $forum_ary)
	{
...
function add_permission_language()
{
	...
	foreach (array('acp/', 'mods/') as $path)
	{
		...
		if ($dh)
		{
			...
			{
				if ($file !== 'permissions_phpbb.' . $phpEx && strpos($file, 'permissions_') === 0 && substr($file, -(strlen(phpbb::$phpEx) + 1)) === '.' . phpbb::$phpEx)
				{
					$files_to_add[] = $path . substr($file, 0, -(strlen(phpbb::$phpEx) + 1));
				}
			}
			closedir($dh);
		}
...
function obtain_latest_version_info($force_update = false, $warn_fail = false, $ttl = 86400)
{
	...
	$info = phpbb::$cache->get('versioncheck');
	...
	if ($info === false || $force_update)
	{
		...
		if ($info === false)
		{
			phpbb::$cache->destroy('versioncheck');
			if ($warn_fail)
			{
		...
		phpbb::$cache->put('versioncheck', $info, $ttl);
	}
...
function enable_bitfield_column_flag($table_name, $column_name, $flag, $sql_more = '')
{
	...
	$sql = 'UPDATE ' . $table_name . '
		SET ' . $column_name . ' = ' . phpbb::$db->sql_bit_or($column_name, $flag) . '
		' . $sql_more;
	phpbb::$db->sql_query($sql);
}
