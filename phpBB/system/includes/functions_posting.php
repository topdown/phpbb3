...
function generate_smilies($mode, $forum_id)
{
	...
	if ($mode == 'window')
	{
		...
		phpbb::$template->assign_var('PAGINATION',
			generate_pagination(append_sid(phpbb::$phpbb_root_path . "posting." . phpbb::$phpEx, 'mode=smilies&amp;f=' . $forum_id),
				$smiley_count, phpbb::$config['smilies_per_page'], $start, true)
		);
	}
	...
	if ($mode == 'window')
	{
		...
		$result = phpbb::$db->sql_query_limit($sql, phpbb::$config['smilies_per_page'], $start, 3600);
	}
	else
	{
		$sql = 'SELECT *
	...
	if (sizeof($smilies))
	{
		$root_path = (defined('PHPBB_USE_BOARD_URL_PATH') && PHPBB_USE_BOARD_URL_PATH) ? generate_board_url() . '/' : phpbb::$phpbb_root_path;
		...
		foreach ($smilies as $row)
		{
			phpbb::$template->assign_block_vars('smiley', array(
				...
				'SMILEY_IMG'	=> $root_path . phpbb::$config['smilies_path'] . '/' . $row['smiley_url'],
	...
	if ($mode == 'inline' && $display_link)
	{
		phpbb::$template->assign_vars(array(
			...
			'U_MORE_SMILIES' 		=> append_sid(phpbb::$phpbb_root_path . "posting." . phpbb::$phpEx, 'mode=smilies&amp;f=' . $forum_id))
		);
	}
		...
		foreach ($icons as $id => $data)
		{
			if ($data['display'])
			{
				phpbb::$template->assign_block_vars('topic_icon', array(
					'ICON_ID'		=> $id,
					'ICON_IMG'		=> phpbb::$phpbb_root_path . phpbb::$config['icons_path'] . '/' . $data['img'],
...
function posting_gen_topic_types($forum_id, $cur_topic_type = POST_NORMAL)
{
	...
	foreach ($topic_types as $auth_key => $topic_value)
	{
		...
		if (phpbb::$auth->acl_get('f_' . $auth_key, $forum_id))
		{
			$toggle = true;
	...
	if ($toggle)
	{
		...
		phpbb::$template->assign_vars(array(
			'S_TOPIC_TYPE_STICKY'	=> (phpbb::$auth->acl_get('f_sticky', $forum_id)),
			'S_TOPIC_TYPE_ANNOUNCE'	=> (phpbb::$auth->acl_get('f_announce', $forum_id)))
		);
	}
...
function upload_attachment($form_name, $forum_id, $local = false, $local_storage = '', $is_message = false, $local_filedata = false)
{
	...
	include_once(phpbb::$phpbb_root_path . 'system/core/upload.' . phpbb::$phpEx);
	$upload = new fileupload();

	if (phpbb::$config['check_attachment_content'] && isset(phpbb::$config['mime_triggers']))
	{
	...
	$extensions = phpbb::$cache->obtain_attach_extensions((($is_message) ? false : (int) $forum_id));
	...
	if (!phpbb::$auth->acl_get('a_') && !phpbb::$auth->acl_get('m_', $forum_id) && $cat_id == ATTACHMENT_CATEGORY_IMAGE)
	{
		$file->upload->set_allowed_dimensions(0, 0, phpbb::$config['img_max_width'], phpbb::$config['img_max_height']);
	}

	// Admins and mods are allowed to exceed the allowed filesize
	if (!phpbb::$auth->acl_get('a_') && !phpbb::$auth->acl_get('m_', $forum_id))
	{
	...
	if ($free_space = @disk_free_space(phpbb::$phpbb_root_path . phpbb::$config['upload_path']))
	{
...
function posting_gen_attachment_entry($attachment_data, &$filename_data, $show_attach_box = true)
{
	...
	if (sizeof($attachment_data))
	{
		...
		foreach ($attachment_data as $count => $attach_row)
		{
			...
			$download_link = append_sid(phpbb::$phpbb_root_path . "download/file." . phpbb::$phpEx, 'mode=view&amp;id=' . (int) $attach_row['attach_id'], true, ($attach_row['is_orphan']) ? phpbb::$user->session_id : false);
...
function load_drafts($topic_id = 0, $forum_id = 0, $id = 0, $pm_action = '', $msg_id = 0)
{
	...
	foreach ($draft_rows as $draft)
	{
		...
		if (isset($topic_rows[$draft['topic_id']])
			&& (
				($topic_rows[$draft['topic_id']]['forum_id'] && phpbb::$auth->acl_get('f_read', $topic_rows[$draft['topic_id']]['forum_id']))
				||
				(!$topic_rows[$draft['topic_id']]['forum_id'] && phpbb::$auth->acl_getf_global('f_read'))
			))
		{
			$topic_forum_id = ($topic_rows[$draft['topic_id']]['forum_id']) ? $topic_rows[$draft['topic_id']]['forum_id'] : $forum_id;

			$link_topic = true;
			$view_url = append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $topic_forum_id . '&amp;t=' . $draft['topic_id']);
			$title = $topic_rows[$draft['topic_id']]['topic_title'];

			$insert_url = append_sid(phpbb::$phpbb_root_path . "posting." . phpbb::$phpEx, 'f=' . $topic_forum_id . '&amp;t=' . $draft['topic_id'] . '&amp;mode=reply&amp;d=' . $draft['draft_id']);
		}
		else if ($draft['forum_id'] && phpbb::$auth->acl_get('f_read', $draft['forum_id']))
		{
			$link_forum = true;
		...
		else
		{
			...
			$link_pm = true;
			$insert_url = append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, "i=$id&amp;mode=compose&amp;d={$draft['draft_id']}" . (($pm_action) ? "&amp;action=$pm_action" : '') . (($msg_id) ? "&amp;p=$msg_id" : ''));
		}
...
function topic_review($topic_id, $forum_id, $mode = 'topic_review', $cur_post_id = 0, $show_quote_button = true)
{
	...
	if ($has_attachments && phpbb::$auth->acl_get('u_download') && phpbb::$auth->acl_get('f_download', $forum_id))
	{
	...
	for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
	{
		...
		if ($show_quote_button && phpbb::$auth->acl_get('f_reply', $forum_id))
		{
		...
		$u_show_post = append_sid(phpbb::$phpbb_root_path . 'viewtopic.' . phpbb::$phpEx, "f=$forum_id&amp;t=$topic_id&amp;p={$row['post_id']}&amp;view=show#p{$row['post_id']}");
		...
		phpbb::$template->assign_block_vars($mode . '_row', array(
			...
			'L_IGNORE_POST'		=> ($row['foe']) ? sprintf(phpbb::$user->lang['POST_BY_FOE'], get_username_string('full', $poster_id, $row['username'], $row['user_colour'], $row['post_username']), "<a href=\"{$u_show_post}\" onclick=\"dE('{$post_anchor}', 1); return false;\">", '</a>') : '',
			...
			'POST_ID'			=> $row['post_id'],
			'U_MINI_POST'		=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'p=' . $row['post_id']) . '#p' . $row['post_id'],
			'U_MCP_DETAILS'		=> (phpbb::$auth->acl_get('m_info', $forum_id)) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=main&amp;mode=post_details&amp;f=' . $forum_id . '&amp;p=' . $row['post_id'], true, phpbb::$user->session_id) : '',
			'POSTER_QUOTE'		=> ($show_quote_button && phpbb::$auth->acl_get('f_reply', $forum_id)) ? addslashes(get_username_string('username', $poster_id, $row['username'], $row['user_colour'], $row['post_username'])) : '')
		);
	...
	{
		phpbb::$template->assign_var('QUOTE_IMG', phpbb::$user->img('icon_post_quote', phpbb::$user->lang['REPLY_WITH_QUOTE']));
	}
...
function user_notification($mode, $subject, $topic_title, $forum_name, $forum_id, $topic_id, $post_id)
{
	...
	foreach (phpbb::$auth->acl_get_list(array_keys($notify_rows), 'f_read', $forum_id) as $forum_id => $forum_ary)
	{
...
function delete_post($forum_id, $topic_id, $post_id, &$data)
{
	...
	switch ($post_mode)
	{
		...
		case 'delete_first_post':
			...
			$sql_data[TOPICS_TABLE] = 'topic_poster = ' . intval($row['poster_id']) . ', topic_first_post_id = ' . intval($row['post_id']) . ", topic_first_poster_colour = '" . phpbb::$db->sql_escape($row['user_colour']) . "', topic_first_poster_name = '" . (($row['poster_id'] == ANONYMOUS) ? phpbb::$db->sql_escape($row['post_username']) : phpbb::$db->sql_escape($row['username'])) . "', topic_time = " . (int) $row['post_time'];
	...
	if (($post_mode == 'delete') || ($post_mode == 'delete_last_post') || ($post_mode == 'delete_first_post'))
	{
		$sql = 'SELECT 1 AS has_attachments
			...
			WHERE topic_id = ' . $topic_id;
		$result = phpbb::$db->sql_query_limit($sql, 1);
		$has_attachments = (int) phpbb::$db->sql_fetchfield('has_attachments');
		phpbb::$db->sql_freeresult($result);
...
function submit_post($mode, $subject, $username, $topic_type, &$poll, &$data, $update_message = true, $update_search_index = true)
{
	...
	if (!phpbb::$auth->acl_get('f_noapprove', $data['forum_id']) && !phpbb::$auth->acl_get('m_approve', $data['forum_id']))
	{
	...
	switch ($post_mode)
	{
		...
		case 'reply':
			$sql_data[POSTS_TABLE]['sql'] = array(
				...
				'post_username'		=> (!phpbb::$user->data['is_registered']) ? $username : '',
				...
				'post_postcount'	=> (phpbb::$auth->acl_get('f_postcount', $data['forum_id'])) ? 1 : 0,
		...
		case 'edit_topic':
			...
			if ($data['post_edit_reason'] || (!phpbb::$auth->acl_get('m_edit', $data['forum_id']) && ($post_mode == 'edit' || $post_mode == 'edit_first_post')))
			{
			...
			else if (!$data['post_edit_reason'] && $mode == 'edit' && phpbb::$auth->acl_get('m_edit', $data['forum_id']))
			{
				$sql_data[POSTS_TABLE]['sql'] = array(
					'post_edit_reason'	=> '',
				);
	...
	switch ($post_mode)
	{
		case 'post':
			$sql_data[TOPICS_TABLE]['sql'] = array(
				...
				'topic_title'				=> $subject,
				'topic_first_poster_name'	=> (!phpbb::$user->data['is_registered'] && $username) ? $username : ((phpbb::$user->data['user_id'] != ANONYMOUS) ? phpbb::$user->data['username'] : ''),
			...
			$sql_data[USERS_TABLE]['stat'][] = "user_lastpost_time = $current_time" . ((phpbb::$auth->acl_get('f_postcount', $data['forum_id']) && $post_approval) ? ', user_posts = user_posts + 1' : '');
		...
		case 'edit_first_post':
			...
			if (!$post_approval && $data['topic_approved'])
			{
				...
				if (phpbb::$auth->acl_get('f_postcount', $data['forum_id']))
				{
	...
	if ($post_mode == 'post' || $post_mode == 'reply')
	{
		...
		if ($post_mode == 'post')
		{
			$sql_data[TOPICS_TABLE]['sql'] = array(
				...
				'topic_last_poster_name'	=> (!phpbb::$user->data['is_registered'] && $username) ? $username : ((phpbb::$user->data['user_id'] != ANONYMOUS) ? phpbb::$user->data['username'] : ''),
	...
	if (!empty($data['attachment_data']) && $data['post_id'] && in_array($mode, array('post', 'reply', 'quote', 'edit')))
	{
		...
		foreach ($data['attachment_data'] as $pos => $attach_row)
		{
			...
			else
			{
				...
				if (!@file_exists(phpbb::$phpbb_root_path . phpbb::$config['upload_path'] . '/' . utf8_basename($orphan_rows[$attach_row['attach_id']]['physical_filename'])))
				{
					continue;
				}
	...
	if ($topic_type != POST_GLOBAL && !$make_global && ($post_approved || !$data['post_approved']))
	{
		...
		if (($post_mode == 'post' || $post_mode == 'reply') && $post_approved)
		{
			...
			$sql_data[FORUMS_TABLE]['stat'][] = "forum_last_poster_name = '" . phpbb::$db->sql_escape((!phpbb::$user->data['is_registered'] && $username) ? $username : ((phpbb::$user->data['user_id'] != ANONYMOUS) ? phpbb::$user->data['username'] : '')) . "'";
	...
	if ($post_approved)
	{
		...
		if ($post_mode == 'reply')
		{
			...
			$sql_data[TOPICS_TABLE]['stat'][] = "topic_last_poster_name = '" . phpbb::$db->sql_escape((!phpbb::$user->data['is_registered'] && $username) ? $username : ((phpbb::$user->data['user_id'] != ANONYMOUS) ? phpbb::$user->data['username'] : '')) . "'";
			$sql_data[TOPICS_TABLE]['stat'][] = "topic_last_poster_colour = '" . ((phpbb::$user->data['user_id'] != ANONYMOUS) ? phpbb::$db->sql_escape(phpbb::$user->data['user_colour']) : '') . "'";
	...
	if ($update_search_index && $data['enable_indexing'])
	{
		...
		{
			include(phpbb::$phpbb_root_path . "system/search/$search_type." . phpbb::$phpEx);
		}
	...
	if (phpbb::$user->data['user_id'] == $poster_id)
	{
		...
		else if ((phpbb::$config['email_enable'] || phpbb::$config['jab_enable']) && $data['notify_set'] && !$data['notify'])
		{
			$sql = 'DELETE FROM ' . TOPICS_WATCH_TABLE . '
	...
	if ((phpbb::$config['load_db_lastread'] && phpbb::$user->data['is_registered']) || phpbb::$config['load_anon_lastread'] || phpbb::$user->data['is_registered'])
	{
		// Update forum info
		if ($topic_type == POST_GLOBAL)
		{
	...
	$url = (!$params) ? phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx : phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx;
...
function phpbb_bump_topic($forum_id, $topic_id, $post_data, $bump_time = false)
{
	...
	$sql = 'UPDATE ' . FORUMS_TABLE . "
		SET forum_last_post_id = " . $post_data['topic_last_post_id'] . ",
			...
			forum_last_post_subject = '" . phpbb::$db->sql_escape($post_data['topic_last_post_subject']) . "',
			forum_last_post_time = $bump_time,
			forum_last_poster_name = '" . phpbb::$db->sql_escape($post_data['topic_last_poster_name']) . "',
			forum_last_poster_colour = '" . phpbb::$db->sql_escape($post_data['topic_last_poster_colour']) . "'
		WHERE forum_id = $forum_id";
	phpbb::$db->sql_query($sql);
	...
	$url = append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, "f=$forum_id&amp;t=$topic_id&amp;p={$post_data['topic_last_post_id']}") . "#p{$post_data['topic_last_post_id']}";
