...
function connect_check_db($error_connect, &$error, $dbms_details, $table_prefix, $dbhost, $dbuser, $dbpasswd, $dbname, $dbport, $prefix_may_exist = false, $load_dbal = true, $unicode_check = true)
{
	...
	// Try and connect ...
	if (is_array(phpbb::$db->sql_connect($dbhost, $dbuser, $dbpasswd, $dbname, $dbport, false, true)))
	{
		$db_error = phpbb::$db->sql_error();
	...
	else
	{
		...
		{
			case 'mysqli':
				if (version_compare(mysqli_get_server_info(phpbb::$db->db_connect_id), '4.1.3', '<'))
				{
			...
			case 'firebird':
				...
				if (phpbb::$db->service_handle !== false && function_exists('ibase_server_info'))
				{
					$val = @ibase_server_info(phpbb::$db->service_handle, IBASE_SVC_SERVER_VERSION);
					preg_match('#V([\d.]+)#', $val, $match);
					if ($match[1] < 2)
					{
						$error[] = $lang['INST_ERR_DB_NO_FIREBIRD'];
					}
					$db_info = @ibase_db_info(phpbb::$db->service_handle, $dbname, IBASE_STS_HDR_PAGES);
				...
				else
				{
					...
					{
						// Kill the old table
						phpbb::$db->sql_query('DROP TABLE ' . $final . ';');
					}
					unset($final);
				}
			break;
...
function adjust_language_keys_callback($matches)
{
	if (!empty($matches[1]))
	{
		...
		return (!empty($lang[$matches[1]])) ? phpbb::$db->sql_escape($lang[$matches[1]]) : phpbb::$db->sql_escape($matches[1]);
	}
}
