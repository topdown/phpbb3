...
function import_avatar_gallery($gallery_name = '', $subdirs_as_galleries = false)
{
	...
	{
		$convert->p_master->error(sprintf(phpbb::$user->lang['CONV_ERROR_NO_GALLERY_PATH'], 'import_avatar_gallery()'), __LINE__, __FILE__);
	}
...
function import_attachment_files($category_name = '')
{
	...
	$result = phpbb::$db->sql_query($sql);
	phpbb::$config['upload_path'] = phpbb::$db->sql_fetchfield('upload_path');
	phpbb::$db->sql_freeresult($result);
	...
	{
		$convert->p_master->error(sprintf(phpbb::$user->lang['CONV_ERROR_NO_UPLOAD_DIR'], 'import_attachment_files()'), __LINE__, __FILE__);
	}
...
function _import_check($config_var, $source, $use_target)
{
	...
	$target = phpbb::$config[$config_var] . '/' . utf8_basename(($use_target === false) ? $source : $use_target);
...
function import_attachment($source, $use_target = false)
{
	...
	if (empty($convert->convertor['upload_path']))
	{
		$convert->p_master->error(sprintf(phpbb::$user->lang['CONV_ERROR_NO_UPLOAD_DIR'], 'import_attachment()'), __LINE__, __FILE__);
	}
...
function import_rank($source, $use_target = false)
{
	...
	{
		$convert->p_master->error(sprintf(phpbb::$user->lang['CONV_ERROR_NO_RANKS_PATH'], 'import_rank()'), __LINE__, __FILE__);
	}
...
function import_smiley($source, $use_target = false)
{
	...
	{
		$convert->p_master->error(sprintf(phpbb::$user->lang['CONV_ERROR_NO_SMILIES_PATH'], 'import_smiley()'), __LINE__, __FILE__);
	}
...
function import_avatar($source, $use_target = false, $user_id = false)
{
	...
	{
		$convert->p_master->error(sprintf(phpbb::$user->lang['CONV_ERROR_NO_AVATAR_PATH'], 'import_avatar()'), __LINE__, __FILE__);
	}
...
function get_smiley_dim($source, $axis)
{
	...
	if (!isset($convert->convertor['smilies_path']))
	{
		$convert->p_master->error(sprintf(phpbb::$user->lang['CONV_ERROR_NO_SMILIES_PATH'], 'get_smiley_dim()'), __LINE__, __FILE__);
	}
...
function get_upload_avatar_dim($source, $axis)
{
	...
	if (!isset($convert->convertor['avatar_path']))
	{
		$convert->p_master->error(sprintf(phpbb::$user->lang['CONV_ERROR_NO_AVATAR_PATH'], 'get_upload_avatar_dim()'), __LINE__, __FILE__);
	}
...
function get_gallery_avatar_dim($source, $axis)
{
	...
	{
		$convert->p_master->error(sprintf(phpbb::$user->lang['CONV_ERROR_NO_GALLERY_PATH'], 'get_gallery_avatar_dim()'), __LINE__, __FILE__);
	}
...
function user_group_auth($group, $select_query, $use_src_db)
{
	...
	{
		$convert->p_master->error(sprintf(phpbb::$user->lang['CONV_ERROR_WRONG_GROUP'], $group, 'user_group_auth()'), __LINE__, __FILE__, true);
		return;
	}

	$sql = 'SELECT group_id
		FROM ' . GROUPS_TABLE . "
		WHERE group_name = '" . phpbb::$db->sql_escape(strtoupper($group)) . "'";
	$result = phpbb::$db->sql_query($sql);
	...
	if (!$group_id)
	{
		$convert->p_master->error(sprintf(phpbb::$user->lang['CONV_ERROR_NO_GROUP'], $group, 'user_group_auth()'), __LINE__, __FILE__, true);
		return;
	}
...
function get_config()
{
	...
	if ($convert->config_schema['table_format'] != 'file')
	{
		...
		if (!$row)
		{
			$convert->p_master->error(phpbb::$user->lang['CONV_ERROR_GET_CONFIG'], __LINE__, __FILE__);
		}
	}
	...
	else if ($convert->config_schema['table_format'] == 'file')
	{
		...
		{
			$convert->p_master->error(phpbb::$user->lang['FILE_NOT_FOUND'] . ': ' . $filename, __LINE__, __FILE__);
		}
	...
	{
		$convert->p_master->error(phpbb::$user->lang['CONV_ERROR_CONFIG_EMPTY'], __LINE__, __FILE__);
	}
...
function update_folder_pm_count()
{
	...
	while ($row = phpbb::$db->sql_fetchrow($result))
	{
		phpbb::$db->sql_query('UPDATE ' . PRIVMSGS_FOLDER_TABLE . ' SET pm_count = ' . $row['num_messages'] . '
...
function get_path($src_path, $src_url, $test_file)
{
	...
	$test_file = preg_replace('/\.php$/i', "." . phpbb::$phpEx, $test_file);
	$src_path = path($src_path);

	if (@file_exists(phpbb::$phpbb_root_path . $src_path . $test_file))
	{
		return $src_path;
	}
	...
	if (!empty($src_url) && !empty($board_config['server_name']))
	{
		...
		if (!empty($path))
		{
			if (@file_exists(phpbb::$phpbb_root_path . $path . $test_file))
			{
				return $path;
			}
		}
...
function mass_auth($ug_type, $forum_id, $ug_id, $acl_list, $setting = ACL_NO)
{
	...
	if (($ug_type == 'group' || $ug_type == 'group_role') && is_string($ug_id))
	{
		if (!isset($group_ids[$ug_id]))
		{
			$sql = 'SELECT group_id
				FROM ' . GROUPS_TABLE . "
				WHERE group_name = '" . phpbb::$db->sql_escape(strtoupper($ug_id)) . "'";
			$result = phpbb::$db->sql_query_limit($sql, 1);
			$id = (int) phpbb::$db->sql_fetchfield('group_id');
			phpbb::$db->sql_freeresult($result);
	...
	if ($ug_type == 'user_role' || $ug_type == 'group_role')
	{
		if (is_numeric($forum_id))
		{
			$sql = 'SELECT role_id
				...
				WHERE role_name = 'ROLE_" . phpbb::$db->sql_escape($acl_list) . "'";
			$result = phpbb::$db->sql_query_limit($sql, 1);
	...
	$sql_forum = 'AND ' . phpbb::$db->sql_in_set('a.forum_id', array_map('intval', $forum_id), false, true);
...
function update_unread_count()
{
	...
	while ($row = phpbb::$db->sql_fetchrow($result))
	{
		phpbb::$db->sql_query('UPDATE ' . USERS_TABLE . ' SET user_unread_privmsg = ' . $row['num_messages'] . '
...
function add_default_groups()
{
	...
	$sql = 'SELECT *
		FROM ' . GROUPS_TABLE . '
		WHERE ' . phpbb::$db->sql_in_set('group_name', array_keys($default_groups));
	$result = phpbb::$db->sql_query($sql);
	...
	if (sizeof($sql_ary))
	{
		phpbb::$db->sql_multi_insert(GROUPS_TABLE, $sql_ary);
	}
}
...
function sync_post_count($offset, $limit)
{
	...
	while ($row = phpbb::$db->sql_fetchrow($result))
	{
		phpbb::$db->sql_query('UPDATE ' . USERS_TABLE . " SET user_posts = {$row['num_posts']} WHERE user_id = {$row['poster_id']}");
	}
	phpbb::$db->sql_freeresult($result);
}
...
function add_bots()
{
	...
	phpbb::$db->sql_query($convert->truncate_statement . BOTS_TABLE);

	$sql = 'SELECT group_id FROM ' . GROUPS_TABLE . " WHERE group_name = 'BOTS'";
	$result = phpbb::$db->sql_query($sql);
	$group_id = (int) phpbb::$db->sql_fetchfield('group_id', false, $result);
	phpbb::$db->sql_freeresult($result);
	...
	if (!$group_id)
	{
		...
		{
			global $install;
			$install->error(phpbb::$user->lang['CONV_ERROR_INCONSISTENT_GROUPS'], __LINE__, __FILE__);
		}
	}
...
function fix_empty_primary_groups()
{
	...
	phpbb::$db->sql_query('UPDATE ' . USERS_TABLE . ' SET group_id = ' . get_group_id('guests') . ' WHERE user_id = ' . ANONYMOUS);
	...
	if (sizeof($user_ids))
	{
		phpbb::$db->sql_query('UPDATE ' . USERS_TABLE . ' SET group_id = ' . get_group_id('administrators') . '
			WHERE group_id = 0 AND ' . phpbb::$db->sql_in_set('user_id', $user_ids));
	}
	...
	if (sizeof($user_ids))
	{
		phpbb::$db->sql_query('UPDATE ' . USERS_TABLE . ' SET group_id = ' . get_group_id('global_moderators') . '
			WHERE group_id = 0 AND ' . phpbb::$db->sql_in_set('user_id', $user_ids));
	}
	...
	while ($row = phpbb::$db->sql_fetchrow($result))
	{
		phpbb::$db->sql_query('UPDATE ' . USERS_TABLE . " SET user_colour = '{$row['group_colour']}' WHERE group_id = {$row['group_id']}");
	}
	phpbb::$db->sql_freeresult($result);
}
...
function copy_file($src, $trg, $overwrite = false, $die_on_failure = true, $source_relative_path = true)
{
	...
	if (!@copy($src_path, phpbb::$phpbb_root_path . $trg_path))
	{
		$convert->p_master->error(sprintf(phpbb::$user->lang['COULD_NOT_COPY'], $src_path, phpbb::$phpbb_root_path . $trg_path), __LINE__, __FILE__, !$die_on_failure);
		return;
	}

	if ($perm = @fileperms($src_path))
	{
		@chmod(phpbb::$phpbb_root_path . $trg_path, $perm);
	}
...
function copy_dir($src, $trg, $copy_subdirs = true, $overwrite = false, $die_on_failure = true, $source_relative_path = true)
{
	...
	$trg = path($trg);
	$src_path = relative_base($src, $source_relative_path, __LINE__, __FILE__);
	$trg_path = phpbb::$phpbb_root_path . $trg;
	...
	{
		$convert->p_master->error(sprintf(phpbb::$user->lang['CONV_ERROR_COULD_NOT_READ'], relative_base($src, $source_relative_path)), __LINE__, __FILE__);
	}
	...
	if (sizeof($bad_dirs))
	{
		$str = (sizeof($bad_dirs) == 1) ? phpbb::$user->lang['MAKE_FOLDER_WRITABLE'] : phpbb::$user->lang['MAKE_FOLDERS_WRITABLE'];
		sort($bad_dirs);
...
function relative_base($path, $is_relative = true, $line = false, $file = false)
{
	...
	if (empty($convert->options['forum_path']) && $is_relative)
	{
		...
		$convert->p_master->error(phpbb::$user->lang['CONV_ERROR_NO_FORUM_PATH'], $line, $file);
	}
