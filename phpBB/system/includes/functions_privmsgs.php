...
function get_folder($user_id, $folder_id = false)
{
	...
	foreach ($folder as $f_id => $folder_ary)
	{
		...
		phpbb::$template->assign_block_vars('folder', array(
			...
			'U_FOLDER'			=> ($f_id > 0) ? append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm&amp;folder=' . $f_id) : append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm&amp;folder=' . $folder_id_name),
		...
		!in_array($cur_folder_id, array(PRIVMSGS_NO_BOX, PRIVMSGS_OUTBOX)) && $cur_folder_id != $dest_folder)
	{
		...
		if ($dest_folder != PRIVMSGS_INBOX)
		{
			...
			if ($message_limit && $row['pm_count'] + sizeof($move_msg_ids) > $message_limit)
			{
				...
				$message .= sprintf(phpbb::$user->lang['CLICK_RETURN_FOLDER'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm&amp;folder=' . $row['folder_id']) . '">', '</a>', $row['folder_name']);
		...
		else
		{
			...
			if ($message_limit && $num_messages + sizeof($move_msg_ids) > $message_limit)
			{
				...
				$message .= sprintf(phpbb::$user->lang['CLICK_RETURN_FOLDER'], '<a href="' . append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm&amp;folder=inbox') . '">', '</a>', phpbb::$user->lang['PM_INBOX']);
				trigger_error($message);
			}
		}
	...
	switch ($mark_action)
	{
		...
		case 'delete_marked':
			...
			if (confirm_box(true))
			{
				...
				$redirect = append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm&amp;folder=' . $cur_folder_id);
...
function submit_pm($mode, $subject, &$data, $put_in_outbox = true)
{
	...
	if ($mode != 'edit')
	{
		...
		if (isset($data['address_list']['g']) && sizeof($data['address_list']['g']))
		{
			...
			$sql_allow_pm = (!phpbb::$auth->acl_gets('a_', 'm_') && !phpbb::$auth->acl_getf_global('m_')) ? ' AND u.user_allow_pm = 1' : '';
...
function message_history($msg_id, $user_id, $message_row, $folder, $in_post_mode = false)
{
	...
	$sql = 'SELECT t.*, p.*, u.*
		...
		WHERE t.msg_id = p.msg_id
			...
			AND ' . phpbb::$db->sql_in_set('t.author_id', $recipients, false, true) . "
	...
	$rowset = array();
	$bbcode_bitfield = '';
	$folder_url = append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=pm') . '&amp;folder=';
	...
	for ($i = 0, $size = sizeof($rowset); $i < $size; $i++)
	{
		...
		if ($in_post_mode && phpbb::$auth->acl_get('u_sendpm') && $author_id != ANONYMOUS)
		{
		...
		phpbb::$template->assign_block_vars('history_row', array(
			...
			'U_QUOTE'			=> (!$in_post_mode && phpbb::$auth->acl_get('u_sendpm') && $author_id != ANONYMOUS) ? "$url&amp;mode=compose&amp;action=quote&amp;f=" . $folder_id . "&amp;p=" . $row['msg_id'] : '',
			'U_POST_REPLY_PM'	=> ($author_id != phpbb::$user->data['user_id'] && $author_id != ANONYMOUS && phpbb::$auth->acl_get('u_sendpm')) ? "$url&amp;mode=compose&amp;action=reply&amp;f=$folder_id&amp;p=" . $row['msg_id'] : '')
		);
		unset($rowset[$i]);
		$prev_id = $id;
	}
	...
	phpbb::$template->assign_vars(array(
		'QUOTE_IMG'			=> phpbb::$user->img('icon_post_quote', phpbb::$user->lang['REPLY_WITH_QUOTE']),
		'HISTORY_TITLE'		=> $title,
	...
	foreach ($pm_by_id as $message_id => $row)
	{
		...
		foreach ($_types as $ug_type)
		{
			if (isset($address[$message_id][$ug_type]) && sizeof($address[$message_id][$ug_type]))
			{
				...
				{
					$recipient_list[$ug_type][$ug_id] = array('name' => phpbb::$user->lang['NA'], 'colour' => '');
				}
			}
		}
	}
		...
		foreach ($adr_ary as $type => $id_ary)
		{
			foreach ($id_ary as $ug_id => $_id)
			{
				...
				else
				{
					...
					$link = '<a href="' . append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=group&amp;g=' . $ug_id) . '"' . $user_colour . '>';
