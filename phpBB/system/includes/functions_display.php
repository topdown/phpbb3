...
function display_forums($root_data = '', $display_moderators = true, $return_moderators = false)
{
	...
	{
		$unread_ga_list = get_unread_topics(phpbb::$user->data['user_id'], 'AND t.forum_id = 0', '', 1);
	...
	while ($row = phpbb::$db->sql_fetchrow($result))
	{
		...
		$row['forum_id_unapproved_topics'] = (phpbb::$auth->acl_get('m_approve', $forum_id) && ($row['forum_topics_real'] != $row['forum_topics'])) ? $forum_id : 0;
		$row['forum_topics'] = (phpbb::$auth->acl_get('m_approve', $forum_id)) ? $row['forum_topics_real'] : $row['forum_topics'];

		// Display active topics from this forum?
		if ($show_active && $row['forum_type'] == FORUM_POST && phpbb::$auth->acl_get('f_read', $forum_id) && ($row['forum_flags'] & FORUM_FLAG_ACTIVE_TOPICS))
		{
	...
	if ($mark_read == 'forums' || $mark_read == 'all')
	{
		...
		if (check_link_hash($token, 'global'))
		{
			...
			{
				markread('all');
				$message = sprintf(phpbb::$user->lang['RETURN_INDEX'], '<a href="' . $redirect . '">', '</a>');
			}
			else
			{
				...
				$message = sprintf(phpbb::$user->lang['RETURN_FORUM'], '<a href="' . $redirect . '">', '</a>');
			}
		...
		{
			$message = sprintf(phpbb::$user->lang['RETURN_PAGE'], '<a href="' . $redirect . '">', '</a>');
			meta_refresh(3, $redirect);
	...
	foreach ($forum_rows as $row)
	{
		...
		if ($row['parent_id'] == $root_data['forum_id'] && $row['forum_type'] == FORUM_CAT)
		{
			phpbb::$template->assign_block_vars('forumrow', array(
				...
				'FORUM_IMAGE'			=> ($row['forum_image']) ? '<img src="' . phpbb::$phpbb_root_path . $row['forum_image'] . '" alt="' . phpbb::$user->lang['FORUM_CAT'] . '" />' : '',
		...
		if (isset($subforums[$forum_id]))
		{
			foreach ($subforums[$forum_id] as $subforum_id => $subforum_row)
			{
				...
				{
					$subforums_list[] = array(
						'link'		=> append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $subforum_id),
			...
			$l_subforums = (sizeof($subforums[$forum_id]) == 1) ? phpbb::$user->lang['SUBFORUM'] . ': ' : phpbb::$user->lang['SUBFORUMS'] . ': ';
		...
		if ($row['forum_last_post_id'])
		{
			...
			$last_post_url = append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 'f=' . $row['forum_id_last_post'] . '&amp;p=' . $row['forum_last_post_id']) . '#p' . $row['forum_last_post_id'];
		}
		else
		{
		...
		{
			$l_moderator = (sizeof($forum_moderators[$forum_id]) == 1) ? phpbb::$user->lang['MODERATOR'] : phpbb::$user->lang['MODERATORS'];
		...
		{
			$s_subforums_list[] = '<a href="' . $subforum['link'] . '" class="subforum ' . (($subforum['unread']) ? 'unread' : 'read') . '" title="' . (($subforum['unread']) ? phpbb::$user->lang['UNREAD_POSTS'] : phpbb::$user->lang['NO_UNREAD_POSTS']) . '">' . $subforum['name'] . '</a>';
		}
		...
		else
		{
			...
			if (($row['forum_flags'] & FORUM_FLAG_LINK_TRACK) || $row['forum_password'] || !phpbb::$auth->acl_get('f_read', $forum_id))
			{
				$u_viewforum = append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $row['forum_id']);
			}
			else
		...
		phpbb::$template->assign_block_vars('forumrow', array(
			...
			'S_FEED_ENABLED'	=> (phpbb::$config['feed_forum'] && !phpbb_optionget(FORUM_OPTION_FEED_EXCLUDE, $row['forum_options']) && $row['forum_type'] == FORUM_POST) ? true : false,
			...
			'U_UNAPPROVED_TOPICS'	=> ($row['forum_id_unapproved_topics']) ? append_sid(phpbb::$phpbb_root_path . "mcp." . phpbb::$phpEx, 'i=queue&amp;mode=unapproved_topics&amp;f=' . $row['forum_id_unapproved_topics']) : '',
	...
	phpbb::$template->assign_vars(array(
		'U_MARK_FORUMS'		=> (phpbb::$user->data['is_registered'] || phpbb::$config['load_anon_lastread']) ? append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'hash=' . generate_link_hash('global') . '&amp;f=' . $root_data['forum_id'] . '&amp;mark=forums') : '',
		'S_HAS_SUBFORUM'	=> ($visible_forums) ? true : false,
		'L_SUBFORUM'		=> ($visible_forums == 1) ? phpbb::$user->lang['SUBFORUM'] : phpbb::$user->lang['SUBFORUMS'],
		'LAST_POST_IMG'		=> phpbb::$user->img('icon_topic_latest', 'VIEW_LATEST_POST'),
		'UNAPPROVED_IMG'	=> phpbb::$user->img('icon_topic_unapproved', 'TOPICS_UNAPPROVED'),
	));
...
function generate_forum_nav(&$forum_data)
{
	...
	if (!phpbb::$auth->acl_get('f_list', $forum_data['forum_id']))
	{
		return;
	}
	...
	if (!empty($forum_parents))
	{
		foreach ($forum_parents as $parent_forum_id => $parent_data)
		{
			...
			if (!phpbb::$auth->acl_get('f_list', $parent_forum_id))
			{
				continue;
			}
			...
			phpbb::$template->assign_block_vars('navlinks', array(
				...
				'U_VIEW_FORUM'	=> append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $parent_forum_id))
			);
		}
	}
	...
	phpbb::$template->assign_block_vars('navlinks', array(
		...
		'U_VIEW_FORUM'	=> append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $forum_data['forum_id']))
	);
	...
	phpbb::$template->assign_vars(array(
		...
		'S_ENABLE_FEEDS_FORUM'	=> (phpbb::$config['feed_forum'] && $forum_data['forum_type'] == FORUM_POST && !phpbb_optionget(FORUM_OPTION_FEED_EXCLUDE, $forum_data['forum_options'])) ? true : false,
	));
...
function get_moderators(&$forum_moderators, $forum_id = false)
{
	...
	while ($row = phpbb::$db->sql_fetchrow($result))
	{
		...
		else
		{
			...
			if (phpbb::$user->data['user_id'] != ANONYMOUS && !phpbb::$auth->acl_get('u_viewprofile'))
			{
				$forum_moderators[$f_id][] = '<span' . (($row['group_colour']) ? ' style="color:#' . $row['group_colour'] . ';"' : '') . '>' . $group_name . '</span>';
			}
			else
			{
				$forum_moderators[$f_id][] = '<a' . (($row['group_colour']) ? ' style="color:#' . $row['group_colour'] . ';"' : '') . ' href="' . append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=group&amp;g=' . $row['group_id']) . '">' . $group_name . '</a>';
			}
		}
	}
	phpbb::$db->sql_freeresult($result);
...
function gen_forum_auth_level($mode, $forum_id, $forum_status)
{
	...
	$locked = ($forum_status == ITEM_LOCKED && !phpbb::$auth->acl_get('m_edit', $forum_id)) ? true : false;

	$rules = array(
		(phpbb::$auth->acl_get('f_post', $forum_id) && !$locked) ? phpbb::$user->lang['RULES_POST_CAN'] : phpbb::$user->lang['RULES_POST_CANNOT'],
		(phpbb::$auth->acl_get('f_reply', $forum_id) && !$locked) ? phpbb::$user->lang['RULES_REPLY_CAN'] : phpbb::$user->lang['RULES_REPLY_CANNOT'],
		(phpbb::$user->data['is_registered'] && phpbb::$auth->acl_gets('f_edit', 'm_edit', $forum_id) && !$locked) ? phpbb::$user->lang['RULES_EDIT_CAN'] : phpbb::$user->lang['RULES_EDIT_CANNOT'],
		(phpbb::$user->data['is_registered'] && phpbb::$auth->acl_gets('f_delete', 'm_delete', $forum_id) && !$locked) ? phpbb::$user->lang['RULES_DELETE_CAN'] : phpbb::$user->lang['RULES_DELETE_CANNOT'],
	);

	if (phpbb::$config['allow_attachments'])
	{
		$rules[] = (phpbb::$auth->acl_get('f_attach', $forum_id) && phpbb::$auth->acl_get('u_attach') && !$locked) ? phpbb::$user->lang['RULES_ATTACH_CAN'] : phpbb::$user->lang['RULES_ATTACH_CANNOT'];
	}
...
function display_user_activity(&$userdata)
{
	...
	$forum_read_ary = phpbb::$auth->acl_getf('!f_read');
	...
	$fid_m_approve = phpbb::$auth->acl_getf('m_approve', true);
	$sql_m_approve = (!empty($fid_m_approve)) ? 'OR ' . phpbb::$db->sql_in_set('forum_id', array_keys($fid_m_approve)) : '';
	...
	phpbb::$template->assign_vars(array(
		...
		'U_ACTIVE_FORUM'		=> append_sid(phpbb::$phpbb_root_path . "viewforum." . phpbb::$phpEx, 'f=' . $active_f_id),
		'U_ACTIVE_TOPIC'		=> append_sid(phpbb::$phpbb_root_path . "viewtopic." . phpbb::$phpEx, 't=' . $active_t_id),
...
function watch_topic_forum($mode, &$s_watching, $user_id, $forum_id, $topic_id, $notify_status = 'unset', $start = 0)
{
	...
	if ($user_id != ANONYMOUS)
	{
		...
		if (!is_null($notify_status) && $notify_status !== '')
		{
			...
			if (isset($_GET['unwatch']))
			{
				...
				if ($uid != $user_id)
				{
					$redirect_url = append_sid(phpbb::$phpbb_root_path . "view" . $mode . "." . phpbb::$phpEx, "$u_url=$match_id&amp;start=$start");
	...
	if ($can_watch)
	{
		$s_watching['link'] = append_sid(phpbb::$phpbb_root_path . "view" . $mode . "." . phpbb::$phpEx, "$u_url=$match_id&amp;" . (($is_watching) ? 'unwatch' : 'watch') . "=$mode&amp;start=$start&amp;hash=" . generate_link_hash("{$mode}_$match_id"));
...
function get_user_rank($user_rank, $user_posts, &$rank_title, &$rank_img, &$rank_img_src)
{
	...
	if (!empty($user_rank))
	{
		...
		$rank_img = (!empty($ranks['special'][$user_rank]['rank_image'])) ? '<img src="' . phpbb::$phpbb_root_path . phpbb::$config['ranks_path'] . '/' . $ranks['special'][$user_rank]['rank_image'] . '" alt="' . $ranks['special'][$user_rank]['rank_title'] . '" title="' . $ranks['special'][$user_rank]['rank_title'] . '" />' : '';
		$rank_img_src = (!empty($ranks['special'][$user_rank]['rank_image'])) ? phpbb::$phpbb_root_path . phpbb::$config['ranks_path'] . '/' . $ranks['special'][$user_rank]['rank_image'] : '';
	}
		...
		if (!empty($ranks['normal']))
		{
			foreach ($ranks['normal'] as $rank)
			{
				if ($user_posts >= $rank['rank_min'])
				{
					...
					$rank_img = (!empty($rank['rank_image'])) ? '<img src="' . phpbb::$phpbb_root_path . phpbb::$config['ranks_path'] . '/' . $rank['rank_image'] . '" alt="' . $rank['rank_title'] . '" title="' . $rank['rank_title'] . '" />' : '';
					$rank_img_src = (!empty($rank['rank_image'])) ? phpbb::$phpbb_root_path . phpbb::$config['ranks_path'] . '/' . $rank['rank_image'] : '';
					break;
				}
			}
		}
...
{
	if (empty($avatar) || !$avatar_type || (!phpbb::$config['allow_avatar'] && !$ignore_config))
	{
		return '';
	}
	...
	{
		case AVATAR_UPLOAD:
			if (!phpbb::$config['allow_avatar_upload'] && !$ignore_config)
			{
				return '';
			}
			$avatar_img = phpbb::$phpbb_root_path . "download/file." . phpbb::$phpEx . "?avatar=";
		break;

		case AVATAR_GALLERY:
			if (!phpbb::$config['allow_avatar_local'] && !$ignore_config)
			{
				return '';
			}
			$avatar_img = phpbb::$phpbb_root_path . phpbb::$config['avatar_gallery_path'] . '/';
		break;

		case AVATAR_REMOTE:
			if (!phpbb::$config['allow_avatar_remote'] && !$ignore_config)
			{
				return '';
			}
		break;
