...
function bump_topic_allowed($forum_id, $topic_bumped, $last_post_time, $topic_poster, $last_topic_poster)
{
	...
	if (!phpbb::$auth->acl_get('f_bump', $forum_id) || $topic_bumped)
	{
		return false;
	}
...
function censor_text($text)
{
	...
	if (!isset($censors) || !is_array($censors))
	{
		...
		if (!phpbb::$user->optionget('viewcensors') && phpbb::$config['allow_nocensors'] && phpbb::$auth->acl_get('u_chgcensors'))
		{
			$censors = array();
		}
		else
		{
			$censors = phpbb::$cache->obtain_word_list();
		}
	}
...
function smiley_text($text, $force_option = false)
{
	...
	else
	{
		...
		return preg_replace('#<!\-\- s(.*?) \-\-><img src="\{SMILIES_PATH\}\/(.*?) \/><!\-\- s\1 \-\->#', '<img src="' . $root_path . phpbb::$config['smilies_path'] . '/\2 />', $text);
	}
}
...
function parse_attachments($forum_id, &$message, &$attachments, &$update_count, $preview = false)
{
	...
	foreach ($attachments as $attachment)
	{
		...
		$filename = phpbb::$phpbb_root_path . phpbb::$config['upload_path'] . '/' . utf8_basename($attachment['physical_filename']);
		$thumbnail_filename = phpbb::$phpbb_root_path . phpbb::$config['upload_path'] . '/thumb_' . utf8_basename($attachment['physical_filename']);
		...
		if (isset($extensions[$attachment['extension']]))
		{
			...
			{
				$upload_icon = '<img src="' . phpbb::$phpbb_root_path . phpbb::$config['upload_icons_path'] . '/' . trim($extensions[$attachment['extension']]['upload_icon']) . '" alt="" />';
			}
		}
		...
		if (!extension_allowed($forum_id, $attachment['extension'], $extensions))
		{
			...
			$block_array += array(
				'S_DENIED'			=> true,
				'DENIED_MESSAGE'	=> sprintf(phpbb::$user->lang['EXTENSION_DISABLED_AFTER_POSTING'], $attachment['extension'])
			);
		}
		...
		if (!$denied)
		{
			...
			$download_link = append_sid(phpbb::$phpbb_root_path . "download/file." . phpbb::$phpEx, 'id=' . $attachment['attach_id']);
			...
			switch ($display_cat)
			{
				...
				case ATTACHMENT_CATEGORY_IMAGE:
					...
					$inline_link = append_sid(phpbb::$phpbb_root_path . "download/file." . phpbb::$phpEx, 'id=' . $attachment['attach_id']);
				...
				case ATTACHMENT_CATEGORY_THUMB:
					$l_downloaded_viewed = 'VIEWED_COUNT';
					$thumbnail_link = append_sid(phpbb::$phpbb_root_path . "download/file." . phpbb::$phpEx, 'id=' . $attachment['attach_id'] . '&amp;t=1');
					$download_link .= '&amp;mode=view';
...
function get_username_string($mode, $user_id, $username, $username_colour = '', $guest_username = false, $custom_profile_url = false)
{
	...
	{
		$_profile_cache['base_url'] = append_sid(phpbb::$phpbb_root_path . "memberlist." . phpbb::$phpEx, 'mode=viewprofile&amp;u={USER_ID}');
	...
	switch ($mode)
	{
		...
		case 'profile':
			...
			if ($user_id && $user_id != ANONYMOUS && (phpbb::$user->data['user_id'] == ANONYMOUS || phpbb::$auth->acl_get('u_viewprofile')))
			{
