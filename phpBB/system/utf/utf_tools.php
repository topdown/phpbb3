...
function utf8_recode($string, $encoding)
{
	...
	if (preg_match('/iso[_ -]?8859[_ -]?(\\d+)/', $encoding, $array))
	{
		switch ($array[1])
		{
			...
			case '15':
				...
				{
					if (!file_exists(phpbb::$phpbb_root_path . 'system/utf/data/recode_basic.' . phpbb::$phpEx))
					{
	...
	if (preg_match('/sjis(?:[_ -]?win)?|(?:cp|ibm)[_ -]?932|shift[_ -]?jis/', $encoding))
	{
		...
		{
			if (!file_exists(phpbb::$phpbb_root_path . 'system/utf/data/recode_cjk.' . phpbb::$phpEx))
			{
				trigger_error('CJK reencoder file is missing', E_USER_ERROR);
			}
			include(phpbb::$phpbb_root_path . 'system/utf/data/recode_cjk.' . phpbb::$phpEx);
		}
		return sjis($string);
	}
...
function utf8_case_fold($text, $option = 'full')
{
	...
	if (!isset($uniarray['c']))
	{
		$uniarray['c'] = include(phpbb::$phpbb_root_path . 'system/utf/data/case_fold_c.' . phpbb::$phpEx);
	}

	// only set full if we need to
	if ($option === 'full' && !isset($uniarray['f']))
	{
		$uniarray['f'] = include(phpbb::$phpbb_root_path . 'system/utf/data/case_fold_f.' . phpbb::$phpEx);
	}

	// only set simple if we need to
	if ($option !== 'full' && !isset($uniarray['s']))
	{
		$uniarray['s'] = include(phpbb::$phpbb_root_path . 'system/utf/data/case_fold_s.' . phpbb::$phpEx);
	}
...
function utf8_clean_string($text)
{
	...
	if (empty($homographs))
	{
		$homographs = include(phpbb::$phpbb_root_path . 'system/utf/data/confusables.' . phpbb::$phpEx);
	}
