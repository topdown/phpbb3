...
class template
{
	...
	*/
	function set_template()
	{
		...
		if (file_exists(phpbb::$phpbb_root_path . 'styles/' . phpbb::$user->theme['template_path'] . '/template'))
		{
			$this->root = phpbb::$phpbb_root_path . 'styles/' . phpbb::$user->theme['template_path'] . '/template';
			$this->cachepath = phpbb::$phpbb_root_path . 'cache/tpl_' . str_replace('_', '-', phpbb::$user->theme['template_path']) . '_';

			if ($this->orig_tpl_storedb === null)
			{
				$this->orig_tpl_storedb = phpbb::$user->theme['template_storedb'];
			}

			if ($this->orig_tpl_inherits_id === null)
			{
				$this->orig_tpl_inherits_id = phpbb::$user->theme['template_inherits_id'];
			}

			phpbb::$user->theme['template_storedb'] = $this->orig_tpl_storedb;
			phpbb::$user->theme['template_inherits_id'] = $this->orig_tpl_inherits_id;

			if (phpbb::$user->theme['template_inherits_id'])
			{
				$this->inherit_root = phpbb::$phpbb_root_path . 'styles/' . phpbb::$user->theme['template_inherit_path'] . '/template';
			}
		}
		else
		{
	...
	*/
	function set_custom_template($template_path, $template_name, $fallback_template_path = false)
	{
		...
		$this->cachepath = phpbb::$phpbb_root_path . 'cache/ctpl_' . str_replace('_', '-', $template_name) . '_';
	...
	*/
	function _tpl_load(&$handle)
	{
		...
		$this->files_template[$handle] = (isset(phpbb::$user->theme['template_id'])) ? phpbb::$user->theme['template_id'] : 0;
		...
		else if (phpbb::$config['load_tplcompile'])
		{
			// No way around it: we need to check inheritance here
			if (phpbb::$user->theme['template_inherits_id'] && !file_exists($this->files[$handle]))
			{
				$this->files[$handle] = $this->files_inherit[$handle];
				$this->files_template[$handle] = phpbb::$user->theme['template_inherits_id'];
			}
		...
		{
			include(phpbb::$phpbb_root_path . 'system/core/template_compile.' . phpbb::$phpEx);
		}

		// Inheritance - we point to another template file for this one. Equality is also used for store_db
		if (isset(phpbb::$user->theme['template_inherits_id']) && phpbb::$user->theme['template_inherits_id'] && !file_exists($this->files[$handle]))
		{
			$this->files[$handle] = $this->files_inherit[$handle];
			$this->files_template[$handle] = phpbb::$user->theme['template_inherits_id'];
		}
		...
		if (isset(phpbb::$user->theme['template_storedb']) && phpbb::$user->theme['template_storedb'])
		{
			$rows = array();
			$ids = array();
			// Inheritance
			if (isset(phpbb::$user->theme['template_inherits_id']) && phpbb::$user->theme['template_inherits_id'])
			{
				$ids[] = phpbb::$user->theme['template_inherits_id'];
			}
			$ids[] = phpbb::$user->theme['template_id'];
			...
			foreach ($ids as $id)
			{
				...
				WHERE template_id = ' . $id . "
					AND (template_filename = '" . phpbb::$db->sql_escape($this->filename[$handle]) . "'
						OR template_included " . phpbb::$db->sql_like_expression(phpbb::$db->any_char . $this->filename[$handle] . ':' . phpbb::$db->any_char) . ')';
			...
			if (sizeof($rows))
			{
				foreach ($rows as $row)
				{
					...
					$force_reload = false;
					if ($row['template_id'] != phpbb::$user->theme['template_id'])
					{
						// make sure that we are not overlooking a file not in the db yet
						if (isset(phpbb::$user->theme['template_inherits_id']) && phpbb::$user->theme['template_inherits_id'] && !file_exists($file))
						{
							...
							$this->files_template[$row['template_filename']] = phpbb::$user->theme['template_inherits_id'];
						}
						else if (isset(phpbb::$user->theme['template_inherits_id']) && phpbb::$user->theme['template_inherits_id'])
						{
					...
					{
						$this->files_template[$row['template_filename']] = phpbb::$user->theme['template_id'];
					}
					...
					else
					{
						...
						if (!file_exists($this->cachepath . str_replace('/', '.', $row['template_filename']) . '.' . phpbb::$phpEx))
						{
							$this->filename[$row['template_filename']] = $row['template_filename'];
	...
	*/
	function _php_include($filename)
	{
		...
		$file = phpbb::$phpbb_root_path . $filename;
