...
//require(phpbb::$phpbb_root_path . 'system/core/phpbb.' . phpbb::$phpEx);
...
class cache extends acm
{
	...
	*/
	function obtain_config()
	{
		...
		if ((phpbb::$config = $this->get('config')) !== false)
		{
	...
	*/
	function obtain_cfg_items($theme)
	{
		...
		foreach ($parsed_items as $key => $parsed_array)
		{
			...
			$reparse = false;
			$filename = phpbb::$phpbb_root_path . 'styles/' . $theme[$key . '_path'] . '/' . $key . '/' . $key . '.cfg';
	...
	*/
	function obtain_hooks()
	{
		if (($hook_files = $this->get('_hooks')) === false)
		{
			...
			$dh = @opendir(phpbb::$phpbb_root_path . 'system/hooks/');

			if ($dh)
			{
				while (($file = readdir($dh)) !== false)
				{
					if (strpos($file, 'hook_') === 0 && substr($file, -(strlen(phpbb::$phpEx) + 1)) === '.' . phpbb::$phpEx)
					{
						$hook_files[] = substr($file, 0, -(strlen(phpbb::$phpEx) + 1));
					}
				}
				closedir($dh);
			}
