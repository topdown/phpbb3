...
class p_master
{
	...
	protected $include_path = false;
	public $active_module = false;
	protected $active_module_row_id = false;
	public $acl_forum_id = false;
	protected $module_ary = array();
	 ...
	 * @return void
	 */
	public function p_master($include_path = false)
	{


		$this->include_path = ($include_path !== false) ? $include_path : phpbb::$phpbb_root_path . '/system/modules/';
	 ...
	 * @return void
	 */
	public function list_modules($p_class)
	{
		...
		// Get cached modules
		if (($this->module_cache = phpbb::$cache->get('_modules_' . $this->p_class)) === false)
		{
			// Get modules
			$sql = 'SELECT *
				FROM ' . MODULES_TABLE . "
			...
			phpbb::$cache->put('_modules_' . $this->p_class, $this->module_cache);
		}
		...
		foreach ($this->module_cache['modules'] as $key => $row)
		{
			...
			$module_row = array(
				...
				'lang' => ($row['module_basename'] && function_exists($lang_func))
					? $lang_func($row['module_mode'], $row['module_langname'])
					: ((!empty(phpbb::$user->lang[$row['module_langname']])) ? phpbb::$user->lang[$row['module_langname']]
						: $row['module_langname']),
	 ...
	 */
	public function loaded($module_basename, $module_mode = false)
	{
	...
	public function module_auth($module_auth, $forum_id = false)
	{
		...
		$is_auth = false;
		eval('$is_auth = (int) (' . preg_replace(array('#acl_([a-z0-9_]+)(,\$id)?#', '#\$id#', '#aclf_([a-z0-9_]+)#', '#cfg_([a-z0-9_]+)#', '#request_([a-zA-Z0-9_]+)#'), array('(int) phpbb::$auth->acl_get(\'\\1\'\\2)', '(int) $forum_id', '(int) phpbb::$auth->acl_getf_global(\'\\1\')', '(int) phpbb::$config[\'\\1\']', '!empty($_REQUEST[\'\\1\'])'), $module_auth) . ');');
	 ...
	 * @return void
	 */
	public function set_active($id = false, $mode = false)
	{
		$icat = false;
	 ...
	 *
	 * @param bool $mode
	 * @param bool $module_url
	 * @param bool $execute_module
	 * @return
	 */
	public function load_active($mode = false, $module_url = false, $execute_module = true)
	{
		...
		{
			if (!file_exists("$module_path/{$this->p_class}_$this->p_name." . phpbb::$phpEx))
			{
				trigger_error("Cannot find module $module_path/{$this->p_class}_$this->p_name." . phpbb::$phpEx, E_USER_ERROR);
			}

			include("$module_path/{$this->p_class}_$this->p_name." . phpbb::$phpEx);
			...
			if (defined('IN_ADMIN'))
			{
				...
				$this->module->u_action = append_sid("{$phpbb_admin_path}index." . phpbb::$phpEx, "i={$this->p_name}") . (($icat)
					? '&amp;icat=' . $icat : '') . "&amp;mode={$this->p_mode}";
			}
			else
			{
				...
				{
					$this->module->u_action = phpbb::$phpbb_root_path . ((phpbb::$user->page['page_dir'])
						? phpbb::$user->page['page_dir'] . '/' : '') . phpbb::$user->page['page_name'];
				}

				$this->module->u_action = append_sid($this->module->u_action, "i={$this->p_name}") . (($icat)
					? '&amp;icat=' . $icat : '') . "&amp;mode={$this->p_mode}";
			}
	 ...
	 * One example can be seen in /system/includes/mcp/mcp_notes.php - $this->p_master->adjust_url() call.
	 *
	 * @param string $url_extra Extra url parameters, e.g.: &amp;u=$user_id
	 *
	 */
	public function adjust_url($url_extra)
	{
	 ...
	 * @return bool
	 */
	public function is_active($id, $mode = false)
	{
	 ...
	 * @return array
	 */
	public function get_parents($parent_id, $left_id, $right_id, &$all_parents)
	{
	 ...
	 * @return array
	 */
	public function get_branch($left_id, $right_id, $remaining)
	{
		$branch = array();
	 ...
	 * @return array
	 */
	public function build_tree(&$modules, &$parents)
	{
		$tree = array();
		...
		foreach ($this->module_ary as $row_id => $item_ary)
		{
			...
			else if ($depth < $current_depth)
			{
				for ($i = $current_depth - $depth; $i > 0; $i--)
				{
					...
					$tabular_offset = ($i + $depth > 1) ? substr($tabular_offset, 0, strrpos($tabular_offset, '.'))
						: $tabular_offset;
				}
			}

			$u_title = $module_url . $delim . 'i=' . (($item_ary['cat']) ? $item_ary['id']
				: $item_ary['name'] . (($item_ary['is_duplicate']) ? '&amp;icat=' . $current_id
					: '') . '&amp;mode=' . $item_ary['mode']);
	 ...
	 * @return p_master module.page_title string
	 */
	public function get_page_title()
	{
		...
		return (isset(phpbb::$user->lang[$this->module->page_title])) ? phpbb::$user->lang[$this->module->page_title]
	 ...
	 * @param bool $mode
	 */
	public function load($class, $name, $mode = false)
	{
		$this->p_class = $class;
		$this->p_name = $name;
	 ...
	 */
	public function display($page_title, $display_online_list = true)
	{
	 ...
	 * @param bool $display
	 */
	public function set_display($id, $mode = false, $display = true)
	{
		foreach ($this->module_ary as $row_id => $item_ary)
		{
	 ...
	 * @param $module_class
	 */
	public function add_mod_info($module_class)
	{
		...
		if (file_exists(phpbb::$user->lang_path . phpbb::$user->lang_name . '/mods'))
		{
			...
			if ($dir)
			{
				...
				{
					if (strpos($entry, 'info_' . strtolower($module_class) . '_') === 0 && substr(strrchr($entry, '.'), 1) == phpbb::$phpEx)
					{
						$add_files[] = 'mods/' . substr(basename($entry), 0, -(strlen(phpbb::$phpEx) + 1));
					}
				}
				closedir($dir);
			}
