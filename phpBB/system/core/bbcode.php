...
class bbcode
{
	...
	public $bbcode_cache = array();
	public $bbcode_template = array();
	...
	/**
	 * Constructor
	 * Init bbcode cache entries if bitfield is
	 *
	 * @param string $bitfield
	 */
	public function bbcode($bitfield = '')
	{
		if ($bitfield)
		{
	 ...
	 * @param bool $bbcode_bitfield
	 * @return
	 */
	public function bbcode_second_pass(&$message, $bbcode_uid = '', $bbcode_bitfield = false)
	{
		if ($bbcode_uid)
		{
	...
	*/
	public function bbcode_cache_init()
	{

		if (empty($this->template_filename))
		{
			$this->template_bitfield = new bitfield(phpbb::$user->theme['bbcode_bitfield']);
			$this->template_filename = phpbb::$phpbb_root_path . 'styles/' . phpbb::$user->theme['template_path'] . '/template/bbcode.html';

			if (!@file_exists($this->template_filename))
			{
				if (isset(phpbb::$user->theme['template_inherits_id']) && phpbb::$user->theme['template_inherits_id'])
				{
					$this->template_filename = phpbb::$phpbb_root_path . 'styles/' . phpbb::$user->theme['template_inherit_path'] . '/template/bbcode.html';
					if (!@file_exists($this->template_filename))
					{
	 ...
	 * @param $tpl_name
	 * @param $bbcode_id
	 * @param bool $skip_bitfield_check
	 * @return bool
	 */
	public function bbcode_tpl($tpl_name, $bbcode_id = -1, $skip_bitfield_check = false)
	{
	 ...
	 * @return string
	 */
	public function bbcode_tpl_replace($tpl_name, $tpl)
	{
	 ...
	 * @return string
	 */
	public function bbcode_list($type)
	{
		if ($type == '')
		{
	 ...
	 * @return string
	 */
	public function bbcode_second_pass_quote($username, $quote)
	{
	 ...
	 * @return string
	 */
	public function bbcode_second_pass_code($type, $code)
	{
		// when using the /e modifier, preg_replace slashes double-quotes but does not
		// seem to slash anything else
