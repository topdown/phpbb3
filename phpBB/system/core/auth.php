...
class auth
{
	 ...
	 * @var array
	 */
	protected $acl_options = array();
	/**
	 * @var bool
	 */
	protected $acl_forum_ids = false;
	/**
	 * @var
	 */
	public $role_cache;
	 ...
	 * Init permissions
	 *
	 * @param array &$userdata referenced
	 * @return bool
	 */
	public function acl(&$userdata)
	{
		...
		if (($this->acl_options = phpbb::$cache->get('_acl_options')) === false)
		{
			...
			phpbb::$cache->put('_acl_options', $this->acl_options);
		}
	 ...
	 *
	 * @param string $user_permissions user_permissions column
	 ...
	 * @param bool $opts     options
	 * @param bool $forum_id forum id
	 * @return array $auth_ary
	 */
	 ...
	 *
	 * @param array &$userdata User data array reference to
	 * @return bool 
	 */
	 ...
	 *
	 * @param array &$hold_ary array that holds the bit string
	 * @return string
	 */
	...
	function acl_clear_prefetch($user_id = false)
	{
		...
		phpbb::$cache->destroy('_role_cache');
		...
		phpbb::$cache->put('_role_cache', $this->role_cache);
	 ...
	 * Get assigned roles
	 *
	 * @param int  $user_type User group
	 * @param int  $role_type User role type
	 * @param bool $ug_id     User group id
	 * @param bool $forum_id  The id of the forum we are working with
	 * @return array
	 */
	function acl_role_data($user_type, $role_type, $ug_id = false, $forum_id = false)
	{
		...
		$sql_ug = ($ug_id !== false) ? ((!is_array($ug_id)) ? "AND a.$sql_id = $ug_id"
			: 'AND ' . phpbb::$db->sql_in_set("a.$sql_id", $ug_id)) : '';
		$sql_forum = ($forum_id !== false) ? ((!is_array($forum_id)) ? "AND a.forum_id = $forum_id"
			: 'AND ' . phpbb::$db->sql_in_set('a.forum_id', $forum_id)) : '';
	...
	function acl_raw_data($user_id = false, $opts = false, $forum_id = false)
	{
		...
		$sql_user = ($user_id !== false) ? ((!is_array($user_id)) ? 'user_id = ' . (int) $user_id
			: phpbb::$db->sql_in_set('user_id', array_map('intval', $user_id))) : '';
		$sql_forum = ($forum_id !== false) ? ((!is_array($forum_id)) ? 'AND a.forum_id = ' . (int) $forum_id
			: 'AND ' . phpbb::$db->sql_in_set('a.forum_id', array_map('intval', $forum_id))) : '';
		...
		foreach ($sql_ary as $sql)
		{
			...
			while ($row = phpbb::$db->sql_fetchrow($result))
			{
				$option = ($sql_opts_select) ? $row['auth_option']
					: $this->acl_options['option'][$row['auth_option_id']];
	...
	function acl_group_raw_data($group_id = false, $opts = false, $forum_id = false)
	{
		...
		$sql_group = ($group_id !== false) ? ((!is_array($group_id)) ? 'group_id = ' . (int) $group_id
			: phpbb::$db->sql_in_set('group_id', array_map('intval', $group_id))) : '';
		$sql_forum = ($forum_id !== false) ? ((!is_array($forum_id)) ? 'AND a.forum_id = ' . (int) $forum_id
			: 'AND ' . phpbb::$db->sql_in_set('a.forum_id', array_map('intval', $forum_id))) : '';
	...
	function acl_raw_data_single_user($user_id)
	{
		...
		if (($this->role_cache = phpbb::$cache->get('_role_cache')) === false)
		{
			$this->role_cache = array();
		...
		while ($row = phpbb::$db->sql_fetchrow($result))
		{
			...
			if ($row['auth_role_id'])
			{
				$hold_ary[$row['forum_id']] = (empty($hold_ary[$row['forum_id']]))
					? unserialize($this->role_cache[$row['auth_role_id']])
		...
		while ($row = phpbb::$db->sql_fetchrow($result))
		{
			...
			{
				if (!empty($this->role_cache[$row['auth_role_id']]))
				{
	...
	function login($username, $password, $autologin = false, $viewonline = 1, $admin = 0)
	{
		...
		$method = trim(basename(phpbb::$config['auth_method']));
		include_once phpbb::$phpbb_root_path . 'system/auth/auth_' . $method . '.' . phpbb::$phpEx;

		$method = 'login_' . $method;
		if (function_exists($method))
		{
			$login = $method($username, $password, phpbb::$user->ip, phpbb::$user->browser, phpbb::$user->forwarded_for);
			...
			{
				$old_session_id = phpbb::$user->session_id;
				...
				if ($admin)
				{
					...
					phpbb::$user->set_cookie('u', '', $cookie_expire);
					phpbb::$user->set_cookie('sid', '', $cookie_expire);
					unset($cookie_expire);

					$SID = '?sid=';
					phpbb::$user->session_id = $_SID = '';
				}

				$result = phpbb::$user->session_create($login['user_row']['user_id'], $admin, $autologin, $viewonline);
				...
				if ($result === true)
				{
					...
					if ($admin)
					{
						...
						$sql = 'DELETE FROM ' . SESSIONS_TABLE . "
							WHERE session_id = '" . phpbb::$db->sql_escape($old_session_id) . "'
	 ...
	 *
	 * @param string $key
	 * @param mixed  $auth_options
	 * @param mixed  &$sql_opts
	 * @return void
	 */
