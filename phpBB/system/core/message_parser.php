	...
	function path_in_domain($url)
	{
		...
		if (strpos($url, ".{$phpEx}") !== false && strpos($url, $check_path) !== false)
		{
			...
			if ($pos_domain !== false && $pos_path >= $pos_domain && $pos_ext >= $pos_path)
			{
				...
				if (strpos($url, '/' . $check_path . '/download/file.' . phpbb::$phpEx) !== 0)
				{
					return false;
				}
...
class parse_message extends bbcode_firstpass
{
	...
	*/
	function parse($allow_bbcode, $allow_magic_url, $allow_smilies, $allow_img_bbcode = true, $allow_flash_bbcode = true, $allow_quote_bbcode = true, $allow_url_bbcode = true, $update_this_message = true, $mode = 'post')
	{
		...
		{
			if (!isset(phpbb::$config['max_' . $mode . '_' . $key]))
			{
				phpbb::$config['max_' . $mode . '_' . $key] = 0;
			}
		}
		...
		if ((int) phpbb::$config['max_' . $mode . '_chars'] > 0 && $message_length > (int) phpbb::$config['max_' . $mode . '_chars'])
		{
			$this->warn_msg[] = sprintf(phpbb::$user->lang['TOO_MANY_CHARS_' . strtoupper($mode)], $message_length, (int) phpbb::$config['max_' . $mode . '_chars']);
		...
		if ($mode === 'post')
		{
			if (!$message_length || $message_length < (int) phpbb::$config['min_post_chars'])
			{
				$this->warn_msg[] = (!$message_length) ? phpbb::$user->lang['TOO_FEW_CHARS'] : sprintf(phpbb::$user->lang['TOO_FEW_CHARS_LIMIT'], $message_length, (int) phpbb::$config['min_post_chars']);
				return (!$update_this_message) ? $return_message : $this->warn_msg;
			}
		}
	...
	*/
	function parse_attachments($form_name, $mode, $forum_id, $submit, $preview, $refresh, $is_message = false)
	{
		...
		{
			if ($num_attachments < $cfg['max_attachments'] || phpbb::$auth->acl_get('a_') || phpbb::$auth->acl_get('m_', $forum_id))
			{
		...
		if ($preview || $refresh || sizeof($error))
		{
			...
			{
				if ($num_attachments < $cfg['max_attachments'] || phpbb::$auth->acl_gets('m_', 'a_', $forum_id))
				{
					$filedata = upload_attachment($form_name, $forum_id, false, '', $is_message);
	...
	*/
	function parse_poll(&$poll)
	{
		...
		$poll['poll_option_text'] = $this->parse($poll['enable_bbcode'], (phpbb::$config['allow_post_links']) ? $poll['enable_urls'] : false, $poll['enable_smilies'], $poll['img_status'], false, false, phpbb::$config['allow_post_links'], false, 'poll');
		...
		else
		{
			...
			$poll['poll_title'] = $this->parse($poll['enable_bbcode'], (phpbb::$config['allow_post_links']) ? $poll['enable_urls'] : false, $poll['enable_smilies'], $poll['img_status'], false, false, phpbb::$config['allow_post_links'], false, 'poll');
