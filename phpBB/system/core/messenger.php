...
class messenger
{
	...
	*/
	function template($template_file, $template_lang = '', $template_path = '')
	{
		...
		if (!isset($this->tpl_msg[$template_lang . $template_file]))
		{
			...
			if (!$template_path)
			{
				$template_path = (!empty(phpbb::$user->lang_path)) ? phpbb::$user->lang_path : phpbb::$phpbb_root_path . 'language/';
				$template_path .= $template_lang . '/email';

				// we can only specify default language fallback when the path is not a custom one for which we
				// do not know the default language alternative
				if ($template_lang !== basename(phpbb::$config['default_lang']))
				{
					$fallback_template_path = (!empty(phpbb::$user->lang_path)) ? phpbb::$user->lang_path : phpbb::$phpbb_root_path . 'language/';
					$fallback_template_path .= basename(phpbb::$config['default_lang']) . '/email';
				}
			}
	...
	*/
	function send($method = NOTIFY_EMAIL, $break = false)
	{
		...
		{
			$this->assign_vars(array(
				'EMAIL_SIG'	=> str_replace('<br />', "\n", "-- \n" . htmlspecialchars_decode(phpbb::$config['board_email_sig'])),
			));
		}
		...
		{
			$this->assign_vars(array(
				'SITENAME'	=> htmlspecialchars_decode(phpbb::$config['sitename']),
			));
		}
	...
	*/
	function msg_email()
	{
		...
		$encode_eol = (phpbb::$config['smtp_delivery']) ? "\r\n" : $this->eol;
	...
	*/
	function msg_jabber()
	{
		...
		if (!$use_queue)
		{
			include_once(phpbb::$phpbb_root_path . 'system/includes/functions_jabber.' . phpbb::$phpEx);
			...
			{
				$this->error('JABBER', phpbb::$user->lang['ERR_JAB_CONNECT'] . '<br />' . $this->jabber->get_log());
				return false;
			}

			if (!$this->jabber->login())
			{
				$this->error('JABBER', phpbb::$user->lang['ERR_JAB_AUTH'] . '<br />' . $this->jabber->get_log());
				return false;
			}
...
class queue
{
	...
	*/
	function queue()
	{
		...
		$this->data = array();
		$this->cache_file = phpbb::$phpbb_root_path . "cache/queue.$phpEx";
	...
	*/
	function process()
	{
		...
		if (!file_exists($this->cache_file) || filemtime($this->cache_file) > time() - phpbb::$config['queue_interval'])
		{
			$this->unlock($lock_fp);
			return;
		}
...
class smtp_class
{
	...
	*/
	function digest_md5($username, $password)
	{
		...
		{
			$tokens['realm'] = (function_exists('php_uname')) ? php_uname('n') : phpbb::$user->host;
		}
