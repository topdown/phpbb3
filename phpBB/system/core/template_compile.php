...
class template_compile
{
	...
	public function compile($code, $no_echo = false, $echo_var = '')
	{
		...
		for ($curr_tb = 0, $tb_size = sizeof($blocks); $curr_tb < $tb_size; $curr_tb++)
		{
			...
			switch ($block_val[1])
			{
				...
				case 'INCLUDEPHP':
					$compile_blocks[] = (phpbb::$config['tpl_allow_php']) ? '<?php ' . $this->compile_tag_include_php(array_shift($includephp_blocks)) . ' ?>' : '';
				break;

				case 'PHP':
					$compile_blocks[] = (phpbb::$config['tpl_allow_php']) ? '<?php ' . array_shift($php_blocks) . ' ?>' : '';
				break;
	...
	private function compile_var_tags(&$text_blocks)
	{
		...
		{
			$text_blocks = preg_replace('#\{L_([A-Z0-9\-_]+)\}#', "<?php echo ((isset(\$this->_rootref['L_\\1'])) ? \$this->_rootref['L_\\1'] : ((isset(phpbb::\$user->lang['\\1'])) ? phpbb::\$user->lang['\\1'] : '{ \\1 }')); ?>", $text_blocks);
		}
		...
		{
			$text_blocks = preg_replace('#\{LA_([A-Z0-9\-_]+)\}#', "<?php echo ((isset(\$this->_rootref['LA_\\1'])) ? \$this->_rootref['LA_\\1'] : ((isset(\$this->_rootref['L_\\1'])) ? addslashes(\$this->_rootref['L_\\1']) : ((isset(phpbb::\$user->lang['\\1'])) ? addslashes(phpbb::\$user->lang['\\1']) : '{ \\1 }'))); ?>", $text_blocks);
		}
	...
	function compile_tag_define($tag_args, $op)
	{
		...
		else
		{
			...
			// @todo Undefined offset fix
			if(key_exists(0, $type))
			{
				$t = strtolower($type[0]);
			}
			else
	...
	function compile_write($handle, $data)
	{
		...
		$filename = $this->template->cachepath . str_replace('/', '.', $this->template->filename[$handle]) . '.' . phpbb::$phpEx;
