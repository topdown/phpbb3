...
class session
{
	...
	function extract_current_page($root_path)
	{
		...
		$args = (!empty($_SERVER['QUERY_STRING'])) ? explode('&', $_SERVER['QUERY_STRING'])
			: explode('&', getenv('QUERY_STRING'));

		// If we are unable to get the script name we use REQUEST_URI as a failover and note it within the page array for easier support...
		if (!$script_name)
		{
			$script_name = (!empty($_SERVER['REQUEST_URI'])) ? $_SERVER['REQUEST_URI'] : getenv('REQUEST_URI');
			$script_name = (($pos = strpos($script_name, '?')) !== false) ? substr($script_name, 0, $pos)
				: $script_name;
	...
	function extract_current_hostname()
	{
		...
		if (empty($host))
		{
			...
			{
				$host = (strpos(phpbb::$config['cookie_domain'], '.') === 0) ? substr(phpbb::$config['cookie_domain'], 1)
					: phpbb::$config['cookie_domain'];
			}
			else
			{
	...
	function session_begin($update_session_page = true)
	{
		...
		$this->forwarded_for = (!empty($_SERVER['HTTP_X_FORWARDED_FOR']))
			? htmlspecialchars((string) $_SERVER['HTTP_X_FORWARDED_FOR']) : '';

		$this->host = $this->extract_current_hostname();
		$this->page = $this->extract_current_page(phpbb::$phpbb_root_path);
		...
		if (isset($_COOKIE[phpbb::$config['cookie_name'] . '_sid']) || isset($_COOKIE[phpbb::$config['cookie_name'] . '_u']))
		{
		...
		if (!empty($this->session_id) && (!defined('NEED_SID') || (isset($_GET['sid']) && $this->session_id === $_GET['sid'])))
		{
			...
			if (isset($this->data['user_id']))
			{
				...
				{
					$s_ip = short_ipv6($this->data['session_ip'], phpbb::$config['ip_check']);
					$u_ip = short_ipv6($this->ip, phpbb::$config['ip_check']);
				}
				else
				{
					$s_ip = implode('.', array_slice(explode('.', $this->data['session_ip']), 0, phpbb::$config['ip_check']));
					$u_ip = implode('.', array_slice(explode('.', $this->ip), 0, phpbb::$config['ip_check']));
				}

				$s_browser = (phpbb::$config['browser_check'])
					? trim(strtolower(substr($this->data['session_browser'], 0, 149))) : '';
				$u_browser = (phpbb::$config['browser_check']) ? trim(strtolower(substr($this->browser, 0, 149))) : '';

				$s_forwarded_for = (phpbb::$config['forwarded_for_check'])
					? substr($this->data['session_forwarded_for'], 0, 254) : '';
				$u_forwarded_for = (phpbb::$config['forwarded_for_check']) ? substr($this->forwarded_for, 0, 254) : '';

				// referer checks
				// The @ before phpbb::$config['referer_validation'] suppresses notices present while running the updater
				$check_referer_path = (@phpbb::$config['referer_validation'] == REFERER_VALIDATE_PATH);
				$referer_valid = true;

				// we assume HEAD and TRACE to be foul play and thus only whitelist GET
				if (@phpbb::$config['referer_validation'] && isset($_SERVER['REQUEST_METHOD']) && strtolower($_SERVER['REQUEST_METHOD']) !== 'get')
				{
				...
				if ($u_ip === $s_ip && $s_browser === $u_browser && $s_forwarded_for === $u_forwarded_for && $referer_valid)
				{
					...
					include_once(phpbb::$phpbb_root_path . 'system/auth/auth_' . $method . '.' . phpbb::$phpEx);
					...
					if (!$session_expired)
					{
						...
						if ($this->time_now - $this->data['session_time'] > 60 || ($this->update_session_page && $this->data['session_page'] != $this->page['page']))
						{
							...
							if ($this->data['user_id'] != ANONYMOUS && !empty(phpbb::$config['new_member_post_limit']) && $this->data['user_new'] && phpbb::$config['new_member_post_limit'] <= $this->data['user_posts'])
							{
	...
	function session_create($user_id = false, $set_admin = false, $persist_login = false, $viewonline = true)
	{
		 ...
		 */
		$bot = false;
		$active_bots = phpbb::$cache->obtain_bots();
		...
		{
			$this->data['session_last_visit'] = (isset($this->data['session_time']) && $this->data['session_time'])
				? $this->data['session_time'] : (($this->data['user_lastvisit']) ? $this->data['user_lastvisit']
					: time());
		}
		else
		{
		...
		if ($this->data['is_bot'] && $bot == $this->data['user_id'] && $this->data['session_id'])
		{
			...
			$s_browser = (phpbb::$config['browser_check']) ? trim(strtolower(substr($this->data['session_browser'], 0, 149)))
				: '';
			$u_browser = (phpbb::$config['browser_check']) ? trim(strtolower(substr($this->browser, 0, 149))) : '';

			$s_forwarded_for = (phpbb::$config['forwarded_for_check']) ? substr($this->data['session_forwarded_for'], 0, 254)
				: '';
			$u_forwarded_for = (phpbb::$config['forwarded_for_check']) ? substr($this->forwarded_for, 0, 254) : '';
		...
		$session_autologin = (($this->cookie_data['k'] || $persist_login) && $this->data['is_registered']) ? true
			: false;
		...
		if (!defined('IN_ERROR_HANDLER') && (!$this->session_id || !phpbb::$db->sql_query($sql) || !phpbb::$db->sql_affectedrows()))
		{
			// Limit new sessions in 1 minute period (if required)
			if (empty($this->data['session_time']) && phpbb::$config['active_sessions'])
			{
				//				phpbb::$db->sql_return_on_error(false);
		...
		if (!$bot)
		{
			$cookie_expire = $this->time_now + ((phpbb::$config['max_autologin_time'])
				? 86400 * (int) phpbb::$config['max_autologin_time'] : 31536000);
	...
	function session_gc()
	{
		...
		if ($del_sessions < $batch_size)
		{
			...
			phpbb_captcha_factory::garbage_collect(phpbb::$config['captcha_plugin']);

			$sql = 'DELETE FROM ' . LOGIN_ATTEMPT_TABLE . '
				WHERE attempt_time < ' . (time() - (int) phpbb::$config['ip_login_limit_time']);
			phpbb::$db->sql_query($sql);
		}
	...
	function set_cookie($name, $cookiedata, $cookietime)
	{
		...
		$domain = (!phpbb::$config['cookie_domain'] || phpbb::$config['cookie_domain'] == 'localhost' || phpbb::$config['cookie_domain'] == '127.0.0.1')
			? '' : '; domain=' . phpbb::$config['cookie_domain'];

		header('Set-Cookie: ' . $name_data . (($cookietime) ? '; expires=' . $expire
				: '') . '; path=' . phpbb::$config['cookie_path'] . $domain . ((!phpbb::$config['cookie_secure']) ? ''
	 ...
	 *
	 * @param bool $user_id
	 * @param array|bool|string $user_ips Can contain a string with one IP or an array of multiple IPs
	...
	function check_ban($user_id = false, $user_ips = false, $user_email = false, $return = false)
	{
		...
		if ($banned && !$return)
		{
			...
			$message .= ($ban_row['ban_give_reason'])
				? '<br /><br />' . sprintf($this->lang['BOARD_BAN_REASON'], $ban_row['ban_give_reason']) : '';
	...
	function reset_login_keys($user_id = false)
	{
		...
		{
			$sql = 'UPDATE ' . USERS_TABLE . '
				SET user_lastvisit = ' . (int) $row['session_time'] . ", user_lastpage = '" . phpbb::$db->sql_escape($row['session_page']) . "'
				WHERE user_id = " . (int) $user_id;
			phpbb::$db->sql_query($sql);
		}
		...
		$sql_where .= ($user_id === (int) $this->data['user_id'])
			? " AND session_id <> '" . phpbb::$db->sql_escape($this->session_id) . "'" : '';
	...
	function validate_referer($check_script_path = false)
	{
		...
		if (!(stripos($ref, $host) === 0) && (!phpbb::$config['force_server_vars'] || !(stripos($ref, phpbb::$config['server_name']) === 0)))
		{
			return false;
		}
		else if ($check_script_path && rtrim($this->page['root_script_path'], '/') !== '')
		{
			...
			$server_port = (!empty($_SERVER['SERVER_PORT'])) ? (int) $_SERVER['SERVER_PORT']
...
class user extends session
{
	...
	public $keyoptions = array('viewimg' => 0, 'viewflash' => 1, 'viewsmilies' => 2, 'viewsigs' => 3, 'viewavatars' => 4, 'viewcensors' => 5, 'attachsig' => 6, 'bbcode' => 8, 'smilies' => 9, 'popuppm' => 10, 'sig_bbcode' => 15, 'sig_smilies' => 16, 'sig_links' => 17);
	...
	function user()
	{
		$this->lang_path = phpbb::$phpbb_root_path . 'system/language/';
	}
	...
	function setup($lang_set = false, $style = false)
	{
		if ($this->data['user_id'] != ANONYMOUS)
		{
			$this->lang_name = (file_exists($this->lang_path . $this->data['user_lang'] . "/common." . phpbb::$phpEx))
				? $this->data['user_lang'] : basename(phpbb::$config['default_lang']);
		...
		$include_result = (defined('DEBUG_EXTRA')) ? (include $this->lang_path . $this->lang_name . "/common." . phpbb::$phpEx)
			: (@include $this->lang_path . $this->lang_name . "/common." . phpbb::$phpEx);

		if ($include_result === false)
		{
			die('Language file ' . $this->lang_path . $this->lang_name . "/common." . phpbb::$phpEx . " couldn't be opened.");
		}

		$this->add_lang($lang_set);
		unset($lang_set);

		if (!empty($_GET['style']) && phpbb::$auth->acl_get('a_styles') && !defined('ADMIN_START'))
		{
		...
		{
			// Set up style
			$style = ($style) ? $style : ((!phpbb::$config['override_user_style']) ? $this->data['user_style']
				: phpbb::$config['default_style']);
		}
		...
		$parsed_items = phpbb::$cache->obtain_cfg_items($this->theme);
		...
		if (!$this->theme['theme_storedb'] && $this->theme['parse_css_file'])
		{
			...
			$stylesheet = file_get_contents(phpbb::$phpbb_root_path . "styles/{$this->theme['theme_path']}/theme/stylesheet.css");
			// Match CSS imports
			$matches = array();
			...
			if (sizeof($matches))
			{
				...
				{
					if ($content = @file_get_contents(phpbb::$phpbb_root_path . "styles/{$this->theme['theme_path']}/theme/" . $matches[1][$idx]))
					{
		...
		$this->img_lang = (file_exists(phpbb::$phpbb_root_path . 'styles/' . $this->theme['imageset_path'] . '/imageset/' . $this->lang_name))
		...
		if (!$localised_images)
		{
			...
			if (@file_exists(phpbb::$phpbb_root_path . "styles/{$this->theme['imageset_path']}/imageset/{$this->img_lang}/imageset.cfg"))
			{
				$cfg_data_imageset_data = parse_cfg_file(phpbb::$phpbb_root_path . "styles/{$this->theme['imageset_path']}/imageset/{$this->img_lang}/imageset.cfg");
		...
		if (!defined('DEBUG_EXTRA') && !defined('ADMIN_START') && !defined('IN_INSTALL') && !defined('IN_LOGIN') && file_exists(phpbb::$phpbb_root_path . 'install') && !is_file(phpbb::$phpbb_root_path . 'install'))
		{
		...
		{
			if ($this->load > floatval(phpbb::$config['limit_load']) && !defined('IN_LOGIN') && !defined('IN_ADMIN'))
			{
		...
		if (!defined('IN_ADMIN') && !defined('ADMIN_START') && phpbb::$config['chg_passforce'] && !empty($this->data['is_registered']) && phpbb::$auth->acl_get('u_chgpasswd') && $this->data['user_passchg'] < time() - (phpbb::$config['chg_passforce'] * 86400))
		{
			if (strpos($this->page['query_string'], 'mode=reg_details') === false && $this->page['page_name'] != "ucp." . phpbb::$phpEx)
			{
				redirect(append_sid(phpbb::$phpbb_root_path . "ucp." . phpbb::$phpEx, 'i=profile&amp;mode=reg_details'));
			}
		}
	 ...
	 *
	 * Example call: <samp>phpbb::$user->lang('NUM_POSTS_IN_QUEUE', 1);</samp>
	 *
	...
	function set_lang(&$lang, &$help, $lang_file, $use_db = false, $use_help = false)
	{
		...
		if (!$use_db)
		{
			...
			{
				$language_filename = $this->lang_path . $this->lang_name . '/' . substr($lang_file, 0, stripos($lang_file, '/') + 1) . 'help_' . substr($lang_file, stripos($lang_file, '/') + 1) . '.' . phpbb::$phpEx;
			}
			else
			{
				$language_filename = $this->lang_path . $this->lang_name . '/' . (($use_help) ? 'help_'
					: '') . $lang_file . '.' . phpbb::$phpEx;
			}
	...
	function img($img, $alt = '', $width = false, $suffix = '', $type = 'full_tag')
	{
		...
		if (empty($img_data))
		{
			...
			// Use URL if told so
			$root_path = (defined('PHPBB_USE_BOARD_URL_PATH') && PHPBB_USE_BOARD_URL_PATH) ? generate_board_url() . '/'
				: phpbb::$phpbb_root_path;

			$path = 'styles/' . rawurlencode($this->theme['imageset_path']) . '/imageset/' . ($this->img_array[$img]['image_lang']
				? $this->img_array[$img]['image_lang'] . '/' : '') . $this->img_array[$img]['image_filename'];
			...
			if ($img == 'site_logo' && file_exists(phpbb::$phpbb_root_path . $path))
			{


				$img_file_hashes = phpbb::$cache->get('imageset_site_logo_md5');
				...
				{
					$img_file_hashes[$key] = md5(file_get_contents(phpbb::$phpbb_root_path . $path));
					phpbb::$cache->put('imageset_site_logo_md5', $img_file_hashes);
				}
		...
		switch ($type)
		{
			...
			default:
				...
				return '<img src="' . $img_data['src'] . '"' . (($use_width) ? ' width="' . $use_width . '"'
					: '') . (($img_data['height']) ? ' height="' . $img_data['height'] . '"'
					: '') . ' alt="' . $alt . '" title="' . $alt . '" />';
				break;
		}
	}
	 ...
	 * @return bool
	 */
	public function leave_newly_registered()
	{
