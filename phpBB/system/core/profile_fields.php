...
class custom_profile
{
	...
	function build_cache()
	{
		...
		$sql = 'SELECT l.*, f.*
			...
			WHERE l.lang_id = ' . phpbb::$user->get_iso_lang_id() . '
				...
				((!phpbb::$auth->acl_gets('a_', 'm_') && !phpbb::$auth->acl_getf_global('m_')) ? '	AND f.field_hide = 0 ' : '') . '
	...
	*/
	function update_profile_field_data($user_id, &$cp_data)
	{
		...
		foreach ($cp_data as $key => $value)
		{
			...
			$cp_data_sql[$left_delim . ((phpbb::$db->sql_layer == 'firebird' || phpbb::$db->sql_layer == 'oracle') ? strtoupper($key) : $key) . $right_delim] = $value;
		}

		$sql = 'UPDATE ' . PROFILE_FIELDS_DATA_TABLE . '
			SET ' . phpbb::$db->sql_build_array('UPDATE', $cp_data_sql) . "
		...
		if (!phpbb::$db->sql_affectedrows())
		{
			...
			$sql = 'INSERT INTO ' . PROFILE_FIELDS_DATA_TABLE . ' ' . phpbb::$db->sql_build_array('INSERT', $cp_data_sql);
			phpbb::$db->sql_query($sql);
	...
	*/
	function get_var($field_validation, &$profile_row, $default_value, $preview)
	{
		...
		{
			$value = (isset($_REQUEST[$profile_row['field_ident']])) ? true : ((!isset(phpbb::$user->profile_fields[$user_ident]) || $preview) ? $default_value : phpbb::$user->profile_fields[$user_ident]);
		}
		else if ($profile_row['field_type'] == FIELD_INT)
		{
			...
			{
				if (!$preview && array_key_exists($user_ident, phpbb::$user->profile_fields) && is_null(phpbb::$user->profile_fields[$user_ident]))
				{
					$value = NULL;
				}
		...
		{
			$value = (isset($_REQUEST[$profile_row['field_ident']])) ? request_var($profile_row['field_ident'], $default_value, true) : ((!isset(phpbb::$user->profile_fields[$user_ident]) || $preview) ? $default_value : phpbb::$user->profile_fields[$user_ident]);
	...
	private function generate_int($profile_row, $preview = false)
	{
		...
		phpbb::$template->assign_block_vars($this->profile_types[$profile_row['field_type']], array_change_key_case($profile_row, CASE_UPPER));
	}
