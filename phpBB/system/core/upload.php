...
class filespec
{
	...
	*/
	function move_file($destination, $overwrite = false, $skip_image_check = false, $chmod = false)
	{
		...
		$this->destination_path = phpbb::$phpbb_root_path . $destination;
	...
	*/
	function additional_checks()
	{
		...
		if ($this->upload->max_filesize && ($this->get('filesize') > $this->upload->max_filesize || $this->filesize == 0))
		{
			...
			$this->error[] = sprintf(phpbb::$user->lang[$this->upload->error_prefix . 'WRONG_FILESIZE'], $max_filesize['value'], $max_filesize['unit']);
...
class fileupload
{
	...
	*/
	function remote_upload($upload_url)
	{
		...
		while (!@feof($fsock))
		{
			if ($get_info)
			{
				...
				if ($this->max_filesize && $filesize > $this->max_filesize)
				{
					...
					$file = new fileerror(sprintf(phpbb::$user->lang[$this->error_prefix . 'WRONG_FILESIZE'], $max_filesize['value'], $max_filesize['unit']));
					return $file;
				}
		...
		$tmp_path = (!@ini_get('safe_mode') || strtolower(@ini_get('safe_mode')) == 'off') ? false : phpbb::$phpbb_root_path . 'cache';
	...
	function assign_internal_error($errorcode)
	{
		...
		switch ($errorcode)
		{
			case 1:
				...
				$error = (empty($max_filesize)) ? phpbb::$user->lang[$this->error_prefix . 'PHP_SIZE_NA'] : sprintf(phpbb::$user->lang[$this->error_prefix . 'PHP_SIZE_OVERRUN'], $max_filesize, phpbb::$user->lang[$unit]);
			break;

			case 2:
				$max_filesize = get_formatted_filesize($this->max_filesize, false);

				$error = sprintf(phpbb::$user->lang[$this->error_prefix . 'WRONG_FILESIZE'], $max_filesize['value'], $max_filesize['unit']);
			break;
	...
	*/
	function common_checks(&$file)
	{
		...
		if ($this->max_filesize && ($file->get('filesize') > $this->max_filesize || $file->get('filesize') == 0))
		{
			...
			$file->error[] = sprintf(phpbb::$user->lang[$this->error_prefix . 'WRONG_FILESIZE'], $max_filesize['value'], $max_filesize['unit']);
		}
