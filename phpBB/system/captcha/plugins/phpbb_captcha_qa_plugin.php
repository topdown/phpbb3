...
class phpbb_captcha_qa
{
	...
	*/
	function init($type)
	{
		...
		phpbb::$user->add_lang('captcha_qa');
		...
		$this->type = (int) $type;
		$this->question_lang = phpbb::$user->lang_name;
		...
		$sql = 'SELECT question_id
			...
			WHERE lang_iso = '" . phpbb::$db->sql_escape(phpbb::$user->lang_name) . "'";
		...
		{
			$this->question_lang = phpbb::$config['default_lang'];
	...
	*/
	function is_installed()
	{
		...
		{
			include("$phpbb_root_path/system/db/db_tools.$phpEx");
		}
		...
		if ($row = phpbb::$db->sql_fetchrow($result))
		{
			...
			if (sizeof($sql_in))
			{
				$sql = 'DELETE FROM ' . CAPTCHA_QA_CONFIRM_TABLE . '
					WHERE ' . phpbb::$db->sql_in_set('confirm_id', $sql_in);
	...
	*/
	function install()
	{
		...
		if (!class_exists('phpbb_db_tools'))
		{
			include(phpbb::$phpbb_root_path . "/system/db/db_tools." . phpbb::$phpEx);
		}
		$db_tool = new phpbb_db_tools($db);
	...
	*/
	function validate()
	{
		...
		if (!$this->confirm_id)
		{
			$error = phpbb::$user->lang['CONFIRM_QUESTION_WRONG'];
		}
		else
		{
	...
	*/
	function select_question()
	{
		...
		$sql = 'INSERT INTO ' . CAPTCHA_QA_CONFIRM_TABLE . ' ' . phpbb::$db->sql_build_array('INSERT', array(
	...
	*/
	function load_confirm_id()
	{
		...
		$sql = 'SELECT confirm_id
			...
			WHERE 
				session_id = '" . phpbb::$db->sql_escape(phpbb::$user->session_id) . "'
				AND lang_iso = '" . phpbb::$db->sql_escape($this->question_lang) . "'
	...
	function load_answer()
	{
		...
		$sql = 'SELECT con.question_id, attempts, question_text, strict
			...
			WHERE con.question_id = qes.question_id
				AND confirm_id = '" . phpbb::$db->sql_escape($this->confirm_id) . "'
				AND session_id = '" . phpbb::$db->sql_escape(phpbb::$user->session_id) . "'
				AND qes.lang_iso = '" . phpbb::$db->sql_escape($this->question_lang) . "'
				AND confirm_type = " . $this->type;
		$result = phpbb::$db->sql_query($sql);
		$row = phpbb::$db->sql_fetchrow($result);
		phpbb::$db->sql_freeresult($result);
	...
	*/
	function acp_page($id, &$module)
	{
		...
		{
			if ($this->get_class_name() !== phpbb::$config['captcha_plugin'] || !$this->acp_is_last($question_id))
			{
				if (confirm_box(true))
				{
					$this->acp_delete_question($question_id);

					trigger_error(phpbb::$user->lang['QUESTION_DELETED'] . adm_back_link($list_url));
				}
				else
				{
			...
			{
				trigger_error(phpbb::$user->lang['QA_LAST_QUESTION'] . adm_back_link($list_url), E_USER_WARNING);
			}
		}
		else
		{
			...
			{
				phpbb::$template->assign_block_vars('langs', array(
					'ISO' => $lang,
					'NAME' => $entry['name'],
				));
			}
			...
			if ($question_id)
			{
				...
				{
					trigger_error(phpbb::$user->lang['FORM_INVALID'] . adm_back_link($list_url));
				}
			}
			else
			{
			...
			if ($submit && check_form_key($form_key))
			{
				...
				else
				{
					...
					trigger_error(phpbb::$user->lang['CONFIG_UPDATED'] . adm_back_link($list_url));
				}
			}
			else if ($submit)
			{
				trigger_error(phpbb::$user->lang['FORM_INVALID'] . adm_back_link($list_url), E_USER_WARNING);
			}
		}
	}
	...
	*/
	function acp_question_list(&$module)
	{
		...
		while ($row = phpbb::$db->sql_fetchrow($result))
		{
			...
			phpbb::$template->assign_block_vars('questions', array(
	...
	*/
	function acp_get_question_data($question_id)
	{
		...
		if ($question_id)
		{
			...
			$result = phpbb::$db->sql_query($sql);
			$question = phpbb::$db->sql_fetchrow($result);
			phpbb::$db->sql_freeresult($result);
	...
	*/
	function acp_update_question($data, $question_id)
	{
		...
		$sql = 'UPDATE ' . CAPTCHA_QUESTIONS_TABLE . '
			SET ' . phpbb::$db->sql_build_array('UPDATE', $question_ary) . "
			WHERE question_id = $question_id";
		phpbb::$db->sql_query($sql);

		$this->acp_insert_answers($data, $question_id);

		phpbb::$cache->destroy('sql', CAPTCHA_QUESTIONS_TABLE);
	}
	...
	*/
	function acp_add_question($data)
	{
		...
		$sql = 'INSERT INTO ' . CAPTCHA_QUESTIONS_TABLE . ' ' . phpbb::$db->sql_build_array('INSERT', $question_ary);
		phpbb::$db->sql_query($sql);

		$question_id = phpbb::$db->sql_nextid();
	...
	*/
	function acp_insert_answers($data, $question_id)
	{
		...
		foreach ($data['answers'] as $answer)
		{
			...
			$sql = 'INSERT INTO ' . CAPTCHA_ANSWERS_TABLE . ' ' . phpbb::$db->sql_build_array('INSERT', $answer_ary);
			phpbb::$db->sql_query($sql);
		}

		phpbb::$cache->destroy('sql', CAPTCHA_ANSWERS_TABLE);
	}
	...
	*/
	function acp_delete_question($question_id)
	{
		...
		phpbb::$cache->destroy('sql', $tables);
	}
