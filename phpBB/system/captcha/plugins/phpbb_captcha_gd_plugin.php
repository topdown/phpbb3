...
//		'captcha_gd'					=> 'CAPTCHA_GD_PREVIEWED',
	...
	function phpbb_captcha_gd()
	{
		...
		{
			include(phpbb::$phpbb_root_path . 'system/captcha/captcha_gd.' . phpbb::$phpEx);
		}
	}
	...
	*/
	...
	function acp_page($id, &$module)
	{
		...
		else
		{
			...
			{
				$var = (isset($_REQUEST[$captcha_var])) ? request_var($captcha_var, 0) : phpbb::$config[$captcha_var];
	...
	function execute_demo()
	{
		...
		foreach ($this->captcha_vars as $captcha_var => $template_var)
		{
				phpbb::$config[$captcha_var] = request_var($captcha_var, (int) phpbb::$config[$captcha_var]);
		}
		parent::execute_demo();
		$config = $config_old;
	}
